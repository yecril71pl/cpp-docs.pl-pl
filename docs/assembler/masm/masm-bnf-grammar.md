---
title: Gramatyka programu Microsoft Macro Assembler BNF
description: BNF opis MASM dla x64.
ms.date: 12/17/2019
helpviewer_keywords:
...
ms.locfileid: "81317974"
---
# <a name="microsoft-macro-assembler-bnf-grammar"></a>Gramatyka programu Microsoft Macro Assembler BNF

Ta strona zawiera opis BNF gramatyki MASM. Jest on dostarczony jako dodatek do odniesienia i nie jest gwarantowane, aby być kompletne. Szczegółowe informacje na temat słów kluczowych, parametrów, operacji itd.

Aby zilustrować użycie BNF, na poniższym diagramie przedstawiono definicję dyrektywy TYPEDEF, począwszy od nieterminalnej *`typedefDir`*.

![Przykład MASM BNF](media/bnf.png)

Wpisy pod każdym poziomym nawiasem klamrowym **`NEAR32`** **`FAR16`** są **`FAR32`** zaciskami, takimi jak **`NEAR16`**, , i . Lub, są one nonterminals *`qualifier`* *`qualifiedType`* takich *`distance`* jak *`protoSpec`* , , i które mogą być dalej zdefiniowane. Każdy kursywa nieterminowa w *`typedefDir`* definicji jest również wpisem w BNF. Trzy pionowe kropki wskazują definicję rozgałęziania dla nieterminalnej, której ze względu na prostotę ta liczba nie ilustruje.

Gramatyka BNF umożliwia definicje cykliczne. Na przykład gramatyka używa qualifiedType jako możliwej definicji qualifiedType, który jest również składnikiem definicji kwalifikatora. Symbol "|" określa wybór między wyrażeniami alternatywnymi, na przykład *`endOfLine`*  |  *`comment`*. Podwójne nawiasy klamrowe określają parametr opcjonalny, na *`macroParmList`* przykład Nawiasy nie są wyświetlane w kodzie źródłowym.
...
*`character`*\
&nbsp;&nbsp;&nbsp;&nbsp;Dowolny znak z porządkiem w zakresie od 0 do 255 z wyjątkiem wysuwu liniowego (10).
...
*`delimiter`*\
&nbsp;&nbsp;&nbsp;&nbsp;Dowolny znak z wyjątkiem*`whiteSpaceCharacter`*
...
*`id`*\
&nbsp;&nbsp;&nbsp;&nbsp;Pierwszy znak identyfikatora może być znakiem alfabetycznym`[A–Za-z]`(lub dowolnym z tych `@ _ $ ?` czterech znaków) lub dowolnym z tych czterech znaków:`[0–9]`Pozostałe znaki mogą być dowolnym z tych samych znaków lub cyfrą dziesiętną ( ). Maksymalna długość to 247 znaków.
...
*`stringChar`*\
&nbsp;&nbsp;&nbsp;&nbsp;*`quote`**`quote`* | Dowolny znak z wyjątkiem cytatu.
...
*`text`*\
&nbsp;&nbsp;&nbsp;&nbsp;*`textLiteral`* | *`text`* znak | **`!`** *`character`* *`text`* | *`character`* | **`!`***`character`*
