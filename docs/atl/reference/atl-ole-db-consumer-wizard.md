 ...
 **połączenia**  
  ...
  Dla danych SQL:  
    ...
    **Użyj nazwy źródła danych:** tej listy rozwijanej wyświetla źródła danych zarejestrowane na tym komputerze. Możesz skonfigurować źródła danych, które wcześniej przy użyciu Administratora źródła danych ODBC- lub -**Użyj parametrów połączenia:** wprowadź parametry połączenia, już uzyskali, lub kliknij przycisk **kompilacji** przycisk; **Wybierz źródło danych** pojawi się okno dialogowe. Wybierz źródło danych maszyny lub pliku, a następnie kliknij przycisk **OK**.  
 ...
 **Opartego na atrybutach**  
 ...
 -   Po wybraniu odbiorcy **typu** tabeli, kreator używa `db_source` i `db_table` atrybutów, aby utworzyć tabelę i metoda dostępu do tabeli deklaracje klas i używa `db_column` do tworzenia mapy kolumny, na przykład:  
...
```
   ...
   zamiast używania `CTable` klasy szablonu, aby zadeklarować w tabeli i akcesora tabeli i BEGIN_COLUMN_MAP i END_COLUMN_MAP makra do tworzenia mapy kolumny, na przykład:  
...
-   Po wybraniu odbiorcy **typu** polecenia, kreator używa `db_source` i `db_command` atrybutów i używa `db_column` do tworzenia mapy kolumny, na przykład:  
