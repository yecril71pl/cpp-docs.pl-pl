...
|[CComPtrBase::CopyTo](#copyto)|Wywołaj tę metodę, aby `CComPtrBase` skopiować wskaźnik do innej zmiennej wskaźnika.|
|[CComPtrBase::Detach](#detach)|Wywołaj tę metodę, aby zwolnić własność wskaźnika.|
|[CComPtrBase:: isequalobject](#isequalobject)|Wywołaj tę metodę, aby sprawdzić, `IUnknown` czy określone punkty do tego samego obiektu są `CComPtrBase` skojarzone z obiektem.|
...
|[CComPtrBase:: SetSite](#setsite)|Wywołaj tę metodę, aby ustawić lokację `CComPtrBase` obiektu `IUnknown` do obiektu nadrzędnego.|
...
*P2*<br/>
`CComPtrBase` Obiekt przejdzie na własność tego wskaźnika.

### <a name="remarks"></a>Uwagi

`Attach`wywołuje [CComPtrBase:: Release](#release) w istniejącej zmiennej składowej [CComPtrBase::p](#p) , a następnie przypisuje `CComPtrBase::p`P2 do. Gdy obiekt przejmuje własność wskaźnika, zostanie automatycznie wywołana `Release` na wskaźniku, który usunie wskaźnik i wszystkie przydzielono dane, jeśli liczba odwołań w obiekcie przejdzie do 0. `CComPtrBase`
...
*pUnkOuter*<br/>
Jeśli wartość jest równa NULL, wskazuje, że obiekt nie jest tworzony w ramach agregacji. Jeśli wartość nie jest równa null, jest wskaźnikiem do `IUnknown` interfejsu obiektu agregacji (kontrolka `IUnknown`).
...
Wywołaj tę metodę, aby `CComPtrBase` skopiować wskaźnik do innej zmiennej wskaźnika.
...
Zwraca S_OK w przypadku powodzenia, E_POINTER w przypadku niepowodzenia.

### <a name="remarks"></a>Uwagi

Kopiuje wskaźnik do *PPT.* `CComPtrBase` Liczba odwołań dla zmiennej składowej [CComPtrBase::p](#p) jest zwiększana.
...
Wywołaj tę metodę, aby sprawdzić, `IUnknown` czy określone punkty do tego samego obiektu są `CComPtrBase` skojarzone z obiektem.
...
Zwraca wartość [CComPtrBase::p](#p); oznacza to wskaźnik do obiektu, do którego odwołuje `CComPtrBase` się obiekt.
...
Zwraca S_OK w przypadku powodzenia lub E_NOINTERFACE w przypadku niepowodzenia.

### <a name="remarks"></a>Uwagi

Ta metoda wywołuje [IUnknown:: QueryInterface](/windows/win32/api/unknwn/nf-unknwn-iunknown-queryinterface(q_)).
...
Wywołaj tę metodę, aby ustawić lokację `CComPtrBase` obiektu `IUnknown` do obiektu nadrzędnego.
