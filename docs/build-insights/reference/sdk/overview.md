---
title: C++Zestaw SDK usługi Build Insights
...
ms.locfileid: "78332398"
---
# <a name="c-build-insights-sdk"></a>C++Zestaw SDK usługi Build Insights
...
Zestaw C++ SDK usługi Build Insights to zbiór interfejsów API, które umożliwiają tworzenie spersonalizowanych narzędzi na podstawie C++ platformy Build Insights. Ta strona zawiera ogólne omówienie ułatwiające rozpoczęcie pracy.

## <a name="obtaining-the-sdk"></a>Uzyskiwanie zestawu SDK

Zestaw SDK usługi C++ Build Insights można pobrać jako pakiet NuGet, wykonując następujące czynności:

1. W programie Visual Studio 2017 lub nowszym Utwórz nowy C++ projekt.
...
Zapoznaj się z informacjami na temat ogólnych pojęć związanych z zestawem SDK. Możesz również uzyskać dostęp do oficjalnego [ C++ repozytorium usługi Build Insights](https://github.com/microsoft/cpp-build-insights-samples) w witrynie GitHub, aby zobaczyć C++ przykłady rzeczywistych aplikacji korzystających z zestawu SDK.

## <a name="collecting-a-trace"></a>Zbieranie śladu

Korzystanie z C++ zestawu SDK usługi Build Insights do analizowania zdarzeń pochodzących z MSVC łańcucha narzędzi wymaga najpierw zebrania śladu. Zestaw SDK wykorzystuje śledzenie zdarzeń systemu Windows (ETW) jako podstawową technologię śledzenia. Zbieranie śladów można wykonać na dwa sposoby:
...
1. Uruchom następujące polecenie: `vcperf /stopnoanalyze MySessionName outputTraceFile.etl`
   ...
   > [!IMPORTANT]
   > Użyj `/stopnoanalyze` polecenia podczas zatrzymywania śledzenia przy użyciu vcperf. Nie można użyć zestawu C++ SDK usługi Build Insights do analizowania śladów zatrzymanych przez regularne `/stop` polecenie.

### <a name="method-2-programmatically"></a>Metoda 2. programowane

Użyj dowolnej z tych C++ funkcji zbierania danych śledzenia zestawu SDK usługi Build Insights, aby programowo uruchamiać i zatrzymywać dane śledzenia. **Program wykonujący te wywołania funkcji musi mieć uprawnienia administracyjne.** Tylko funkcje uruchamiania i zatrzymywania śledzenia wymagają uprawnień administracyjnych. Wszystkie inne funkcje w zestawie C++ SDK usługi Build Insights można wykonać bez nich.
...
Po utworzeniu śledzenia ETW Użyj zestawu SDK usługi C++ Build Insights, aby go rozpakować. Zestaw SDK udostępnia zdarzenia w formacie, który umożliwia szybkie tworzenie narzędzi. Nie zalecamy korzystania z nieprzetworzonego śledzenia ETW bez użycia zestawu SDK. Format zdarzenia używany przez MSVC jest nieudokumentowany, zoptymalizowany pod kątem skalowania do dużych kompilacji i trudno jest zrozumieć. Ponadto interfejs API C++ zestawu SDK usługi Build Insights jest stabilny, podczas gdy pierwotny format śledzenia ETW może ulec zmianie bez powiadomienia.
...
|--|--|--|--|
| Konfigurowanie wywołań zwrotnych zdarzeń | [IAnalyzer](other-types/ianalyzer-class.md)<br />[IRelogger](other-types/irelogger-class.md) | [ANALYSIS_CALLBACKS](other-types/analysis-callbacks-struct.md)<br />[RELOG_CALLBACKS](other-types/relog-callbacks-struct.md) | Zestaw C++ SDK usługi Build Insights udostępnia zdarzenia za pomocą funkcji wywołania zwrotnego. W C++programie zaimplementuj funkcje wywołania zwrotnego, tworząc Analizator lub klasę rerejestratora, która dziedziczy interfejs IAnalyzer lub IRelogger. W języku C Zaimplementuj wywołania zwrotne w funkcjach globalnych i podaj do nich wskaźniki w strukturze ANALYSIS_CALLBACKS lub RELOG_CALLBACKS. |
| Kompilowanie grup | [MakeStaticAnalyzerGroup](functions/make-static-analyzer-group.md)<br />[MakeStaticReloggerGroup](functions/make-static-relogger-group.md)<br />[MakeDynamicAnalyzerGroup](functions/make-dynamic-analyzer-group.md)<br />[MakeDynamicReloggerGroup](functions/make-dynamic-relogger-group.md) |  | C++ Interfejs API udostępnia funkcje i typy pomocnika do grupowania wielu obiektów analizatora i ponownego rejestratora. Grupy to sposób, aby podzielić złożoną analizę na prostsze czynności. [vcperf](https://github.com/microsoft/vcperf) jest zorganizowany w ten sposób. |
| Analizowanie i ponowne rejestrowanie | [Analiza](functions/analyze.md)<br />[Relog](functions/relog.md) | [Analiza](functions/analyze-a.md)<br />[AnalyzeW](functions/analyze-w.md)<br />[RelogA](functions/relog-a.md)<br />[RelogW](functions/relog-w.md) |  |
...
Ponowne rejestrowanie dotyczy analiz specjalnych, które wymagają utworzenia pliku wyjściowego ETW. Korzystając z C++ ponownego rejestrowania, można przetłumaczyć zdarzenia usługi Build Insights na własny format zdarzenia ETW. Odpowiednie użycie ponownego rejestrowania może spowodować, że dane z C++ usługi Build Insights zostaną przechwycone do istniejących narzędzi i infrastruktury funkcji ETW. Na przykład [vcperf](https://github.com/microsoft/vcperf) korzysta z interfejsów rejestrowania. Jest to spowodowane tym, że musi utworzyć dane analizatora wydajności systemu Windows, narzędzie ETW, które może zrozumieć. Niektóre z wcześniejszych informacji na temat działania funkcji ETW są wymagane, jeśli planujesz korzystanie z interfejsów rejestrowania.
...
Relacje nadrzędny-podrzędny każdego działania i prostego zdarzenia są wskazane w [tabeli zdarzeń](event-table.md). Znajomość tych relacji jest ważna w przypadku używania C++ zdarzeń kompilacji szczegółowych. Często należy polegać na nich, aby zrozumieć pełen kontekst zdarzenia.
...
### <a name="consuming-events-provided-by-the-c-build-insights-sdk"></a>Zużywanie zdarzeń dostarczonych przez zestaw C++ SDK usługi Build Insights

#### <a name="the-event-stack"></a>Stos zdarzeń

Za każdym C++ razem, gdy zestaw SDK usługi Build Insights dostarcza zdarzenie, ma postać stosu. Ostatni wpis w stosie to bieżące zdarzenie, a wpisy, zanim są jego hierarchią nadrzędną. Na przykład [LTCG](event-table.md#ltcg) zdarzenia uruchamiania i zatrzymywania występują podczas przebiegu 1 konsolidatora. W takim przypadku otrzymany stos zawiera: \[[konsolidator](event-table.md#linker), [PASS1](event-table.md#pass1), LTCG\]. Hierarchia nadrzędna jest wygodna, ponieważ można śledzić zdarzenia do jego katalogu głównego. Jeśli działanie LTCG wymienione powyżej jest powolne, możesz od razu sprawdzić, które wywołanie konsolidatora zostało uwzględnione.

#### <a name="matching-events-and-event-stacks"></a>Zgodne zdarzenia i stosy zdarzeń

Zestaw C++ SDK usługi Build Insights zapewnia każde zdarzenie w śladach, ale większość z nich jest czasochłonna. W niektórych przypadkach można zadbać o to tylko podzbiór *stosów zdarzeń*. Zestaw SDK udostępnia funkcje ułatwiające szybkie wyodrębnienie potrzebnych zdarzeń lub stosu zdarzeń oraz odrzucanie tych elementów. Jest to wykonywane przez następujące zgodne funkcje:
...
Funkcja dopasowywania stosu zdarzeń, taka jak `MatchEventStack` zezwalać na przerwy podczas opisywania hierarchii nadrzędnej do dopasowania. Na przykład możesz powiedzieć, że jesteś zainteresowani \[[konsolidatora](event-table.md#linker), [LTCG](event-table.md#ltcg)\] Stack. Będzie on również pasował do \[KONSOLIDATORa, [PASS1](event-table.md#pass1)LTCG\] stos. Ostatni określony typ musi być typem zdarzenia do dopasowania i nie jest częścią hierarchii nadrzędnej.

#### <a name="capture-classes"></a>Klasy przechwytywania

Korzystając z funkcji `Match*`, należy określić typy, które mają być zgodne. Te typy są wybierane z listy *klas przechwytywania*. Klasy przechwytywania pochodzą z kilku kategorii opisanych poniżej.

| Kategoria | Opis |
|--|--|
| Dokładny | Te klasy przechwytywania są używane do dopasowania określonego typu zdarzenia i nie są inne. Przykładem jest Klasa [kompilatora](cpp-event-data-types/compiler.md) , która odpowiada zdarzeniu [kompilatora](event-table.md#compiler) . |
...
Po pomyślnym zakończeniu dopasowania funkcje `Match*` konstruują obiekty klasy przechwytywania i przekazują je do określonej funkcji. Użyj tych obiektów klasy przechwytywania, aby uzyskać dostęp do właściwości zdarzenia.
