---
title: Elementy wewnętrzne programu MSBuild C++ dla projektów w programie Visual Studio
ms.date: 02/26/2020
helpviewer_keywords:
- MSBuild overview
...
- % VSINSTALLDIR% MSBuild\\Microsoft\\VC\\*wersja*\\VCTargets\\
...
- % VSINSTALLDIR% MSBuild\\Microsoft\\VC\\*wersja*\\VCTargets\\platform\\*platform*\\
...
- % VSINSTALLDIR% MSBuild\\Microsoft\\VC\\*wersja*\\VCTargets\\platform\\*platform*\\PlatformToolsets\\zestaw *narzędzi*\\

  Zawiera katalogi, które umożliwiają kompilacji generowanie C++ aplikacji przy użyciu określonego zestawu *narzędzi*. Symbol zastępczy *platformy* reprezentuje podkatalog ARM, Win32 lub x64. Symbol *zastępczy zestawu narzędzi reprezentuje* podkatalog zestawu narzędzi.
...
- % VSINSTALLDIR% Common7\\IDE\\VC\\VCTargets\\platform\\*platform*\\

  Zawiera pliki docelowe i właściwości specyficzne dla platformy, które zastępują elementy docelowe i właściwości w katalogu nadrzędnym. Ten katalog zawiera również bibliotekę DLL, która definiuje zadania, które są używane przez obiekty docelowe w tym katalogu. Symbol zastępczy *platformy* reprezentuje podkatalog ARM, Win32 lub x64.

- % VSINSTALLDIR% Common7\\IDE\\VC\\VCTargets\\platform\\*platform*\\PlatformToolsets\\zestaw *narzędzi*\\

  Zawiera katalogi, które umożliwiają kompilacji generowanie C++ aplikacji przy użyciu określonego zestawu *narzędzi*. Symbol zastępczy *platformy* reprezentuje podkatalog ARM, Win32 lub x64. Symbol *zastępczy zestawu narzędzi reprezentuje* podkatalog zestawu narzędzi.

### <a name="visual-studio-2015-and-earlier"></a>Visual Studio 2015 i starsze

- *dysk*: pliki programu\\ *(x86)* \\MSBuild\\Microsoft. Cpp (x86)\\v 4.0\\*wersja*\\
...
Właściwość `PlatformToolset` określa, który zestaw narzędzi MSVC jest używany w kompilacji. Domyślnie używany jest bieżący zestaw narzędzi. Gdy ta właściwość jest ustawiona, jej wartość jest pobierana z ciągami literału w celu utworzenia ścieżki. Jest to katalog, który zawiera pliki właściwości i obiekty docelowe wymagane do skompilowania projektu dla konkretnej platformy. Zestaw narzędzi platformy musi być zainstalowany do kompilowania przy użyciu tej wersji zestawu narzędzi platformy.

Na przykład ustaw właściwość `PlatformToolset` na `v140`, aby użyć narzędzi i bibliotek programu Visual Studio 2015 do kompilowania aplikacji:

`msbuild myProject.vcxproj /p:PlatformToolset=v140`

### <a name="preferredtoolarchitecture-property"></a>Właściwość PreferredToolArchitecture

Właściwość `PreferredToolArchitecture` określa, czy kompilator i narzędzia z 32-bitowym, 64-bitowym są używane w kompilacji. Ta właściwość nie ma wpływu na architekturę ani konfigurację platformy wyjściowej. Domyślnie program MSBuild używa wersji x86 kompilatora i narzędzi, jeśli ta właściwość nie jest ustawiona.

Na przykład ustaw właściwość `PreferredToolArchitecture` na `x64`, aby użyć kompilatora 64-bitowego oraz narzędzi do kompilowania aplikacji:

`msbuild myProject.vcxproj /p:PreferredToolArchitecture=x64`

### <a name="useenv-property"></a>Właściwość UseEnv

Domyślnie ustawienia specyficzne dla platformy dla bieżącego projektu zastępują zmienne środowiskowe PATH, INCLUDE, LIB, LIBPATH, CONFIGURATION i PLATFORM. Ustaw właściwość `UseEnv` na **wartość true** , aby zagwarantować, że zmienne środowiskowe nie zostaną zastąpione.
...
| ------ | ----------- |
| BscMake | Wykonuje narzędzie do konserwacji informacji przeglądarki Microsoft, BSCMAKE. exe. |
...
| Czyst | Usuwa tymczasowe i pośrednie pliki kompilacji. |
...
| Odtworzyć | Czyści, a następnie kompiluje projekt. |
...
> [!NOTE]
> W programie Visual Studio 2017 i nowszych C++ obsługa projektu dla plików **XSD** jest przestarzała. Nadal można użyć **programu Microsoft. VisualC. CppCodeProvider** , ręcznie dodając **CppCodeProvider. dll** do pamięci podręcznej GAC.

## <a name="see-also"></a>Zobacz też

[Odwołanie do zadania programu MSBuild](/visualstudio/msbuild/msbuild-task-reference)\
[BscMake\ zadania](/visualstudio/msbuild/bscmake-task)
\ [zadań CL](/visualstudio/msbuild/cl-task)
[Cppclean —\ zadania](/visualstudio/msbuild/cppclean-task)
\ [zadania lib](/visualstudio/msbuild/lib-task)
[Połącz\ zadania](/visualstudio/msbuild/link-task)
[MIDL\ zadania](/visualstudio/msbuild/midl-task)
[MT Task](/visualstudio/msbuild/mt-task)\
[RC\ zadań](/visualstudio/msbuild/rc-task)
[SetEnv\ zadania](/visualstudio/msbuild/setenv-task)
[Vcmessage —\ zadania](/visualstudio/msbuild/vcmessage-task)
