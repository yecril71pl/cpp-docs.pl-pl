...
ms.locfileid: "34704243"
---
# <a name="optimization-best-practices"></a>Najlepsze praktyki optymalizacji

Ten dokument zawiera opis najlepsze rozwiązania dotyczące optymalizacji w programie Visual C++.
...
Visual C++ obsługuje *Optymalizacja sterowana profilem* (PGO). Tego rodzaju optymalizacji korzysta z danych profilu z wykonaniami szkolenia instrumentowanej wersji aplikacji do nowszej optymalizacji aplikacji. Przy użyciu PGO może zająć dużo czasu, dlatego nie może być coś, która używa każdy Deweloper, ale firma Microsoft zaleca PGO dla kompilacji ostatecznej wersji produktu. Aby uzyskać więcej informacji, zobacz [optymalizacje Profile-Guided](../../build/reference/profile-guided-optimizations.md).

Ponadto *Optymalizacja Całoprogramowa* (również wie jak generowanie kodu w czasie konsolidacji) i **/O1** i **/O2** ulepszono optymalizacji. Ogólnie rzecz biorąc będzie szybsze niż w tej samej aplikacji kompilowane przy użyciu kompilatora starszych aplikacji kompilowane przy użyciu jednej z tych opcji. 
...
### <a name="which-level-of-optimization-to-use"></a>Poziom optymalizacji do użycia
...
**/Gy** przełącznik jest również przydatne. Generuje oddzielny COMDAT dla każdej funkcji, zapewniając większą elastyczność konsolidator, jeśli chodzi o usunięcie nieużywanej Comdat i COMDAT składanie. Jedyną wadą korzystania z interfejsu **/Gy** jest, że może spowodować problemy podczas debugowania. W związku z tym zazwyczaj zalecane jest z niego korzystać. Aby uzyskać więcej informacji, zobacz [/Gy (Włącz funkcję łączenia na poziomie)](../../build/reference/gy-enable-function-level-linking.md).
...
### <a name="floating-point-switches"></a>Przełączniki liczb zmiennoprzecinkowych
...
Funkcje wewnętrzne są funkcja wywołuje którym kompilator ma wewnętrzna wiedzę o wywołaniu i zamiast wywoływania funkcji w bibliotece, jego emituje kod dla tej funkcji. Plik nagłówka \<intrin.h > zawiera wszystkie dostępne funkcje wewnętrzne dla poszczególnych platform obsługiwanego sprzętu.
...
- Kod jest przenośną. Niektóre funkcje wewnętrzne są dostępne w wielu architektury Procesora.

- Kod jest łatwiejsze do czytania, ponieważ kod nadal jest napisany w języku C/C++.

- Kod pobiera korzyści optymalizacje kompilatora. Jak kompilator pobiera lepsze, poprawia generowanie kodu dla funkcji wewnętrznych.
