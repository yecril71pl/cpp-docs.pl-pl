---
title: Bity na znak | Dokumentacja firmy Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-language
ms.tgt_pltfrm: 
ms.topic: article
dev_langs: C++
ms.assetid: ce317fce-67bf-4963-9e6d-13682cda5f3f
caps.latest.revision: "6"
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload: cplusplus
ms.openlocfilehash: 2639923778f51201b66ed1a5c6b4a98886d7e319
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/21/2017
---
# <a name="bits-per-character"></a>Bity na znak
**ANSI 2.2.4.2.1** liczba bitów w znaku  
  
 Liczba bitów znak jest reprezentowany przez manifestu stała **char_bit —**. LIMITY. Określa plik H **char_bit —** jako 8.  
  
## <a name="see-also"></a>Zobacz też  
 [Znaki](../c-language/characters.md)