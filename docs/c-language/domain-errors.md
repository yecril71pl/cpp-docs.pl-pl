---
title: "Błędy domeny | Dokumentacja firmy Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-language
ms.tgt_pltfrm: 
ms.topic: article
dev_langs: C++
helpviewer_keywords: domain errors
ms.assetid: 67faa973-c872-40bc-a25f-2ef5559db96c
caps.latest.revision: "6"
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload: cplusplus
ms.openlocfilehash: fbbf3d5f20c7935dbf8d600c2380929e05c0a367
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/21/2017
---
# <a name="domain-errors"></a>Błędy domeny
**ANSI 4.5.1** wartości zwracanych przez funkcje matematyce na błędy domeny  
  
 Numer błędu. Plik H definiuje błąd stała domeny `EDOM` jako 33.  Zobacz temat pomocy dla określonej funkcji, który spowodował błąd, aby uzyskać informacje o wartości zwracanej.  
  
## <a name="see-also"></a>Zobacz też  
 [Funkcje bibliotek](../c-language/library-functions.md)