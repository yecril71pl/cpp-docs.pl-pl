---
title: Functions (C) | Dokumentacja firmy Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-language
ms.tgt_pltfrm: 
ms.topic: article
dev_langs: C++
helpviewer_keywords:
- functions [C]
- Visual C, functions
ms.assetid: 31839f65-49f4-43e1-85ec-8769924ffa96
caps.latest.revision: "7"
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload: cplusplus
ms.openlocfilehash: 7935b355042ed7221684fe60a69b5286cd39f00d
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/21/2017
---
# <a name="functions-c"></a>Funkcje (C)
Funkcja jest podstawową jednostkę moduły w C. Funkcja zwykle zaprojektowano w celu wykonania określonego zadania, a jego nazwa odzwierciedla często zadania. Funkcja zawiera deklaracje i instrukcje. W tej sekcji opisano, jak zadeklarować, zdefiniuj i wywoływać funkcje C. Inne tematy omówione są:  
  
-   [Omówienie funkcji](../c-language/overview-of-functions.md)  
  
-   [Atrybuty funkcji](../c-language/function-attributes.md)  
  
-   [Określanie konwencji wywoływania](../c-language/specifying-calling-conventions.md)  
  
-   [Funkcje śródwierszowe](../c-language/inline-functions.md)  
  
-   [Biblioteki DLL eksportowania i importowania funkcji](../c-language/dll-import-and-export-functions.md)  
  
-   [Gołe funkcje](../c-language/naked-functions.md)  
  
-   [Klasa magazynu](../c-language/storage-class.md)  
  
-   [Zwracany typ](../c-language/return-type.md)  
  
-   [Argumenty](../c-language/arguments.md)  
  
-   [Parametry](../c-language/parameters.md)  
  
## <a name="see-also"></a>Zobacz też  
 [Dokumentacja języka C](../c-language/c-language-reference.md)