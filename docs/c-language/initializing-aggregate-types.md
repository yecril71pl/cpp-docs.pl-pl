...
W tym przykładzie `nlist` jest zadeklarowana jako tablica o strukturze 2-by-3, każda struktura ma trzy składowe. Wiersz 1 inicjalizacji przypisuje wartości do pierwszego wiersza w `nlist` następujący sposób:
...
1. Drugi lewy nawias klamrowy wskazuje, że inicjalizacja pierwszego zagregowanego elementu członkowskiego `nlist[0]` (czyli struktury w `nlist[0][0]` ) zaczyna się.

1. Pierwszy prawy nawias zamykający zakończył inicjalizację struktury `nlist[0][0]` ; Następny lewy nawias klamrowy zaczyna inicjalizację `nlist[0][1]` .
...
W `list` powyższej strukturze trzy elementy z pierwszego wiersza `m` są inicjowane do 4,0; elementy pozostałego wiersza `m` są domyślnie inicjowane do 0,0.
