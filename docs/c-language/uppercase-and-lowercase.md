---
title: Wielkie i małe litery | Dokumentacja firmy Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-language
ms.topic: language-reference
dev_langs:
- C++
ms.assetid: f33caa65-15ce-4b06-97a6-d19b3ab4d5ad
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: 22eade414a42913d3fbd3c5281336279af6390cb
ms.sourcegitcommit: be2a7679c2bd80968204dee03d13ca961eaa31ff
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 05/03/2018
ms.locfileid: "32386699"
---
# <a name="uppercase-and-lowercase"></a>Wielkie i małe litery
**ANSI 3.1.2** Określa, czy wielkość różnice są znaczące  
  
 Microsoft C traktuje identyfikatorów w ramach jednostki kompilacji jako wielkość liter.  
  
 Konsolidator firmy Microsoft jest rozróżniana wielkość liter. Należy określić wszystkie identyfikatory spójnie zgodnie z liter.  
  
## <a name="see-also"></a>Zobacz też  
 [Identyfikatory zachowania](../c-language/behavior-of-identifiers.md)