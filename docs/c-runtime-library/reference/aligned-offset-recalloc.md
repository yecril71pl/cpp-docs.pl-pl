---
title: "_aligned_offset_recalloc — | Dokumentacja firmy Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
## <a name="remarks"></a>Uwagi  
 Podobnie jak [_aligned_offset_malloc —](../../c-runtime-library/reference/aligned-offset-malloc.md), `_aligned_offset_recalloc` umożliwia struktury wyrównania przy przesunięciu w strukturze.  
  
 `_aligned_offset_recalloc`jest oparta na `malloc`. Aby uzyskać więcej informacji o korzystaniu z `_aligned_offset_malloc`, zobacz [— funkcja malloc](../../c-runtime-library/reference/malloc.md). Jeśli `memblock` jest `NULL`, wywołania funkcji `_aligned_offset_malloc` wewnętrznie.  
  
 Ta funkcja ustawia `errno` do `ENOMEM` czy alokacja pamięci nie powiodła się, czy żądany rozmiar (`num` * `size`) była większa niż `_HEAP_MAXREQ`. Aby uzyskać więcej informacji na temat `errno`, zobacz [errno _doserrno —, _sys_errlist — i _sys_nerr —](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md). `_aligned_offset_recalloc` również sprawdza poprawność parametrów. Jeśli `alignment` nie jest potęgą liczby 2 lub, jeśli `offset` jest większa niż lub równa żądany rozmiar i różną od zera, ta funkcja wywołuje program obsługi nieprawidłowych parametrów, zgodnie z opisem w [sprawdzanie poprawności parametru](../../c-runtime-library/parameter-validation.md). Jeśli jest dozwolone wykonywanie, aby kontynuować, ta funkcja zwraca `NULL` i ustawia `errno` do `EINVAL`.  
