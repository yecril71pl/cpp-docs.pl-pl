...
## <a name="remarks"></a>Uwagi  
 ...
 `asctime` Funkcja konwertuje godzinę przechowywane jako struktura na ciąg znaków. `timeptr` Wartość zazwyczaj jest uzyskiwana w wyniku wywołania `gmtime` lub `localtime`, które zwraca wskaźnik do `tm` struktury zdefiniowane w czasie. H.  
