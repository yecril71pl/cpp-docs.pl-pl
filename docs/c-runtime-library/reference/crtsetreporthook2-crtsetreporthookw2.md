...
#### <a name="parameters"></a>Parametry  
 ...
 `pfnNewHook`  
 Raport punktu zaczepienia w celu zainstalowania lub usunięcia w wersji znak wąskie tej funkcji.  
  
 `pfnNewHook`  
 Raport punktu zaczepienia w celu zainstalowania lub usunięcia w wersji znaków dwubajtowych w tej funkcji.  
...
```  
 ...
 Te funkcje walidację ich parametrów. Jeśli `mode` lub `pfnNewNook` jest nieprawidłowy, te funkcje Wywołaj program obsługi nieprawidłowych parametrów, zgodnie z opisem w [sprawdzanie poprawności parametru](../../c-runtime-library/parameter-validation.md). Jeśli dozwolone jest wykonywanie aby kontynuować, ustawianie tych funkcji `errno` do `EINVAL` i zwróć -1.  
