...
> [!NOTE]
>  Nie należy używać tej metody do zamknięcia aplikacji systemu Windows platformy Uniwersalnej lub [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] aplikacji, z wyjątkiem testowania i debugowania scenariuszy. Programowe lub interfejsu użytkownika sposób, aby zamknąć [!INCLUDE[win8_appname_long](../../build/includes/win8_appname_long_md.md)] aplikacji nie są dozwolone. Aby uzyskać więcej informacji o aplikacji systemu Windows 8 i 8.1, zobacz [cykl życia aplikacji](http://go.microsoft.com/fwlink/?LinkId=262853). Aby uzyskać więcej informacji o aplikacji systemu Windows 10, zobacz [prowadzi instrukcje dla aplikacji systemu Windows 10](http://go.microsoft.com/fwlink/p/?linkid=619133).  
...
## <a name="remarks"></a>Uwagi  
 ...
 Mimo że `exit`, `_Exit` i `_exit` wywołania nie zwraca wartości, wartość w `status` dostępnego w środowisku hosta lub oczekującego procesu wywołującego, jeśli istnieje, po zamknięciu procesu. Zazwyczaj zestawy wywołującego `status` wartość na 0, aby wskazać normalne zakończenia lub wartość wystąpił błąd. Wartość `status` jest dostępna dla polecenia wsadowego systemu operacyjnego `ERRORLEVEL` i jest reprezentowana przez jedną z dwóch stałych: `EXIT_SUCCESS`, która reprezentuje wartość 0, lub `EXIT_FAILURE`, która reprezentuje wartość 1.  
