...
#### <a name="parameters"></a>Parametry  
 ...
 Jeśli **NULL** jest określony, ta funkcja przydziela buforu z co najmniej `maxlen` rozmiarze przy użyciu **— funkcja malloc**i wartość zwracaną `_getdcwd` wskaźnik do przydzielonego buforu. Bufor zwolniona, wywołując `free` i przekazanie jej przez wskaźnik.  
...
## <a name="return-value"></a>Wartość zwracana  
 ...
 Jeśli `buffer` jest określony jako `NULL` i ma za mało pamięci do przydzielenia `maxlen` znaków, wystąpi błąd i `errno` ma ustawioną wartość `ENOMEM`. Jeśli długość ścieżki, która zawiera znak końcowy null, przekracza `maxlen`, wystąpi błąd i `errno` ma ustawioną wartość `ERANGE`. Aby uzyskać więcej informacji na temat kodów błędów, zobacz [errno _doserrno —, _sys_errlist — i _sys_nerr —](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md).  
...
## <a name="remarks"></a>Uwagi  
 ...
 Gdy `_DEBUG` i `_CRTDBG_MAP_ALLOC` są zdefiniowane, wywołań `_getdcwd` i `_wgetdcwd` zastępuje wywołań `_getdcwd_dbg` i `_wgetdcwd_dbg` tak, aby umożliwić debugowanie alokacji pamięci. Aby uzyskać więcej informacji, zobacz[_getdcwd_dbg —, _wgetdcwd_dbg —](../../c-runtime-library/reference/getdcwd-dbg-wgetdcwd-dbg.md).  
...
## <a name="example"></a>Przykład  
 Zapoznaj się z przykładem w [_getdrive —](../../c-runtime-library/reference/getdrive.md).  
