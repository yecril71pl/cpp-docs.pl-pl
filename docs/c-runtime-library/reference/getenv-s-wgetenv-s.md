...
description: Opisuje getenv_s biblioteki środowiska uruchomieniowego języka Microsoft C i funkcje _wgetenv_s.
ms.date: 01/15/2020
api_name:
- getenv_s
- _wgetenv_s
...
|*pReturnValue*|*buffer*|*numberOfElements*|*nazwa_zmiennej*|Wartość zwrócona|
|--------------------|--------------|------------------------|---------------|------------------|
|**NULL**|ile|ile|ile|**EINVAL**|
|ile|**NULL**|>0|ile|**EINVAL**|
|ile|ile|ile|**NULL**|**EINVAL**|

Każdy z tych warunków błędu wywołuje procedurę obsługi nieprawidłowego parametru, zgodnie z opisem w [walidacji parametru](../../c-runtime-library/parameter-validation.md). Jeśli wykonanie może być kontynuowane, funkcje ustawione **errno** na **EINVAL** i zwracają **EINVAL** .
