...
Kopie `n` bajtów ciągu do ciągu docelowego. Te wersje programu [_mbsnbcpy —, _mbsnbcpy_l —](../../c-runtime-library/reference/mbsnbcpy-mbsnbcpy-l.md) zostały ulepszone zabezpieczenia, zgodnie z opisem w [funkcje zabezpieczeń w CRT](../../c-runtime-library/security-features-in-the-crt.md).  
...
## <a name="remarks"></a>Uwagi  
 `_mbsnbcpy_s` Funkcji kopie `count` bajtów z `strSource` do `strDest`. Jeśli `count` przekracza rozmiar `strDest`, albo ciągów wejściowych jest wskaźnika o wartości null, lub `sizeInBytes` lub `count` wynosi 0, funkcja wywołuje program obsługi nieprawidłowych parametrów, zgodnie z opisem w [sprawdzanie poprawności parametru](../../c-runtime-library/parameter-validation.md) . Jeśli jest dozwolone wykonywanie, aby kontynuować, funkcja zwraca `EINVAL`. Jeśli nakłada ciągów źródłowych i docelowych, zachowanie `_mbsnbcpy_s` jest niezdefiniowana.  
