...
`strftime` i `wcsftime` funkcji format **tm** wartości w czasie *timeptr* zgodnie z podane *format* argumentu i zapisać wynik w Bufor *strDest*. Co najwyżej *maxsize* znaki są umieszczane w ciągu. Opis pól w *timeptr* struktury, zobacz [asctime —](../../c-runtime-library/reference/asctime-wasctime.md). `wcsftime` jest to równoważne znaków dwubajtowych `strftime`; jej argument ciągu wskaźnik wskazuje ciąg znaków dwubajtowych. Funkcje te działają tak samo w przeciwnym razie wartość.

> [!NOTE]
>  W wersjach przed Visual C++ 2005, opisane dokumentacji *format* parametr `wcsftime` jako mający typ danych `const wchar_t *`, ale rzeczywistego wykonania *format* danych Typ `const char *`. Implementacja *format* — typ danych została zaktualizowana w celu uwzględnienia w poprzednim i bieżącym dokumentacji, oznacza to, `const wchar_t *`.
...
|`%R`|Wartość równoważna wartości `%H:%M`|
...
ISO 8601 tygodnia oraz oparte na tydzień roku wywołanych przez `%V`, `%g`, i `%G`, używa tygodnia, który zaczyna się w poniedziałek, 1 tydzień w przypadku tydzień zawierający stycznia 4, który jest w pierwszym tygodniu, która zawiera co najmniej czterech dni w roku. Jeśli pierwszy poniedziałek rok 2, 3 lub 4, poprzednich dni są częścią ostatniego tygodnia w poprzednim roku. Te dni `%V` zastępuje 53 i obu `%g` i `%G` zastępuje cyfr w poprzednim roku.
