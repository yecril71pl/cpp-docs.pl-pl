...
## <a name="remarks"></a>Uwagi  
 ...
 Brak wyjątek akapitu powyżej. Jeśli `count` jest `_TRUNCATE`, następnie tyle `strSource` jako mieści się w `strDest` jest kopiowana, pozostawiając nadal miejsca do zakończenia wartość null, która jest zawsze dołączany.  
 ...
 oznacza, że prosimy `strncpy_s` do skopiowania pięć znaków w buforze pięć bajtów; to mogłoby spowodować nie miejsca dla terminatorem null, dlatego `strncpy_s` wyzerowania ciągu i wywołuje program obsługi nieprawidłowych parametrów.  
  
 Jeśli wymagane jest zachowanie obcięcie, użyj `_TRUNCATE` lub (`size` - 1):  
  
 `strncpy_s(dst, 5, "a long string", _TRUNCATE);`  
  
 `strncpy_s(dst, 5, "a long string", 4);`  
  
 Należy pamiętać, że w przeciwieństwie do `strncpy`, jeśli `count` jest większa niż długość `strSource`, ciąg docelowy nie jest wypełniane znaki null do długości `count`.  
...
> [!NOTE]
>  _strncpy_s_l —, `_wcsncpy_s_l` i `_mbsncpy_s_l` mieć nie zależność od ustawień regionalnych i są dostępne tylko dla `_tcsncpy_s_l` i nie są przeznaczone do bezpośredniego wywoływania.  
