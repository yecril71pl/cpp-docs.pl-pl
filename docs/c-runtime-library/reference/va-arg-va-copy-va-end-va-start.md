...
Należy zauważyć, że **testit** oczekuje, że drugi parametr jest albo **`int`** lub **`char*`** . Przekazane argumenty to 0xFFFFFFFF (a **`unsigned int`** nie **`int`** a) i **null** (faktycznie a **`int`** **`char*`** ). Gdy program jest kompilowany do kodu natywnego, generuje następujące dane wyjściowe:
