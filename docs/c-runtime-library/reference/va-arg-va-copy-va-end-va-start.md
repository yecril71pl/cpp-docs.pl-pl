...
```  
 ...
 Zwróć uwagę, że `testit` oczekuje, że jej drugi parametr musi mieć przypisany `int` lub `char*`. Argumenty przekazywany są 0xffffffff ( `unsigned int`, a nie `int`) i `NULL` (faktycznie `int`, a nie `char*`). Gdy program jest kompilowany dla kodu natywnego, tworzy te dane wyjściowe:  
...
## <a name="see-also"></a>Zobacz też  
 [Dostęp do argumentów](../../c-runtime-library/argument-access.md)   
