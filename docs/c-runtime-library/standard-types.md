...
|----------|-------------------------------|
|Int8\_t, Uint8\_t|znak ze znakiem, znak bez znaku|
|Int16\_t, UInt16\_t|krótkie, niepodpisane, krótkie|
...
|`_complex`XML|Przechowuje rzeczywiste i urojone części liczb zespolonych; używany przez [_cabs](../c-runtime-library/reference/cabs.md).|MATH.H|
...
|`_diskfree_t`XML|Zawiera informacje o stacji dysków. Używany przez [_getdiskfree](../c-runtime-library/reference/getdiskfree.md) **.**|DOS.H i DIRECT.H|
|`div_t``ldiv_t` i struktury`lldiv_t`|Wartości są przechowywane odpowiednio przez [DIV](../c-runtime-library/reference/div.md), [ldiv](../c-runtime-library/reference/ldiv-lldiv.md)i [LLDiv](../c-runtime-library/reference/ldiv-lldiv.md).|STDLIB.H|
|`errno_t`całkowitą|Używane dla zwracanego typu funkcji lub parametru, który zajmuje się kodami `errno`błędów.|STDDEF.H,<br /><br /> CRTDEFS.H|
|`_exception`XML|Przechowuje informacje o błędach dla [_matherr](../c-runtime-library/reference/matherr.md).|MATH.H|
|`_EXCEPTION_POINTERS`|Zawiera rekord wyjątku. Aby uzyskać więcej informacji, zobacz [EXCEPTION_POINTERS](/windows/desktop/api/winnt/ns-winnt-exception_pointers) .|FPIEEE.H|
|`FILE`XML|Przechowuje informacje o bieżącym stanie strumienia; używany we wszystkich operacjach we/wy strumienia.|STDIO.H|
|`_finddata_t`, `_wfinddata_t` `_finddata32_t` ,`_wfinddatai64_t`,,,, ,`__finddata32i64_t`,, ,`__wfinddata64i32_t` , struktury `__wfinddata64_t` `__finddata64_t` `_wfinddata32_t` `_finddatai64_t` `__wfinddata32i64_t` `__finddata64i32_t`|Przechowuj informacje o atrybutach pliku zwrócone przez [_findfirst, _wfindfirst i powiązane funkcje](../c-runtime-library/reference/findfirst-functions.md) oraz [_findnext, _wfindnext i powiązane funkcje](../c-runtime-library/reference/findnext-functions.md). Aby uzyskać informacje na temat elementów członkowskich struktury, zobacz [funkcje wyszukiwania nazw plików](../c-runtime-library/filename-search-functions.md) .|IO.H, WCHAR.H|
...
|`_locale_t`XML|Zapisuje bieżące wartości ustawień regionalnych; używany we wszystkich bibliotekach wykonawczych C określonych ustawień regionalnych.|CRTDEF.H|
...
|`_RTC_error_fn`Definiuj typ|Definicja typu dla funkcji, która będzie obsługiwać kontrolę błędów czasu wykonywania. Używany w [_RTC_SetErrorFunc](../c-runtime-library/reference/rtc-seterrorfunc.md).|RTCAPI.H|
|`_RTC_error_fnW`Definiuj typ|Definicja typu dla funkcji, która będzie obsługiwać kontrolę błędów czasu wykonywania. Używany w [_RTC_SetErrorFuncW](../c-runtime-library/reference/rtc-seterrorfuncw.md).|RTCAPI.H|
...
|`size_t`(__int64 bez znaku lub liczba całkowita bez znaku, w zależności od platformy docelowej)|`sizeof` Wynik operatora.|CRTDEFS.H i inne pliki include|
...
|`__stat64`XML|Zawiera informacje o stanie pliku zwrócone przez [_fstat64](../c-runtime-library/reference/fstat-fstat32-fstat64-fstati64-fstat32i64-fstat64i32.md) i [_stat64](../c-runtime-library/reference/stat-functions.md)oraz [_wstat64](../c-runtime-library/reference/stat-functions.md).|SYS\STAT.H|
...
|`terminate_function`Definiuj typ|Typ definiuje się dla funkcji wywołania zwrotnego, która jest wywoływana, [](../c-runtime-library/reference/terminate-crt.md) gdy wywoływana jest przerwanie. Używany przez [set_terminate](../c-runtime-library/reference/set-terminate-crt.md).|EH.H|
|`time_t`(__int64 lub Long Integer)|Reprezentuje wartości czasu w [mktime](../c-runtime-library/reference/mktime-mktime32-mktime64.md), [Time](../c-runtime-library/reference/time-time32-time64.md), [CTime, _ctime32, _ctime64, _wctime, _wctime32, _wctime64](../c-runtime-library/reference/ctime-ctime32-ctime64-wctime-wctime32-wctime64.md), [ctime_s, _ctime32_s, _ctime64_s, _wctime_s, _wctime32_s, _wctime64_s](../c-runtime-library/reference/ctime-s-ctime32-s-ctime64-s-wctime-s-wctime32-s-wctime64-s.md), [CTime, _ctime32, _ctime64, _wctime, _ wctime32, _wctime64](../c-runtime-library/reference/ctime-ctime32-ctime64-wctime-wctime32-wctime64.md) i [gmtime, _gmtime32, _gmtime64](../c-runtime-library/reference/gmtime-gmtime32-gmtime64.md). Liczba sekund od 1 stycznia 1970 r., 0:00 czasu UTC. Jeśli _USE_32BIT_TIME_T jest zdefiniowany, `time_t` jest długą liczbą całkowitą. Jeśli nie jest zdefiniowana, jest to liczba całkowita 64-bitowa.|TIME.H,<br /><br /> SYS\STAT.H,<br /><br /> SYS\TIMEB.H|
...
|`uintptr_t`(Long Integer lub `__int64`, w zależności od platformy docelowej)|Nieoznaczona liczba całkowita bez znaku lub niepodpisana `intptr_t`wersja __int64.|STDDEF.H i inne pliki include|
|`unexpected_function`|Typ definiuje się dla funkcji wywołania zwrotnego, która jest wywoływana, [](../c-runtime-library/reference/unexpected-crt.md) gdy wywoływana jest nieoczekiwany. Używany przez [set_unexpected](../c-runtime-library/reference/set-unexpected-crt.md).|EH.H|
|`_utimbuf`XML|Umożliwia zapisanie czasu dostępu do pliku i modyfikacji używanych przez [_utime, _wutime](../c-runtime-library/reference/utime-utime32-utime64-wutime-wutime32-wutime64.md) i [_futime, _futime32, _futime64](../c-runtime-library/reference/futime-futime32-futime64.md) w celu zmiany daty modyfikacji pliku.|SYS\UTIME.H|
|`_utimbuf32`XML|Przechowuje czas dostępu do pliku i modyfikacji używany przez [_utime, _utime32, _utime64, _wutime, _wutime32, _wutime64](../c-runtime-library/reference/utime-utime32-utime64-wutime-wutime32-wutime64.md) i [_futime, _futime32, _futime64,](../c-runtime-library/reference/futime-futime32-futime64.md) aby zmienić daty modyfikacji pliku.|SYS\UTIME.H|
...
|`va_list`XML|Służy do przechowywania informacji wymaganych przez makra [va_arg](../c-runtime-library/reference/va-arg-va-copy-va-end-va-start.md) i [va_end](../c-runtime-library/reference/va-arg-va-copy-va-end-va-start.md) . Wywoływana funkcja deklaruje zmienną typu `va_list` , która może zostać przeniesiona jako argument do innej funkcji.|STDARG.H,<br /><br /> CRTDEFS.H|
