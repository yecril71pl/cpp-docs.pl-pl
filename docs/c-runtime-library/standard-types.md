...
ms.locfileid: "32418526"
---
# <a name="standard-types"></a>Standardowe typy
Biblioteka wykonawcza Microsoft definiuje następujące typy standardowe i definicje typów.  
  
### <a name="fixed-width-integral-types-stdinth"></a>Typy całkowite stałej szerokości (stdint.h)  
  
|Nazwa|Odpowiednik typu wbudowanego|  
|----------|-------------------------------|  
|int8\_t, uint8\_t|char podpisem, char bez znaku|  
|Int16\_t, uint16\_t|krótkie, krótko niepodpisane|  
|Int32\_t, uint32\_t|int, int bez znaku|  
|Int64\_t, uint64\_t|Long long unsigned long long|  
|int_least8_t, uint_least8_t|char podpisem, char bez znaku|  
|int_least16_t, uint_least16_t|krótkie, krótko niepodpisane|  
|int_least32_t, uint_least32_t|int, int bez znaku|  
|int_least64_t, uint_least64_t|Long long unsigned long long|  
|int_fast8_t, uint_fast8_t|char podpisem, char bez znaku|  
|int_fast16_t, uint_fast16_t|int, int bez znaku|  
|int_fast32_t, uint_fast32_t|int, int bez znaku|  
...
|`_complex` Struktura|Przechowuje części rzeczywistych i urojony liczby złożone; używane przez [_cabs —](../c-runtime-library/reference/cabs.md).|MATH.H|  
...
|`_CRT_DUMP_CLIENT`,<br /><br /> `_CRT_DUMP_CLIENT_M`|Definiowanie typu dla funkcji wywołania zwrotnego, która będzie uzyskać nazwę w [_crtmemdumpallobjectssince —](../c-runtime-library/reference/crtmemdumpallobjectssince.md).|CRTDBG.H|  
|`_CrtMemState` Struktura|Zawiera informacje dotyczące bieżącego stanu sterty C debugowania w czasie wykonywania.|CRTDBG.H|  
|`_CRT_REPORT_HOOK`,<br /><br /> `_CRT_REPORT_HOOKW`,<br /><br /> `_CRT_REPORT_HOOKW_M`|Definiowanie typu dla funkcji wywołania zwrotnego, która będzie uzyskać nazwę w [_crtdbgreport —](../c-runtime-library/reference/crtdbgreport-crtdbgreportw.md).<br /><br /> Parametry dla tej funkcji to: typ raportu, komunikat wyjściowy i wartość zwracana z funkcji wywołania zwrotnego.|CRTDBG.H|  
|`dev_t`, `_dev_t` całkowitą krótki i bez znaku|Reprezentuje uchwyty urządzenia.|SYS\TYPES.H|  
...
|`div_t`, `ldiv_t` i `lldiv_t` struktury|Przechowywanie wartości zwracanych przez [div](../c-runtime-library/reference/div.md), [ldiv —](../c-runtime-library/reference/ldiv-lldiv.md), i [lldiv —](../c-runtime-library/reference/ldiv-lldiv.md)odpowiednio.|STDLIB.H|  
|`errno_t` Liczba całkowita|Typ zwracany funkcji lub parametr, który zajmuje się kodów błędów z `errno`.|STDDEF.H,<br /><br /> CRTDEFS.H|  
...
|`_finddata_t`, `_wfinddata_t`, `_finddata32_t`, `_wfinddata32_t`, `_finddatai64_t`, `_wfinddatai64_t`, `__finddata64_t`, `__wfinddata64_t`, `__finddata32i64_t`, `__wfinddata32i64_t`, `__finddata64i32_t`, `__wfinddata64i32_t` struktury|Przechowuje informacje dotyczące atrybutu pliku zwracane przez [_findfirst —, _wfindfirst — i funkcje dotyczące](../c-runtime-library/reference/findfirst-functions.md) i [_findnext —, _wfindnext — i powiązane funkcje](../c-runtime-library/reference/findnext-functions.md). Zobacz [funkcje wyszukiwania Filename](../c-runtime-library/filename-search-functions.md) informacji w członkach struktury.|IO.H, WCHAR.H|  
|`_FPIEEE_RECORD` Struktura|Zawiera informacje dotyczące IEEE zmiennoprzecinkowych wyjątków; przekazywane do obsługi zdefiniowane przez użytkownika pułapki przez [_fpieee_flt —](../c-runtime-library/reference/fpieee-flt.md).|FPIEEE.H|  
|`fpos_t` (długich liczb całkowitych, `__int64`, lub struktury, w zależności od platformy docelowej)|Używane przez [fgetpos —](../c-runtime-library/reference/fgetpos.md) i [fsetpos —](../c-runtime-library/reference/fsetpos.md) do rejestrowania informacji w sposób unikatowy określająca każdej pozycji w pliku.|STDIO.H|  
|`_fsize_t` (bez znaku długich liczb całkowitych)|Używany do reprezentowania rozmiaru pliku.|IO.H,<br /><br /> WCHAR.H|  
|`_HEAPINFO` Struktura|Zawiera informacje o następnej wpisu stosu dla [_heapwalk —](../c-runtime-library/reference/heapwalk.md).|MALLOC.H|  
...
|`va_list` Struktura|Służy do przechowywania informacji wymaganych przez [va_arg](../c-runtime-library/reference/va-arg-va-copy-va-end-va-start.md) i [va_end](../c-runtime-library/reference/va-arg-va-copy-va-end-va-start.md) makra. Wywoływana funkcja deklaruje zmienną typu `va_list` które mogą być przekazywane jako argument do innej funkcji.|STDARG.H,<br /><br /> CRTDEFS.H|  
