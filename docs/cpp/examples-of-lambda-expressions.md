...
### <a name="remarks"></a>Uwagi  
 ...
 Mimo że wyrażenia lambda najczęściej są zadeklarowane w treści funkcji, można zadeklarować ich dowolnym zainicjować zmiennej.  
  
### <a name="example-2"></a>Przykład 2  
 Kompilator języka Visual C++ wiąże wyrażenie lambda do swoich zmiennych przechwyconych, gdy wyrażenie jest zadeklarowane, a nie wtedy, gdy wyrażenie jest wywoływane. W poniższym przykładzie przedstawiono wyrażenie lambda, która przechwytuje zmiennej lokalnej `i` według wartości i zmiennej lokalnej `j` przez odwołanie. Ponieważ przechwytuje wyrażenia lambda `i` wg wartości, ponowne przypisanie `i` nowsze w programie nie wpływa na wynik wyrażenia. Jednakże ponieważ wyrażenie lambda przechwytuje `j` przez odwołanie, ponowne przypisanie `j` ma wpływu na wynik wyrażenia.  
...
### <a name="example-1"></a>Przykład 1  
 W tym przykładzie deklaruje Wyrażenie lambda, które zwraca sumę dwóch liczb całkowitych i wywołuje wyrażenia bezpośrednio z argumentami `5` i `4`:  
...
### <a name="example-2"></a>Przykład 2  
 W tym przykładzie przekazuje wyrażenia lambda jako argumentu w celu `find_if` funkcji. Zwraca wyrażenie lambda `true` jeśli jej parametr jest liczbą parzystą.  
...
### <a name="remarks"></a>Uwagi  
 Aby uzyskać więcej informacji na temat `find_if` funkcji, zobacz [find_if](../standard-library/algorithm-functions.md#find_if). Uzyskać więcej informacji o funkcji standardowa biblioteka C++, wykonujących wspólnej algorytmów, zobacz [ \<algorytm >](../standard-library/algorithm.md).  
...
### <a name="remarks"></a>Uwagi  
 W tym przykładzie `[](int y) { return y * 2; }` jest wyrażeniem lambda zagnieżdżonych.  
...
**Visual Studio 2017 wersji 15.3 i nowszych** (dostępne z [/std:c ++ 17](../build/reference/std-specify-language-standard-version.md)): Przechwytywanie `this` przez wartość (`[*this]`) po lambda będzie używany w operacji asynchronicznej lub równoległego gdzie kod może należy wykonać po oryginalny obiekt wykracza poza zakres.
  
 Można użyć `this` wskaźnika jawnie w funkcji, jak pokazano poniżej:  
...
```  
 ...
 Istnieje również możliwość przechwytywania `this` wskaźnika niejawnie:  
...
### <a name="remarks"></a>Uwagi  
 `ApplyScale` Funkcja używa wyrażenia lambda do drukowania iloczyn wartości skali, a każdy element `vector` obiektu. Wyrażenia lambda niejawnie przechwytuje `this` tak, aby miało dostęp `_scale` elementu członkowskiego.  
...
### <a name="remarks"></a>Uwagi  
 Aby uzyskać więcej informacji na temat obsługi wyjątków, zobacz [obsługi wyjątków](../cpp/exception-handling-in-visual-cpp.md).  
...
> [!IMPORTANT]
>  Wyrażenia lambda nie są obsługiwane w tych wspólnych jednostek zarządzanych środowiska uruchomieniowego (języka wspólnego CLR) języka: `ref class`, `ref struct`, `value class`, i `value struct`.  
...
## <a name="see-also"></a>Zobacz też  
 ...
 [Function — klasa](../standard-library/function-class.md)   
