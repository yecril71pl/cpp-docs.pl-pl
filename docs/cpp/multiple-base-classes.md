...
W przypadku deklarowania wirtualnej klasy bazowej **`virtual`** słowo kluczowe pojawia się na podstawowych listach klas pochodnych.
...
![Symulowany obiad&#45;obiekt wiersza](../cpp/media/vc38xp2.gif "Symulowany obiad&#45;obiekt wiersza") <br/>
...
**`virtual`** Słowo kluczowe zapewnia tylko jedną kopię podobiektu `Queue` (patrz poniższy rysunek).

![Symulowany obiad&#45;obiekt line, wirtualne klasy bazowe](../cpp/media/vc38xp3.gif "Symulowany obiad&#45;obiekt line, wirtualne klasy bazowe") <br/>
...
![Wirtualny & nie&#45;wirtualnego dziedziczenia w układzie obiektu](../cpp/media/vc38xp5.gif "Wirtualny & nie&#45;wirtualnego dziedziczenia w układzie obiektu") <br/>
...
Jeśli w klasie pochodnej przeciążono funkcję wirtualną, która jest dziedziczona z wirtualnej klasy bazowej, oraz jeśli konstruktor lub destruktor klasy pochodnej wywołuje tę funkcję za pomocą wskaźnika do wirtualnej klasy bazowej, kompilator może wprowadzić dodatkowe ukryte pola „vtordisp” do klas z bazami wirtualnymi. `/vd0`Opcja kompilatora pomija dodanie ukrytego elementu członkowskiego przemieszczenia konstruktora vtordisp/destruktora. `/vd1`Opcja kompilatora, domyślnie, umożliwia ich, tam gdzie jest to konieczne. Wyłącz vtordisps tylko wtedy, gdy masz pewność, że wszystkie konstruktory i destruktory klas wywołują funkcje wirtualne w sposób wirtualny.

`/vd`Opcja kompilatora ma wpływ na cały moduł kompilacji. Użyj `vtordisp` dyrektywy pragma, aby pominąć i ponownie włączyć `vtordisp` pola w oparciu o klasę klasy:
...
- Efekt jawnej konwersji wskaźnika uzyskanego przy użyciu operatora address-of na typ klasy bazowej `A` . Należy zauważyć, że przekształcenie adresu obiektu na typ `A*` nie zawsze zapewnia kompilatora z wystarczającą ilością informacji, co oznacza, że podobiektem typu `A` do wybrania; w tym przypadku istnieją dwa podobiekty.
...
Konwersja do typu `A*` (wskaźnik do `A` ) jest niejednoznaczna, ponieważ nie istnieje sposób rozpoznać, który podobiekt typu `A` jest prawidłowy. Należy pamiętać, że można uniknąć niejednoznaczności, jawnie określając, który podobiekt ma być używany, w następujący sposób:
...
![Wirtualne pochodne i nie&#45;e wirtualne](../cpp/media/vc38xr1.gif "Wirtualne pochodne i nie&#45;e wirtualne") <br/>
Wirtualne a niewirtualne wyprowadzanie

Na rysunku dostęp do dowolnego elementu członkowskiego klasy `A` za pomocą niewirtualnych klas bazowych powoduje niejednoznaczność; kompilator nie ma informacji, które wyjaśniają, czy należy używać podobiektu skojarzonego z `B` lub podobiektem skojarzonym z `C` . Jednak jeśli `A` określono jako wirtualną klasę bazową, nie ma pytania, do którego jest uzyskiwany dostęp do obiektu.
