...
Jednak ponieważ jest to szablon funkcji, a kompilator może wywnioskować typ `T` z argumentów *a* i *b*, można wywołać go tak samo jak zwykła funkcja:
...
Słowo kluczowe **`class`** jest równoważne z **`typename`** w tym kontekście. Poprzedni przykład można wyrazić jako:
...
Podstawowe wymagania `std::vector` i inne kontenery biblioteki standardowej nakładają się na elementy `T` , które są `T` przydzielone do kopiowania i kopiowania konstrukcyjną.
...
Zwróć uwagę na składnię w deklaracji szablonu. `size_t`Wartość jest przenoszona jako argument szablonu w czasie kompilacji i musi być **`const`** lub **`constexpr`** wyrażeniem. Używasz tego przykładu:
