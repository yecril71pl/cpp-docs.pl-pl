...
-   Wyliczenie publiczne — klasa  
 ...
 UnorderedMap jest zasadniczo otoki dla [std::unordered_map](../standard-library/unordered-map-class.md) obsługującej magazynu typów środowiska wykonawczego systemu Windows. Jest konkretną implementację [Windows::Foundation::Collections::IMap](http://go.microsoft.com/fwlink/p/?LinkId=262408) i [IObservableMap](http://msdn.microsoft.com/library/windows/apps/br226050.aspx) interfejsy środowiska wykonawczego systemu Windows typy, które są przekazywane przez publicznego. Jeśli spróbujesz użyć typu platform::Collections:: unorderedmap — publiczny zwracać wartość lub parametr, błąd kompilatora, które jest wywoływane C3986. Naprawić ten błąd, zmieniając typ wartości parametru lub return na [Windows::Foundation::Collections::IMap](http://go.microsoft.com/fwlink/p/?LinkId=262408).  
...
|-|-|  
|Nazwa|Opis|  
|[Map::mapchanged —](#mapchanged)`event`|Występuje, gdy mapy.|  
...
### <a name="parameters"></a>Parametry  
 `key`  
 Klucz używana do lokalizowania elementu UnorderedMap. Typ `key` jest typename *K*.  
...
### <a name="return-value"></a>Wartość zwracana  
 `true`Jeśli klucz istniejącego elementu na bieżącej mapy odpowiada `key` i ma ustawioną wartość część elementu `value`. `false`Jeśli nie istniejący element w bieżącej mapy nie jest zgodny `key` i `key` i `value` parametry są wprowadzane na parę klucz wartość, a następnie dodać do bieżącej UnorderedMap.  
...
### <a name="parameters"></a>Parametry  
 `key`  
 Klucz używana do lokalizowania elementu w UnorderedMap. Typ `key` jest typename *K*.  
...
### <a name="parameters"></a>Parametry  
 ...
 Il  
 A [std::initializer_list](../standard-library/initializer-list-class.md) z [std::pair](../standard-library/pair-structure.md)obiektów, które będą używane do zainicjowania mapy.  
