---
title: 'Przewodnik: Tworzenie aplikacji platformy UWP przy użyciu WRL i platforma Media Foundation'
ms.date: 04/23/2019
ms.topic: reference
...
ms.locfileid: "80079888"
---
# <a name="walkthrough-creating-a-uwp-app-using-wrl-and-media-foundation"></a>Przewodnik: Tworzenie aplikacji platformy UWP przy użyciu WRL i platforma Media Foundation
...
- Atrybuty `namespace` i `runtimeclass` oraz wartość atrybutu `NTDDI_WIN8`[Version](/windows/win32/Midl/version) są ważnymi częściami definicji MIDL dla składnika Platforma Media Foundation, który korzysta z WRL.
...
- Makro [InspectableClass](inspectableclass-macro.md) implementuje podstawowe funkcje com, takie jak zliczanie odwołań i Metoda `QueryInterface`, i ustawia nazwę klasy środowiska uruchomieniowego i poziom zaufania.
...
1. W oknie dialogowym **strony właściwości** projektu ustaw następujące właściwości **konsolidatora** .
   ...
   1. W obszarze **dane wejściowe**Dodaj `runtimeobject.lib`, `mfuuid.lib`i `mfplat.lib` do właściwości **dodatkowe zależności** .
...
1. Dodaj nowy  **C# projekt pustej aplikacji (uniwersalnego systemu Windows)** do rozwiązania `MediaCapture`. Nazwij projekt, na przykład *MediaCapture*.

1. W projekcie **MediaCapture** Dodaj odwołanie do projektu `GrayscaleTransform`. Aby dowiedzieć się, jak to zrobić, zobacz [How to: Dodawanie lub usuwanie odwołań za pomocą Menedżera odwołań](/visualstudio/ide/how-to-add-or-remove-references-by-using-the-reference-manager).

1. W `Package.appxmanifest`na karcie **możliwości** wybierz pozycję **mikrofon** i **kamera internetowa**. Obie funkcje są wymagane do przechwytywania zdjęć z kamery internetowej.
