...
> [!NOTE]
>  Obecnie klasy `CRecordset` nie obsługuje aktualizacji do źródła danych, jeśli udało Ci się wdrożyć zbiorcze pobieranie z wiersza. Oznacza to, nie może wykonywać wywołania `AddNew`, `Edit`, `Delete`, lub `Update`. Jednakże można napisać możesz własne funkcje do wykonywania aktualizacji, a następnie wywołać te funkcje w ramach danej transakcji. Aby uzyskać więcej informacji na temat zbiorcze pobieranie z wiersza, zobacz [zestaw rekordów: Pobieranie rekordów (ODBC) zbiorcze](../../data/odbc/recordset-fetching-records-in-bulk-odbc.md).
