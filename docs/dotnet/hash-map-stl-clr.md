...
ms.locfileid: "80208744"
---
# <a name="hash_map-stlclr"></a>hash_map (STL/CLR)

Klasa szablonu opisuje obiekt, który kontroluje różnej długości sekwencje elementów, które mają dostęp dwukierunkowy. `hash_map` kontenera służy do zarządzania sekwencją elementów jako tabelą skrótów, każdy wpis w tabeli przechowujący dwukierunkową listę połączonych węzłów oraz każdy węzeł przechowujący jeden element. Element składa się z klucza, do porządkowania sekwencji i mapowanej wartości, która jest przypisana do przebiegu.
...
`GKey` jest taka sama jak `Key`, chyba że jest to typ referencyjny, w takim przypadku `Key^`

`GMapped` jest taka sama jak `Mapped`, chyba że jest to typ referencyjny, w takim przypadku `Mapped^`
...
Funkcja członkowska zwraca wartość true dla pustej kontrolowanej sekwencji. Jest to odpowiednik`() == 0`[hash_map:: size (STL/CLR)](../dotnet/hash-map-size-stl-clr.md) . Służy do sprawdzania, czy hash_map jest puste.
