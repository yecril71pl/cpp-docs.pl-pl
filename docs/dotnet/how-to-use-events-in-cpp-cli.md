---
title: 'Porady: Korzystanie ze zdarzeń w języku C + +/ CLI | Dokumentacja firmy Microsoft'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
## <a name="interface-events"></a>Zdarzenia interfejsu  
 Poniższy przykładowy kod dodaje program obsługi zdarzeń, wywołuje zdarzenie — co powoduje, że można zapisać jego nazwę w konsoli programu obsługi zdarzeń — a następnie usuwa programu obsługi zdarzeń.  
...
## <a name="custom-accessor-methods"></a>Metody dostępu niestandardowych  
 Następujący przykład przedstawia sposób definiowania zachowania zdarzenia podczas dodawania lub usuwania programów obsługi i zdarzenia.  
...
## <a name="override-default-access-on-add-remove-and-raise-accessors"></a>Zastępowanie domyślnego dostępu na dodawania, usuwania i wywoływania metod dostępu  
...
## <a name="multiple-event-handlers"></a>Procedury obsługi wielu zdarzeń  
...
## <a name="static-events"></a>Zdarzenia statyczne  
 Poniższy przykład przedstawia sposób definiowanie zdarzeń statycznych i korzystanie.  
...
```  
 ...
 Nie można określić zdarzenia prostego do przesłonięcia lub ukrywanie zdarzeń klasy podstawowej.  Należy określić wszystkie funkcje metod dostępu zdarzeń, a następnie określ `new` lub `override` — słowo kluczowe dla każdej funkcji dostępu.  
...
## <a name="abstract-events"></a>Zdarzeń abstrakcyjnych  
 Poniższy przykład przedstawia sposób Implementowanie zdarzeń abstrakcyjnych.  
...
## <a name="raising-events-that-are-defined-in-a-different-assembly"></a>Wywoływanie zdarzeń zdefiniowanych w innym zestawie  
 Zdarzenia i program obsługi zdarzeń można zdefiniowane w jednym zestawie i używane przez inny zestaw.  
...
```  
 ...
 Ten kod klienta wykorzystuje zdarzenia:  
