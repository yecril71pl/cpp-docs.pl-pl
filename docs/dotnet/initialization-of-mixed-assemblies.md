...
ms.locfileid: "87225647"
---
# <a name="initialization-of-mixed-assemblies"></a>Inicjalizacja zestawów mieszanych

Deweloperzy systemu Windows muszą zawsze być ostrożność blokady modułu ładującego podczas uruchamiania kodu `DllMain` . Istnieją jednak dodatkowe problemy, które należy wziąć pod uwagę podczas pracy z zestawami trybów mieszanych języka C++.
...
- Inicjalizacja niezarządzana i zarządzana jest wykonywana w dwóch oddzielnych i odrębnych etapach. Niezarządzana Inicjalizacja odbywa się najpierw (za pośrednictwem DllMain), a Inicjalizacja zarządzana odbywa się później za pośrednictwem. Konstrukcja obsługiwana przez sieć `.cctor` . Drugie jest całkowicie niewidoczne dla użytkownika, chyba że są używane **/zl** lub **/NODEFAULTLIB** . Zobacz[/NODEFAULTLIB (Ignoruj biblioteki)](../build/reference/nodefaultlib-ignore-libraries.md) i [/zl (Pomiń domyślną nazwę biblioteki)](../build/reference/zl-omit-default-library-name.md) , aby uzyskać więcej informacji.
...
Alternatywnie, jeśli nie `DllMain` jest to wymagane lub jeśli nie trzeba go wykonywać w ramach blokady modułu ładującego, `DllMain` można usunąć implementację dostarczoną przez użytkownika, co spowoduje eliminację problemu.
...
To ryzyko zakleszczenia zależy od tego, czy zawierający go moduł jest kompilowany z **/CLR** i czy zostanie wykonane. W odróżnieniu od tego, czy zmienna statyczna jest skompilowana bez **/CLR** (lub znajduje się w `unmanaged` bloku #pragma), a inicjator dynamiczny wymagany do jego zainicjowania powoduje wykonanie instrukcji MSIL, może wystąpić zakleszczenie. Jest to spowodowane tym, że w przypadku modułów skompilowanych bez **/CLR**Inicjalizacja zmiennych statycznych jest wykonywana przez DllMain. Natomiast zmienne statyczne skompilowane z **/CLR** są inicjowane przez `.cctor` program, po zakończeniu etapu inicjalizacji niezarządzanej i wydaniu blokady modułu ładującego.
...
- Wywołanie funkcji wbudowanej odbywa się za pomocą globalnego wskaźnika funkcji statycznej. Ten scenariusz jest istotny, ponieważ funkcje wirtualne są wywoływane za poorednictwem globalnych wskaźników funkcji. Przykład:
...
1. Ustaw tryb debugera na tryb tylko natywny.
   ...
   Otwórz siatkę **Właściwości** dla projektu startowego w rozwiązaniu. Wybierz pozycję Debugowanie **Właściwości konfiguracji**  >  **Debugging**. Ustaw **Typ debugera** na **tylko natywny**.
