...
Współdziałanie funkcje obsługiwane przez Visual C++ oferują danego zaletą za pośrednictwem innych języków .NET, jeśli chodzi o współdziałanie z COM — składniki. W przeciwieństwie do ograniczeń programu .NET Framework [Tlbimp.exe (Importer biblioteki typów)](/dotnet/framework/tools/tlbimp-exe-type-library-importer), takich jak ograniczoną obsługę typy danych i obowiązkowe narażenie każdy element członkowski każdego interfejsu COM, międzyoperacyjności języka C++ pozwala COM składniki do jest dostępny pod adresem będzie i nie wymaga oddzielne zestawy międzyoperacyjne. W przeciwieństwie do języka Visual Basic i C#, Visual C++, można użyć obiektów COM bezpośrednio za pomocą standardowego mechanizmu COM (takich jak **wywołanie funkcji CoCreateInstance** i **QueryInterface**). Jest to możliwe z powodu funkcje usługi międzyoperacyjnej modelu C++ powodujących kompilatora automatyczne wstawianie kodu przejścia do przenoszenia z zarządzanych w niezarządzanych funkcji i z powrotem.

Za pomocą międzyoperacyjności języka C++, składniki COM może służyć jako zwykle są używane, lub można ich opakować wewnątrz klasy C++. Te klasy otoki są nazywane wywoływane otoki niestandardowe środowiska uruchomieniowego lub CRCWs i mieć dwa zalet w porównaniu z bezpośrednio w kodzie aplikacji przy użyciu modelu COM:

- Wynikowa klasa może być używana z języków innych niż Visual C++.

- Szczegóły interfejsu COM może być ukryty z kodu zarządzanego klienta. Typy danych .NET można użyć zamiast natywnych typów, a szczegóły przekazywanie danych można wykonać wewnątrz CRCW przezroczysty.

Niezależnie od tego, czy używany jest COM, bezpośrednio lub za pośrednictwem CRCW musi być organizowany argumentu typu innego niż prosty, typy danych kopiowalnych.

## <a name="blittable-types"></a>Typy Kopiowalne

Dla niezarządzanego interfejsów API, które typy proste, wewnętrzne (zobacz [Kopiowalne i typy Kopiowalne inne niż](/dotnet/framework/interop/blittable-and-non-blittable-types)), nie kodowania są wymagane, ponieważ te typy danych mają taką samą reprezentację w pamięci, ale wymagają bardziej złożone typy danych przekazywanie danych jawnego. Na przykład zobacz [porady: wywołań natywnych bibliotek DLL z zarządzanego kodu za pomocą funkcji PInvoke](../dotnet/how-to-call-native-dlls-from-managed-code-using-pinvoke.md).
