...
ms.locfileid: "33284121"
---
# <a name="compiler-errors-c3300-through-c3399"></a>C3300 błędy kompilatora za pośrednictwem C3399
...
|Błąd|Komunikat|
|-----------|-------------|
|C3300 błąd kompilatora|"*symbol*": niewłaściwy format języka IDL "*wartość*"|
|C3301 błąd kompilatora|"*coclass*": klasa coclass nie może być "*symbol*" — interfejs|
|C3302 błąd kompilatora|"*identyfikator*": identyfikator ma więcej niż *numer* znaków|
|[Błąd kompilatora C3303](compiler-error-c3303.md)|"*atrybutu*": atrybut może być używany tylko na "*typu*"|
|C3304 błąd kompilatora|Nieaktualne.|
|C3305 błąd kompilatora|Nieaktualne.|
|C3306 błąd kompilatora|"*szablonu*": nienazwanej klasy szablonu/rodzajowa jest niedozwolone.|
...
|C3311 błąd kompilatora|Atrybut modułu musi być zdefiniowana w zakresie globalnym|
|C3312 błąd kompilatora|Nie można wywołać "*identyfikator*"Funkcja znaleziono dla typu"*typu*"|
|C3313 błąd kompilatora|"*identyfikator*": zmienna nie może mieć typu "*typu*"|
|C3314 błąd kompilatora|"*symbol*": nie obsługiwanym typem modułu IDL|
|C3315 błąd kompilatora|" *funkcja*": musi być funkcją członkowską|
|C3316 błąd kompilatora|"*typu*": tablica o nieznanym rozmiarze nie można używać w opartej na zakresie dla instrukcji|
|C3317 błąd kompilatora|"*identyfikator*": przeciążona funkcja nie można użyć jako wyrażenia w opartej na zakresie dla instrukcji|
|C3318 błąd kompilatora|"*typu*": tablica nie może mieć typu elementu, który zawiera "auto"|
|C3319 błąd kompilatora|Nieaktualne.|
|[Błąd kompilatora C3320](compiler-error-c3320.md)|"*typu*": typ nie może mieć takiej samej nazwy jak właściwość modułu "name"|
|C3321 błąd kompilatora|Lista inicjalizatora jest nieoczekiwana w tym kontekście|
|[Błąd kompilatora C3322](compiler-error-c3322.md)|"*właściwości*": nie jest prawidłową właściwością dla atrybutu "*atrybutu*"|
|C3323 błąd kompilatora|"alignas" i "__declspec(align)" nie są dozwolone w deklaracjach funkcji|
|C3324 błąd kompilatora|"*właściwości*": właściwość występuje więcej niż raz w atrybucie "*atrybutu*"|
|C3325 błąd kompilatora|"*atrybutu*": atrybut ma za dużo argumentów|
|C3326 błąd kompilatora|"*wartość*": nie jest prawidłową wartością dla właściwości "*właściwości*"atrybutu"*atrybutu*"|
|C3327 błąd kompilatora|"*właściwości*": należy określić wartość dla właściwości atrybutu '*atrybutu*"|
|C3328 błąd kompilatora|"*atrybutu*": atrybut nie ma wystarczającej liczby argumentów|
