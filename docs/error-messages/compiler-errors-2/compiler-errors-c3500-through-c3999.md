...
|Błąd kompilatora C3502|Nie można uzyskać identyfikatora LIBID dla ProgID "*progid*"|
|Błąd kompilatora C3503|znak "0 x*wartość*" nie jest dozwolona w nieprzetworzonego ciągu literału|
|Błąd kompilatora C3504|Nie można utworzyć identyfikatora GUID z ciągu "*ciąg*"|
...
|[Compiler error C3509](compiler-error-c3509.md)|"*typu*": nieprawidłowy zwracany typ automatyzacji; gdy parametr jest oznaczony jako "retval", zwracanym typem musi być "void", "HRESULT" lub "SCODE"|
|[Błąd kompilatora C3510](compiler-error-c3510.md)|Nie można zlokalizować biblioteki typów zależnych *biblioteki*|
|Błąd kompilatora C3511|"*identyfikator*": wywołanie konstruktora delegującego powinny być jedyną inicjacją składowej|
...
|Błąd kompilatora C3513|"*ciąg*": nieobsługiwany znak ograniczający literał nieprzetworzonego ciągu|
|Błąd kompilatora C3514|"*znak*" (*wartość*): nieobsługiwany znak ograniczający literał nieprzetworzonego ciągu|
...
|Błąd kompilatora C3516|Nieoczekiwany koniec pliku znalezione podczas przetwarzania literału; nieprzetworzonego ciągu Sekwencja ogranicznika "*ciąg*" nie został dopasowany.|
|Błąd kompilatora C3517|"*identyfikator*" Deklaracja aliasu nie może mieć typu zawierającego "auto"|
|Błąd kompilatora C3518|"*identyfikator*": w kontekście bezpośredniego Inicjalizacja listy Typ dla "*typu*" można określić tylko na podstawie pojedynczego wyrażenia inicjatora|
...
|Błąd kompilatora C3522|"*typu*": pakiet parametrów nie może być rozwinięty w tym kontekście|
|Błąd kompilatora C3523|"sizeof..." wymaga jako argument nierozwiniętego pakietu parametrów|
|Błąd kompilatora C3524|"*identyfikator*": "sizeof" nie można zastosować do pakietu parametrów. Czy zamierzałeś użyć "sizeof..."?|
|Błąd kompilatora C3525|"*parametr*": Jeśli szablon klasy ma pakiet parametrów szablonu, musi znajdować się na końcu listy parametrów szablonu|
|Błąd kompilatora C3526|"..." nie można zastosować do "this"|
|Błąd kompilatora C3527|"*identyfikator*" nie jest prawidłowym argumentem operacji "sizeof...". Czy zamierzałeś użyć "sizeof"?|
|Błąd kompilatora C3528|"*identifier1*": liczba elementów w tym rozwinięciu pakietu jest niezgodna z liczbą elementów w "*identifier2*"|
...
|Błąd kompilatora C3542|"*identyfikator*": funkcja wirtualna elementu członkowskiego nie posiada zwracany typ, który zawiera "*typu*"|
|Błąd kompilatora C3543|"*typu*": nie zawiera pakietu parametrów|
|Błąd kompilatora C3544|"*parametr*": pakiet parametrów oczekuje argumentu szablonu typu|
|Błąd kompilatora C3545|"*parametr*": pakiet parametrów oczekuje argumentu szablonu bez typu|
|Błąd kompilatora C3546|"...": Brak pakietów parametrów dostępnych do rozwinięcia|
|Błąd kompilatora C3547|Parametr szablonu "*parametru*"nie można użyć, ponieważ następuje po pakiecie parametrów szablonu i nie można wywnioskować z parametrów funkcji"*funkcja*"|
|Błąd kompilatora C3548|"*identyfikator*": pakiet parametrów nie można używać w tym kontekście|
|Błąd kompilatora C3549|"*wartość*": pakiet parametrów funkcji nie może mieć argumentu domyślnego|
...
|Błąd kompilatora C3559|Wywołanie rekurencyjne dla "*funkcja*": rekursja została wykryta podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each: "*funkcja*"|
|Błąd kompilatora C3560|"*funkcja*": IL jest niedostępne podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each: "*funkcja*"|
|Błąd kompilatora C3561|znaleziono w przepływie sterowania, który nie jest różniącym podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each operację bariery sąsiedniego: "*funkcja*"|
|Błąd kompilatora C3562|funkcja wewnętrzna "*funkcja*" jest ograniczona do nie mają więcej niż *numer* parametrów|
|Błąd kompilatora C3563|Wykryto nieskończoną pętlę podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each: "*funkcja*"|
|Błąd kompilatora C3564|czytanie niezainicjalizowanej wartości podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each: "*funkcja*"|
|Błąd kompilatora C3565|Całkowita ilość pamięci sąsiadująco rozmieszczonej statycznie (*numer* bajtów) przekracza limit wynoszący *numer* bajtów podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each|
|Błąd kompilatora C3566|bloki z efektami ubocznymi zagnieżdżone zbyt głęboko podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each: "*funkcja*"|
|Błąd kompilatora C3567|Dzielenie lub dzielenie modulo przez zero wykryta podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each: "*funkcja*"|
|Błąd kompilatora C3568|Suma rejestrów przekracza limit wynoszący *numer* podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each. Uprość program|
|Błąd kompilatora C3569|Wykryto podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each sytuację wyścigu: "*funkcja*"|
|Błąd kompilatora C3570|niedozwolone użycie amp ograniczony zakres podczas kompilowania z/CLR|
|Błąd kompilatora C3571|"*typu*": niedozwolony argument domeny obliczeń; nie jest typem klasy|
|Błąd kompilatora C3572|"*typu*": niedozwolony argument domeny obliczeń; brak publicznej składowej: "static const int rank" lub wartość rankingowa jest Niedodatnia|
|Błąd kompilatora C3573|Liczba wystąpień concurrency::graphics::sampler przekazanych do concurrency::parallel_for_each nie może przekroczyć *numer*|
|Błąd kompilatora C3574|"*typu*": niedozwolone zakresy sąsiadujące: musi być dodatnia i (1) Z < = *numer*; (2) Z*Y*X < = *numer*|
|Błąd kompilatora C3575|"*typu*": niedozwolony argument domeny obliczeń; brak publicznej składowej: "concurrency::index <*numer*> _map_index (const concurrency::index <*numer*> &) restrict(amp) "|
|Błąd kompilatora C3576|"*typu*": concurrency::details::_Parallel_for_each argument #*numer* ma nieobsługiwany typ|
|Błąd kompilatora C3577|argument jądra CONCURRENCY::parallel_for_each jest niedozwolony: nie można rozwiązać wywołania do składowej "void operator() (*typu*) restrict(amp)"|
|Błąd kompilatora C3578|Rozmiar obiektu funkcji, przekazanych do concurrency::parallel_for_each nie może przekraczać *numer* bajtów|
|Błąd kompilatora C3579|Liczba wystąpień tylko do odczytu concurrency::array oraz CONCURRENCY::Graphics:: Texture przekazanych do concurrency::parallel_for_each nie może przekroczyć *numer*|
|Błąd kompilatora C3580|Liczba wystąpień do zapisu concurrency::array oraz CONCURRENCY::Graphics:: Texture przekazanych do concurrency::parallel_for_each nie może przekroczyć *numer*|
|Błąd kompilatora C3581|"*typu*": nieobsługiwany typ w amp z ograniczeniami kodu|
|Błąd kompilatora C3582|Nieaktualne.|
|Błąd kompilatora C3583|"*identyfikator*": rozmiar zmiennej (*numer* bajtów) jest większy niż maksymalny rozmiar (*numer* bajtów) są obsługiwane w kodzie amp z ograniczeniami|
|Błąd kompilatora C3584|nieobsługiwane użycie tile_static na "*identyfikator*"|
|Błąd kompilatora C3585|"*identyfikator*" klasy magazynu w amp ograniczył kodu|
|Błąd kompilatora C3586|"*identyfikator*": przy użyciu globalnych lub statycznych zmiennych jest nieobsługiwane w ograniczonym kodzie amp|
|Błąd kompilatora C3587|dynamic_cast jest nieobsługiwane w ograniczonym kodzie amp|
|Błąd kompilatora C3588|Rzutowanie z "*type1*"to"*type2*" jest nieobsługiwana w ograniczonym kodzie amp|
|Błąd kompilatora C3589|"*ciąg*": nieobsługiwane użycie literałów ciągów w amp z ograniczeniami kodu|
|Błąd kompilatora C3590|"*tokenu*": Przechwytywanie przez odwołanie lub przechwytywania "this" jest nieobsługiwane, jeżeli wyrażenie lambda jest ograniczone przez amp|
|Błąd kompilatora C3591|TypeID operator jest nieobsługiwany w ograniczonym kodzie amp|
|Błąd kompilatora C3592|Wbudowany natywny zestaw (__asm) jest nieobsługiwana w ograniczonym kodzie amp|
|Błąd kompilatora C3593|Instrukcja "goto" jest nieobsługiwana w ograniczonym kodzie amp|
|Błąd kompilatora C3594|Obsługa (try, catch, throw itp.) wyjątków jest nieobsługiwana w ograniczonym kodzie amp|
|Błąd kompilatora C3595|wartość stała jest poza obsługiwanym zakresem w ograniczonym kodzie amp|
|Błąd kompilatora C3596|"*parametru*" ("*typu*"): zmienna przechwycona przez lambda ma nieobsługiwany typ w ograniczonym kodzie amp|
|Błąd kompilatora C3597|"*parametru*": "*identyfikator*" nie może być przechwytywane przez wartość, jeśli wyrażenie lambda jest ograniczone przez amp|
...
|Błąd kompilatora C3600|"*funkcji*": Użyj kafelka\_pamięci statycznej wykryta podczas kompilacji wykresu wywołań dla concurrency::parallel niekafelkowanego\_for_each na: "*funkcja*"|
|Błąd kompilatora C3601|"*typu*": jest nieprawidłowym typem argumentu dla diagnostycznej funkcji amp "*funkcja*"|
|Błąd kompilatora C3602|Wykryto podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each nieobsługiwany przepływ sterowania: "*funkcja*"|
|[Błąd kompilatora C3603](compiler-error-c3603.md)|"*symbol*": typ "*typu*" nie jest jeszcze obsługiwany|
|Błąd kompilatora C3604|"*identyfikator*": tylko można utworzyć obiekt zarządzany na stercie gc|
|Błąd kompilatora C3605|Całkowita liczba próbników (*numer* przechwytywane i *numer* wstępnie zdefiniowanych) przekracza *numer* podczas kompilacji wykresu wywołań dla concurrency::parallel_for_each: " *Funkcja*"|
|Błąd kompilatora C3606|Nieaktualne.|
|Błąd kompilatora C3607|Nieaktualne.|
|Błąd kompilatora C3608|Nieaktualne.|
|[Compiler error C3609](compiler-error-c3609.md)|"*identyfikator*": funkcja "zapieczętowany/final" musi być funkcją wirtualną|
...
|Błąd kompilatora C3613|Brak typu zwracanego po "->" ("zakładany jest int")|
|Błąd kompilatora C3614|Różne wartości rozmiaru pakietu w tej samej klasie; poprzedniej została "*wartość*", nowa wartość to"*wartość*"|
...
|Błąd kompilatora C3620|"*typu*": ustawienie wyrównania jest niedozwolone dla typów WinRT|
|Błąd kompilatora C3621|"*typu*": tylko domyślna wartość pakowania (*numer*) jest dozwolona dla typów WinRT|
...
|Błąd kompilatora C3629|"*tokenu*": wartość domyślna przechwytywania może wystąpić tylko na początku listy przechwytywania lambda|
...
|Błąd kompilatora C3635|"*identyfikator*": nie można zastosować do klasy zarządzane WinRT (Użyj "*identyfikator*" zamiast)|
...
|Błąd kompilatora C3649|Ta składnia jawnego przesłaniania nie jest dozwolona z /ZW|
...
|Błąd kompilatora C3659|"*elementu członkowskiego*": specyfikator override "*— słowo kluczowe*" nie jest obsługiwana|
|Błąd kompilatora C3660|"*Członek1*": ukrywa dziedziczoną składową "*członek2*"|
...
|Błąd kompilatora C3664|"*elementu członkowskiego*": nie można użyć jako jawnego przesłaniania, musi mieć dostępność "public" lub "protected"|
...
|Błąd kompilatora C3667|"*atrybut*": atrybut nie obsługuje rozwinięcia pakietu|
...
|[Compiler error C3669](compiler-error-c3669.md)|"*elementu członkowskiego*": specyfikator override nie "override" dozwolony dla funkcji statycznych składowych lub konstruktory|
...
|Błąd kompilatora C3674|Nie można odnaleźć modułu biblioteki standardowej "*modułu*"|
|[Błąd kompilatora C3675](compiler-error-c3675.md)|"*funkcja*": jest zarezerwowana, ponieważ "*właściwość*" jest zdefiniowana|
|Błąd kompilatora C3676|"*klasy*": klasa ref i klasa bazowa mają niezgodne atrybuty "[*atrybut*]"|
|Błąd kompilatora C3677|literał ciągu po 'operator' nie może mieć prefiksu kodowania|
|Błąd kompilatora C3678|literał ciągu po 'operator' musi być ciągiem pustym "" "" "|
|Błąd kompilatora C3679|Oczekiwano identyfikatora sufiksu literału po "operator" "" "|
|Błąd kompilatora C3680|Nie można połączyć Literały ciągu zdefiniowany przez użytkownika z niezgodnymi identyfikatorami sufiksów literałów|
|Błąd kompilatora C3681|"fallthrough": atrybut może występować tylko w otaczającej instrukcji switch|
|Błąd kompilatora C3682|"operator *identyfikator*": szablon operatora literału operator/literału nie może być zadeklarowana w zapewnienie "Powiązanie C"|
|Błąd kompilatora C3683|Nie można zdefiniować nieprzetworzonego operatora literału i szablonu operatora literału z tym samym identyfikatorem sufiksu literału|
|Błąd kompilatora C3684|"operator *identyfikator*": deklaracja operatora literału ma nieprawidłową listę parametrów|
|Błąd kompilatora C3685|"operator *identyfikator*": szablon operatora literału nie może mieć parametrów funkcji|
|Błąd kompilatora C3686|"operator *identyfikator*": szablon operatora literału musi mieć dokładnie jeden parametr szablonu będący pakietem parametrów|
|Błąd kompilatora C3687|"operator *identyfikator*": szablon operatora literału musi mieć parametr szablonu bez typu o typie "char"|
|Błąd kompilatora C3688|Nieprawidłowy sufiks literału "*sufiks*"; literał operatora lub szablon operatora literału "operator *identyfikator*" nie znaleziono|
|Błąd kompilatora C3689|"operator *identyfikator*": szablon operatora literału operator/literału musi być w zakresie globalnym lub przestrzeni nazw|
|Błąd kompilatora C3690|Oczekiwano literału ciągu, ale znaleziono ciągiem zdefiniowanym przez użytkownika literałów|
...
|Błąd kompilatora C3696|"*— słowo kluczowe*": nie można użyć tego kwalifikatora na "%"|
...
|[Compiler error C3699](compiler-error-c3699.md)|"*operator*": nie można użyć tego operatora pośredniego na typie "*typu*"|
...
|Błąd kompilatora C3720|"*typu*": można tylko zaimplementować interfejs IDispatch podwójnie lub dispinterface|
...
|Błąd kompilatora C3742|"*token1*": nieprawidłowo ustawiona sekwencja tokenów w argumencie atrybutu "*atrybut*", oczekiwano"*token2*"|
...
|Błąd kompilatora C3746|Atrybut standardowy "*identyfikator*" może występować co najwyżej raz na liście atrybutów|
...
|Błąd kompilatora C3750|"*tokenu*": nieoczekiwany token na liście atrybutów|
...
|Błąd kompilatora C3757|"*typu*": typ nie jest dozwolony dla funkcji "constexpr"|
|Błąd kompilatora C3758|"*elementu członkowskiego*": funkcja wirtualna nie można zadeklarować jako "constexpr"|
|Błąd kompilatora C3759|"*elementu członkowskiego*": funkcja składowa typu innego niż literał nie można zadeklarować jako "constexpr"|
|Błąd kompilatora C3760|Użyj słowa kluczowego __property Aby zadeklarować właściwość w zarządzanych/WinRT "*klasy*"|
...
|Błąd kompilatora C3770|"*typu*": nie jest prawidłową klasę podstawową|
...
|Błąd kompilatora C3773|należy użyć przełącznika kompilatora / await w celu włączenia procedur wspólnych|
|Błąd kompilatora C3774|Nie można odnaleźć '*zakres*::*identyfikator*": Zawsze podawaj *nagłówka* nagłówka|
|Błąd kompilatora C3775|zwracany typ "*funkcja*"nie powinien być"*typu*"|
|Błąd kompilatora C3776|Nie można zwrócić, wyrażenie typu void w koprocedury za pomocą ostatecznej zwracany typ inny niż void|
|Błąd kompilatora C3777|"*funkcja*": procedura wspólna nie może mieć zmiennej listy argumentów|
|Błąd kompilatora C3778|alloca: nie można używać w procedurze wspólnej|
|Błąd kompilatora C3779|"*funkcja*": funkcja, która zwraca "*typu*" nie można użyć, zanim zostanie on zdefiniowany|
|Błąd kompilatora C3780|"*funkcja*": funkcja konwersji, która zwraca*typu*"nie można użyć, zanim zostanie on zdefiniowany|
|Błąd kompilatora C3781|"*— słowo kluczowe*": nie może być używane jako procedura wspólna typu "*typu*". Albo *— słowo kluczowe* lub *— słowo kluczowe* w skojarzonych promise_type musi znajdować się|
|Błąd kompilatora C3782|*Typ*: procedura wspólna nie może zawierać zarówno *— słowo kluczowe* i *— słowo kluczowe*|
|Błąd kompilatora C3783|"*identyfikator*": nie może być koprocedurą|
|Błąd kompilatora C3784|*słowo kluczowe* wyrażenia nie może występować w tym kontekście|
|Błąd kompilatora C3785|pierwszy argument szablonu dla elementu "STD::integer_sequence" musi być typu całkowitego|
|Błąd kompilatora C3786|drugi argument szablonu "std::make_integer_sequence" musi być stała liczba całkowita większa lub równa zero|
...
|Błąd kompilatora C3801|"*atrybut*": atrybut nie może mieć klauzuli argumentu|
...
|[Compiler error C3805](compiler-error-c3805.md)|"*tokenu*": nieoczekiwany token, oczekiwano albo "}"lub ","|
|Błąd kompilatora C3806|"*tokenu*": nieoczekiwany token, oczekiwano albo "{" lub inicjator składowej|
...
|[Compiler error C3809](compiler-error-c3809.md)|"*typu*": typ zarządzany WinRT nie może mieć żadnych friend funkcji/klas/interfejsów|
...
|Błąd kompilatora C3827|Atrybut standardowy "deprecated" może mieć żadnych argumentów lub jeden literał ciągu opisujący przyczynę|
...
|Błąd kompilatora C3829|Atrybut standardowy "noreturn" można stosować tylko do funkcji|
...
|Błąd kompilatora C3837|w tym kontekście atrybuty są niedozwolone.|
...
|Błąd kompilatora C3844|"*identyfikator*": nie można zaimportować symbolu z "*źródła*": jako*identyfikator*"już istnieje w bieżącym zakresie|
...
|[Compiler error C3849](compiler-error-c3849.md)|Wywołanie w stylu funkcji na wyrażeniu typu "*typu*" spowoduje utratę kwalifikatorów const i/lub volatile dla wszystkich *numer* dostępnych przeciążeń operatora|
|[Compiler error C3850](compiler-error-c3850.md)|"*tokenu*": universal-character-name Określa nieprawidłowy znak|
...
|[Compiler error C3856](compiler-error-c3856.md)|"*klasy*": klasa nie jest ogólny/szablonu klasy|
...
|[Compiler error C3859](compiler-error-c3859.md)|zakres pamięci wirtualnej dla PCH przekroczona; należy ponownie skompilować przy użyciu opcji wiersza polecenia "-Zm*numer*" lub nowszej|
...
|Błąd kompilatora C3863|typu tablicy "*typu*" nie jest możliwy do przypisania|
...
|[Compiler error C3866](compiler-error-c3866.md)|wywołanie destruktora/finalizator brakuje listy argumentów|
...
|[Compiler error C3869](compiler-error-c3869.md)|w ograniczeniu gcnew brakuje listy parametrów empty "()"|
|Błąd kompilatora C3870|"*parametru*": "__declspec (*specyfikator*)" może być stosowany tylko do parametrów typu całkowitego|
|Błąd kompilatora C3871|"*parametr*": "__declspec(guard(overflow))" jest obsługiwany tylko w pierwszych 64 parametrach funkcji|
...
|Błąd kompilatora C3875|Wywołanie funkcji niestatycznego elementu członkowskiego, Brak listy argumentów|
|Błąd kompilatora C3876|Nieaktualne.|
|Błąd kompilatora C3877|Nieaktualne.|
|Błąd kompilatora C3878|Nieaktualne.|
|Błąd kompilatora C3879|"*elementu członkowskiego*": nie może być składową danych initonly|
|[Błąd kompilatora C3880](compiler-error-c3880.md)|"*elementu członkowskiego*": nie może być literał składowej danych|
|Błąd kompilatora C3881|może dziedziczyć Konstruktor tylko z bezpośredniej klasy podstawowej|
|Błąd kompilatora C3882|"*klasy*": konstruktor został już odziedziczony z "*klasy*"|
|Błąd kompilatora C3883|"*elementu członkowskiego*": statyczna składowa danych initonly musi zostać zainicjowany|
|Błąd kompilatora C3884|"*typu*": Tablica o nieznanym rozmiarze nie może być inicjowany przez wartość|
|Błąd kompilatora C3885|"*typu*": Nie można zainicjować tablicy o nieznanym rozmiarze przy użyciu pustej listy inicjatora|
...
|[Błąd kompilatora C3888](compiler-error-c3888.md)|"*elementu członkowskiego*": wyrażenie const powiązane z tym literałem składowej danych nie jest obsługiwana przez C++sposób niezamierzony|
...
|[Compiler error C3896](compiler-error-c3896.md)|"*elementu członkowskiego*": niewłaściwy inicjator: ten literał składowej danych może być inicjowane tylko z "nullptr"|
...
|Błąd kompilatora C3905|niewyrównane dostępy nie są obsługiwane dla wewnętrznego typu "*typu*"|
|Błąd kompilatora C3906|wewnętrznego typu "*typu*" nie jest obsługiwane zwracanym lub typem argumentu dla funkcji vararg lub funkcji nieprototypowanych|
...
|[Compiler error C3909](compiler-error-c3909.md)|Deklaracja zdarzenia zarządzane WinRT musi wystąpić w typie managed WinRT|
...
|Błąd kompilatora C3925|Oczekiwano pętli (for, while lub) następujących "*dyrektywy*" — dyrektywa|
|Błąd kompilatora C3926|Nieprawidłowa stała w dyrektywie "parallel"|
|Błąd kompilatora C3927|"->": końcowym typem zwracanym nie jest dozwolona po deklaratorze niebędącym funkcją|
|Błąd kompilatora C3928|"->": końcowym typem zwracanym nie jest dozwolona po deklaratorze w nawiasach okrągłych|
|Błąd kompilatora C3929|Nieaktualne.|
|Błąd kompilatora C3930|"*funkcja*": Brak funkcji przeciążonej posiadającej specyfikatory ograniczeń, które są zgodne z otaczającym kontekstem "*kontekstu*"|
|Błąd kompilatora C3931|"*typu*": nie można wywołać funkcję, która ma specyfikatory ograniczeń, które są niezgodne z otaczającym kontekstem|
|Błąd kompilatora C3932|Nieaktualne.|
|Błąd kompilatora C3933|"*klasy*": specyfikatory ograniczenia destruktora muszą obejmować związek ograniczeń na wszystkie konstruktory|
|Błąd kompilatora C3934|Funkcja, która jest w jakimkolwiek formularzu "main" nie może mieć specyfikatorów ograniczenia innych niż restrict(cpu)|
|Błąd kompilatora C3935|"*identyfikator*": zmiana definicji; specyfikatory ograniczenia nakładają się|
|Błąd kompilatora C3936|"*identyfikator*": nierozpoznany specyfikator ograniczenia|
|Błąd kompilatora C3937|pusty specyfikator ograniczenia nie jest dozwolone.|
|Błąd kompilatora C3938|"*identyfikator*": wielokrotne specyfikatory ograniczenia są nieobsługiwane w funkcji \042C\042 extern|
...
|Błąd kompilatora C3940|"*identyfikator*": nie odnaleziono identyfikatora - możliwa niezgodność kompilatora i wersje biblioteki. Sprawdź, czy dopasowanie vccorlib.h/.lib, vccorlib120.dll i c1xx.dll sobie|
|Błąd kompilatora C3941|"*warunek*": wymaga "/ clr" opcja wiersza polecenia|
|Błąd kompilatora C3942|Nieaktualne.|
|Błąd kompilatora C3943|Nieaktualne.|
|Błąd kompilatora C3944|Nieaktualne.|
|Błąd kompilatora C3945|"*typu*": nie można zgłosić lub przechwycić obiektu winrt, który nie pochodzi od Platform::Exception|
|Błąd kompilatora C3946|"*typu*": typeid nie można zastosować do tego typu|
|Błąd kompilatora C3947|"*typeid*": typeid nie można zastosować do rozwinięcia pakietu|
|Błąd kompilatora C3948|"*— słowo kluczowe*": rozwinięcie pakietu nie może występować w tym kontekście|
|Błąd kompilatora C3949|rozwinięcie pakietu, '...', nie może występować w abstrakcyjnym deklaratorze ujęty w nawiasy|
|Błąd kompilatora C3950|Nieaktualne.|
|Błąd kompilatora C3951|Nie można użyć wskaźników do elementów członkowskich w przewidywanego typu WinRT "*typu*". Zamiast tego użyj delegata.|
|Błąd kompilatora C3952|"*typu*": WinRT nie obsługuje "wchodzącym/wychodzącym" tablic. Użyj "const tablicy<T>^" dla "in" i "WriteOnlyArray<T>" lub "tablica<T>^ *" dla "out" w publicznych interfejsach API|
|Błąd kompilatora C3953|Nie można użyć zarządzanej klasy*typu*"w WinRT module.|
|Błąd kompilatora C3954|"*typu*": Tablica zwrócona z opublikowanej metody typu WinRT musi używać formy "Array<T>^"|
|Błąd kompilatora C3955|"*typu*": Konstruktor publiczny nie może zawierać parametru "out" lub "WriteOnlyArray<T>"|
|Błąd kompilatora C3956|"*typu*": Typ jest oznaczony jako wyłączny dla "*typu*"i nie może służyć jako podstawa"*derived_type*"|
|Błąd kompilatora C3957|"*typu*": nie można użyć typu "new" dla WinRT; Użyj zamiast tego "ref new"|
|Błąd kompilatora C3958|"*typu*": nie można Użyj "gcnew" na WinRT typu; Użyj zamiast tego "ref new"|
|Błąd kompilatora C3959|"ref new" może używać tylko do utworzenia obiektu z typem WinRT|
|Błąd kompilatora C3960|Nieaktualne.|
|Błąd kompilatora C3961|Konstruktor statyczny nie jest obsługiwane.|
|Błąd kompilatora C3962|Klasa generyczna nie jest obsługiwane.|
|Błąd kompilatora C3963|Wielowymiarowe tablice nie jest obsługiwana.|
...
|Błąd kompilatora C3966|"*funkcja*": funkcja ogólna nie jest obsługiwana.|
|Błąd kompilatora C3967|Błąd importowania "*identyfikator*"z modułu"*modułu*"|
|Błąd kompilatora C3968|token "*tokenu*" jest nieprawidłowy jako separator nazwy modułu; użyj kropki (".") zamiast tego|
|Błąd kompilatora C3969|Niespójne nazwy modułów: "*module1*"i"*module1*"|
|Błąd kompilatora C3970|"*identyfikator*": "*— słowo kluczowe*" można stosować tylko do "ref class" lub "ref struct" w zakresie globalnym lub zakresie przestrzeni nazw|
|Błąd kompilatora C3971|"*typu*": częściowa definicja nie może występować po pełnej definicji|
|Błąd kompilatora C3972|"*typu*": "partial" może być stosowane tylko do deklaracji lub definicji klasy|
|Błąd kompilatora C3973|Nieaktualne.|
|Błąd kompilatora C3974|Nieaktualne.|
|Błąd kompilatora C3975|"klasie/strukturze *identyfikator*" poprzednio zadeklarowany lub zdefiniowany za pomocą innego modyfikatora|
|Błąd kompilatora C3976|"*identifier1*"musi być zadeklarowany jako "public" Aby można było używać"*identifier2*"|
|Błąd kompilatora C3977|Użycie wymaga odwołania do zestawu, który definiuje "*identyfikator*"|
|Błąd kompilatora C3978|"*identyfikator*": właściwość statyczna nie może być elementem członkowskim typu interfejsu, lub wartość WinRT|
|Błąd kompilatora C3979|"*typu*": Użyj atrybutu "*atrybut*"zamiast z"*wartość*"|
|Błąd kompilatora C3980|"*typu*" nie może być wydane do metadanych|
|Błąd kompilatora C3981|"*typu*": typ wartościowy nie może mieć żadnych statycznych składowych danych*identyfikator*"|
|Błąd kompilatora C3982|"*typu*": typ wartościowy nie może mieć żadnych składowych danych niepublicznych*identyfikator*"|
|Błąd kompilatora C3983|"*typu*": typ wartościowy nie może mieć żadnych publicznych składowych innych niż danych*identyfikator*"|
|Błąd kompilatora C3984|"*typu*": typ inny niż wartości nie może mieć żadnych publicznych składowych danych*identyfikator*"|
|Błąd kompilatora C3985|"*identyfikator*": sygnatura publicznej składowej zawiera typ prywatny "*elementu członkowskiego*"|
|Błąd kompilatora C3986|"*identyfikator*": sygnatura publicznej składowej zawiera typ natywny "*elementu członkowskiego*"|
|Błąd kompilatora C3987|"*identyfikator*": sygnatura publicznej składowej zawiera typ natywny "*typu*"|
|Błąd kompilatora C3988|"*typu*": typ natywny nie może być publiczny|
|Błąd kompilatora C3989|"*typu*': Zagnieżdżony typ nie może być publiczny|
|Błąd kompilatora C3990|"*typu*": atrybut "*atrybut*" nie może być prywatny ani zagnieżdżony|
|Błąd kompilatora C3991|"*typu*": nie można zaimplementować niepublicznego lub zagnieżdżonego interfejsu "*interfejsu*"|
|Błąd kompilatora C3992|"*identyfikator*": sygnatura publicznej składowej zawiera nieprawidłowy typ "*typu*"|
|Błąd kompilatora C3993|"*typu*": typ wartościowy musi zawierać co najmniej jedno pole publiczne|
|Błąd kompilatora C3994|"*typu*": typ wartościowy nie może implementować interfejsów lub mieć funkcji wirtualnych|
|Błąd kompilatora C3995|"*typu*": typ wartościowy nie może mieć żadnych składowych zdarzenia*identyfikator*"|
|Błąd kompilatora C3996|Nieaktualne.|
|Błąd kompilatora C3998|"c ++*wersji*": nieobsługiwana wersja języka C++; przyjęto wartość domyślną "c ++*wersji*"|
|Błąd kompilatora C3999|Nieznany błąd, wybierz polecenie Pomoc techniczna w Visual C++ menu Pomoc lub Otwórz plik pomocy technicznej, aby uzyskać więcej informacji|
