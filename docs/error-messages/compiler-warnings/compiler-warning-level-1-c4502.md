---
title: Kompilatora (poziom 1) ostrzeżenie C4502 | Dokumentacja firmy Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.reviewer: ''
ms.suite: ''
ms.technology:
- cpp-tools
ms.tgt_pltfrm: ''
ms.topic: error-reference
f1_keywords:
- C4502
dev_langs:
- C++
helpviewer_keywords:
- C4502
ms.assetid: d8d43153-a40c-4b96-bc11-64028a144d70
caps.latest.revision: 6
author: corob-msft
ms.author: corob
manager: ghogen
ms.workload:
- cplusplus
ms.openlocfilehash: 57fb8451d094dbcbc4c71d9843346b2b397671c0
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 12/21/2017
---
# <a name="compiler-warning-level-1-c4502"></a>Kompilator C4502 ostrzegawcze (poziom 1)
"Specyfikacja powiązania" wymaga użycia słowa kluczowego "extern" i musi poprzedzać wszystkie inne specyfikatory  
  
 Połączenie został określony bez `extern` — słowo kluczowe. Połączenie nie jest ważna dla typów innych niż zewnętrzny.  
  
 Kompilator zakłada, że `extern` — słowo kluczowe.