---
title: Ostrzeżenia kompilatora od C4000 C4199
ms.date: 11/17/2017
f1_keywords:
- C4000
- C4002
...
ms.locfileid: "50617701"
---
# <a name="compiler-warnings-c4000-through-c4199"></a>Ostrzeżenia kompilatora od C4000 C4199
...
|Ostrzeżenie kompilatora (poziom 1) C4002|zbyt wiele parametrów rzeczywistych dla makra 'Identyfikator'|
...
|[Ostrzeżenie kompilatora (poziom 3) C4018](../../error-messages/compiler-warnings/compiler-warning-level-3-c4018.md)|"expression": niezgodność ze znakiem/bez znaku|
...
|Ostrzeżenie kompilatora (poziom 1) C4036|nienazwane "type" jako rzeczywisty parametr|
|Ostrzeżenie kompilatora (poziom 1) C4038|"modyfikator": niedozwolony modyfikator klasy|
|Ostrzeżenie kompilatora (poziom 1) C4041|ograniczenie kompilatora: Kończenie wyników przeglądarki|
...
|Ostrzeżenie kompilatora (poziom 4) C4052|deklaracje funkcji, inny, jeden z nich zawiera zmienne argumenty|
...
|Ostrzeżenie kompilatora (poziom 1) C4068|Nieznana pragma|
...
|Ostrzeżenie kompilatora (poziom 1) C4076|"type_modifier": nie można używać z typu "typename"|
...
|Ostrzeżenie kompilatora (poziom 1) C4086|Oczekiwano parametru pragma "1", "2", "4", "8" lub "16"|
|Ostrzeżenie kompilatora (poziom 1) C4087|'Funkcja': zadeklarowana z listą parametrów "void"|
...
|Ostrzeżenie kompilatora (poziom 1) C4112|#line wymaga całkowitą z zakresu od 1 do "line_count"|
...
|Ostrzeżenie kompilatora (poziom 1) C4117|Nazwa makro "name" jest zarezerwowana, "polecenia" zignorowany|
|Ostrzeżenie kompilatora (poziom 1) C4119|base1 różnych podstaw "—" i "base2 —" określone|
|Ostrzeżenie kompilatora (poziom 1) C4120|Niezgodność/niepodstawowy|
...
|Ostrzeżenie kompilatora (poziom 4) C4132|"object": obiekt const powinien zostać zainicjowany|
...
|Ostrzeżenie kompilatora (poziom 1) C4141|"modyfikator": użyto więcej niż raz|
...
|Ostrzeżenie kompilatora (poziom 1) C4143|pragma "same_seg" nie jest obsługiwana; Użyj alokacji __based|
...
|Ostrzeżenie kompilatora (poziom 1) C4165|"HRESULT" jest konwertowana na "bool"; Czy na pewno jest to, co chcesz zrobić?|
...
|Ostrzeżenie kompilatora (poziom 1) C4168|ograniczenie kompilatora: nie ma typów debugera, Usuń program bazy danych "bazy danych" i ponownie skompiluj|
...
|Ostrzeżenie kompilatora (poziom 1) C4174|"name": niedostępny jako składnik #pragma|
...
|Ostrzeżenie kompilatora (poziom 1) C4176|"podskładnika": nieznany podskładnik dla składnika przeglądarki #pragma|
|Ostrzeżenie kompilatora (poziom 1) C4177|#pragma "pragma" powinna być użyta tylko w zakresie globalnym lub zakresie przestrzeni nazw|
|Ostrzeżenie kompilatora (poziom 1) C4178|Stała przypadku "" za duża stała dla typu wyrażenia switch|
...
|Ostrzeżenie kompilatora (poziom 1) C4182|#include poziom zagnieżdżenia to "nest_count" głębokiego; możliwa nieskończona rekursja|
...
|Ostrzeżenie kompilatora (poziom 1) C4186|#import atrybutów "attribute" wymaga argumentów 'argument_count'; ignorowane|
|Ostrzeżenie kompilatora (poziom 1) C4187|atrybuty #import "attribute1" i "attribute2" są niezgodne; oba są ignorowane|
|Ostrzeżenie kompilatora (poziom 1) C4188|wyrażenie stałe nie jest spójne|
|Ostrzeżenie kompilatora (poziom 4) C4189|'Identyfikator': lokalna zmienna jest zainicjowana, ale nie odwołuje się|
...
|Ostrzeżenie kompilatora (poziom 3) C4191|' operator/operacja': niebezpieczna konwersja z "type_of_expression" do "type_required" \nCalling tej funkcji za pośrednictwem wynikowego wskaźnika może spowodować awarię programu|
