...
| Ostrzeżenie kompilatora (poziom 4) C4845 | "\_\_declspec (bez\_init\_All)" jest ignorowany, jeśli element "/d1initall\[0\|1\|2\|3]" nie został określony w wierszu polecenia |
...
| Ostrzeżenie kompilatora (poziom 4) C4847 | "\_\_declspec (bez\_init\_All)" można stosować tylko do funkcji, typu klasy lub zmiennej lokalnej: zignorowano |
...
|Ostrzeżenie kompilatora (poziom 4) C4881|Konstruktor i/lub destruktor nie zostaną wywołane dla tile_static zmiennej "*Variable*"|
...
|[Ostrzeżenie kompilatora (poziom 1) C4920](compiler-warning-level-1-c4920.md)|element członkowski wyliczenia enum member_1 = value_1 już występował w wyliczeniu wyliczenia jako member_2 value_2|
...
|Ostrzeżenie kompilatora (poziom 1) C4934|"__delegate (multiemisja)" jest przestarzałe, użyj "\__delegate" zamiast niego|
...
|[Ostrzeżenie kompilatora (poziom 1, błąd) C4936](compiler-warning-c4936.md)|Ta __declspec jest obsługiwana tylko wtedy, gdy skompilowano z opcją/CLR lub/CLR: Pure|
...
|[Ostrzeżenie kompilatora (poziom 1) C4952](compiler-warning-level-1-c4952.md)|"*Function*": nie znaleziono danych profilowych w bazie danych programu "*pgd_file*"|
...
|Ostrzeżenie kompilatora (poziom 1) C4971|Kolejność argumentów: \<> obiektu docelowego, \<funkcji docelowej > dla konstruktora delegatów jest przestarzała, użyj \<funkcji docelowej >, \<obiekt docelowy = "" >|
...
| Ostrzeżenie kompilatora (poziom 1) C5047 | Użycie niestandardowych \_\_, jeśli\_istnieje z modułami nie są obsługiwane |
...
| Ostrzeżenie kompilatora (poziom 1) C5100 | \_\_VA\_ARGS\_\_ jest zarezerwowany do użycia w makrach wariadyczne |
...
| Ostrzeżenie kompilatora (poziom 1) C5104 | znaleziono "*ciąg1*#*ciąg2*" na liście zastąpień makr, czy chodziło o "*ciąg1*" "#*ciąg2*"? |
