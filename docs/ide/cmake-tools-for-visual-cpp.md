...
Niedawna użytkowników programu Visual Studio może użyć CMake generuje pliki projektu MSBuild, które IDE następnie używane przez funkcję IntelliSense, przeglądanie i kompilacji. W programie Visual Studio 2017 r. **Visual C++ Tools dla CMake** składnika używa funkcji "Otwórz Folder", aby umożliwić IDE korzystają z plików projektu CMake (na przykład CMakeLists.txt) bezpośrednio do celów IntelliSense i przeglądania. Jeśli używasz programu Visual Studio generator pliku tymczasowego projektu jest generowana i przekazany do msbuild.exe, ale nigdy nie jest załadowany dla IntelliSense i przeglądania. 
...
Narzędzia Visual C++ Tools for CMake jest instalowany domyślnie w ramach tworzenia klasycznych aplikacji z C++ obciążenia.
...
1. Wybierz element docelowy w listy rozwijanej debugowania i naciśnij klawisz **F5** lub kliknij przycisk "Uruchom". Projekt zostanie automatycznie kompilacji, podobnie jak z rozwiązań programu Visual Studio.
2.  Kliknij prawym przyciskiem CMakeLists.txt i wybierz z menu kontekstowego kompilacji. Jeśli masz wiele obiektów docelowych w strukturze folderu, użytkownik może kompilacji wszystkie lub tylko jeden określony element docelowy, lub
  
3. Wybierz z menu głównego **kompilacji | Tworzenie rozwiązania** (**F7** lub **Ctrl + Shift + B**) (Upewnij się, że docelowy CMake jest zaznaczona w **element startowy** listy rozwijanej w **Ogólne** paska narzędzi).

![Polecenia menu CMake kompilacji](media/cmake-build-menu.png) 
...
![Błędy kompilacji CMake](media/cmake-build-errors.png)

W folderze z wieloma docelowymi kompilacji można wybrać elementu kompilacji w CMake menu lub CMakeLists.txt menu kontekstowym, aby określić, która docelowa CMake do tworzenia. Naciśnięcie przycisku **Ctrl + Shift + B** w CMake projektu kompilacje bieżący aktywny dokument. 

## <a name="debugging-the-project"></a>Debugowanie projektu
Aby debugować projektu CMake, wybrać żądaną konfiguracją i naciśnij klawisz **F5**, lub kliknij przycisk Uruchom na pasku narzędzi. Jeśli przycisk Uruchom jest wyświetlany komunikat "Wybierz element startowy" kliknij strzałkę w dół i wybierz element docelowy, który chcesz uruchomić. (W projekcie CMake "bieżący dokument" opcja jest prawidłowa tylko dla plików .cpp.) 

 ![Przycisk Uruchom CMake](media/cmake-run-button.png)

 Naciśnięcie przycisku **F5** będzie pierwszej kompilacji projektu, jeśli zostały zmienione od czasu poprzedniej kompilacji.

## <a name="configuring-cmake-debugging-sessions"></a>Konfigurowanie CMake sesji debugowania
Wszystkie elementy docelowe wykonywalnego CMake są wyświetlane na liście rozwijanej element startowy ogólne pasku narzędzi. Aby rozpocząć sesję debugowania, wystarczy wybrać jedną i uruchomić debugera.

 ![CMake uruchamiania elementu z listy rozwijanej](media/cmake-startup-item-dropdown.png)

Można również uruchomić sesję debugowania, w menu CMake.

Aby dostosować ustawienia debugera dla dowolnego pliku wykonywalnego CMake docelowego w projekcie, kliknij prawym przyciskiem myszy w określonym pliku CMakeLists.txt, a następnie wybierz **debugowania i ustawienia uruchamiania**, po wybraniu elementu docelowego CMake w podmenu w pliku o nazwie Launch.VS.JSON jest tworzony. Ten plik jest wstępnie wypełnione informacjami o docelowym CMake, wybrana przez Ciebie i pozwala określić dodatkowe parametry, takie jak program argumentów lub typ debugera. Aby odwołać dowolny klawisz, w pliku CMakeSettings.json, należy poprzedzić go z "cmake." w launch.vs.json. W poniższym przykładzie przedstawiono plik prosty launch.vs.json ściągający wartości klucza "remoteCopySources" w pliku CMakeSettings.json dla aktualnie wybranej konfiguracji:
...
```
Bezpośrednio po zapisaniu pliku launch.vs.json wpis jest tworzony na liście rozwijanej element startowy z nową nazwą. Edytując plik launch.vs.json, można tworzyć, jak dla dowolnej liczby CMake celów, takich jak wiele konfiguracje debugowania.

**Visual Studio 2017 wersji 15.4**: Launch.vs.json obsługuje zmienne, które są zadeklarowane w CMakeSettings.json (patrz poniżej). Ponadto wprowadzono pojęcie "currentDir", który zostanie ustawione jako bieżący katalog uruchamiania aplikacji:
...
```
Po uruchomieniu aplikacji, wartość `currentDir` jest 
...
Aby edytować plik CMakeLists.txt, kliknij prawym przyciskiem myszy plik w **Eksploratora rozwiązań** i wybierz polecenie **Otwórz**. Jeśli wprowadzisz zmiany w pliku pasek stanu żółty pojawia się i informuje o IntelliSense zostanie zaktualizowana i daje możliwość anulowania operacji aktualizacji. Aby uzyskać informacje o CMakeLists.txt, zobacz [dokumentacji CMake](https://cmake.org/documentation/).
  ...
  ![Edytowanie pliku CMakeLists.txt](media/cmake-cmakelists.png)

Bezpośrednio po zapisaniu pliku kroku konfiguracji automatycznie uruchom ponownie i wyświetlać informacje w oknie danych wyjściowych. Błędy i ostrzeżenia są wyświetlane na liście błędów lub oknie danych wyjściowych. Kliknij dwukrotnie błąd na liście błędów, aby przejść do ataku wiersza w CMakeLists.txt.

  ![Błędy w pliku CMakeLists.txt](media/cmake-cmakelists-error.png)
 
## <a name="cmake_settings"></a>CMake ustawienia i konfiguracje niestandardowe
Domyślnie program Visual Studio oferuje sześć domyślne konfiguracje CMake ("x86-Debug", "x86 wersja", "x64-Debug", "x64-wersja", "Linux-Debug" i "Linux wersja"). Te konfiguracje zdefiniuj, jak CMake.exe jest wywoływane w celu utworzenia pamięci podręcznej CMake dla danego projektu. Aby zmodyfikować te konfiguracje lub Utwórz nową konfigurację niestandardowe, wybierz **CMake | Zmień ustawienie CMake**, a następnie wybierz plik CMakeLists.txt ustawienia będą dotyczyć. Zmień ustawienia CMake jest również dostępny w menu kontekstowym pliku w **Eksploratora rozwiązań**. To polecenie tworzy plik CMakeSettings.json w folderze projektu. Ten plik służy do ponownego utworzenia pliku pamięci podręcznej CMake, na przykład po wykonaniu operacji "Wyczyść". 
...
```
1.  Nazwa: nazwa, która będzie wyświetlana na liście rozwijanej konfiguracji C++. Wartość tej właściwości mogą służyć jako makro `${name}` do określenia wartości innych właściwości. Na przykład zobacz "buildRoot" definicji CMakeSettings.json.
2.  Generator: mapuje z przełącznikiem -G i określa generatora ma być używany. Ta właściwość umożliwia również jako makro `${generator}` ułatwia określenie wartości innych właściwości. Program Visual Studio obsługuje obecnie następujących generatory CMake:
    - "Nindżą"
    ...
    - "Programu visual Studio 15 2017 Win64"
    - 
...
3.  buildRoot: mapuje - DCMAKE_BINARY_DIR przełącznika i określa, gdzie zostanie utworzone w pamięci podręcznej CMake. Jeśli folder nie istnieje, zostanie utworzona.
4.  zmienne: zawiera parę kombinacji nazwy i wartości zmiennych CMake, które będą zostały przekazane jako - Dname = wartość na CMake. Instrukcje kompilacji projektu CMake określić dodawanie zmiennych bezpośrednio do pliku pamięci podręcznej CMake, zaleca się dodanie je tutaj zamiast tego.
5.  cmakeCommandArgs: określa żadnych dodatkowych przełączników, które mają zostać przekazane do CMake.exe.
6.  configurationType: Określa typ konfiguracji kompilacji dla wybranego generatora. Obecnie obsługiwane wartości to "Debug", "MinSizeRel", "Wersja" i "RelWithDebInfo".
...
CmakeSettings.json obsługuje teraz inherted środowiska. Ta funkcja umożliwia tworzenie zmienną niestandardową, która:

```json
  "inheritEnvironments": [ "msvc_x64_x64" ]
```

To pole określa zmienne, które są automatycznie przekazywane do CMake.exe po uruchomieniu. W powyższym przykładzie jest taki sam jak uruchamianie "wiersz polecenia dla deweloperów VS 2017" z `-arch=amd64 -host_arch=amd64` argumentów.

W poniższej tabeli przedstawiono obsługiwane wartości i ich odpowiedniki wiersza polecenia:
...
Korzystając z wielu projektów CMake taką samą nazwę konfiguracji CMake (na przykład x86-Debug), wszystkie z nich są konfigurowane i wbudowane (folder główny własnych kompilacji) po wybraniu tej konfiguracji. Można debugować elementy docelowe ze wszystkich projektów CMake uczestniczących w tej konfiguracji CMake.
   ...
   ![Tworzenie CMake tylko element menu](media/cmake-build-only.png)

Aby ograniczyć kompilacji i debugowania sesji do podzbioru projekty w obszarze roboczym, Utwórz nową konfigurację o unikatowej nazwie w pliku CMakeSettings.json i dotyczą tylko tych projektów. Podczas tej konfiguracji jest zaznaczone, IntelliSense, kompilowania, i debugowanie zostaną włączone tylko dla określonych tych projektów.

## <a name="troubleshooting-cmake-cache-errors"></a>Rozwiązywanie problemów z błędami pamięci podręcznej CMake
Aby uzyskać więcej informacji o stanie CMake pamięci podręcznej, aby zdiagnozować problem, otwórz menu głównego CMake lub menu kontekstowe CMakeLists.txt **Eksploratora rozwiązań** na jeden z tych poleceń:
