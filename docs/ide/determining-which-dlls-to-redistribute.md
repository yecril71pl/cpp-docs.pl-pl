...
Aby ułatwić obejmują pakietu redystrybucyjnego bibliotek DLL z Instalatorem, są dostępne jako autonomiczny *pakiety redystrybucyjne*. Są to pliki wykonywalne architektury, które zainstaluj pakiet redystrybucyjny pliki na komputerze użytkownika przy użyciu centralnej wdrażania. Na przykład program vcredist\_x86.exe instaluje biblioteki 32-bitowego dla x86 komputery, program vcredist\_x64.exe instaluje 32-bitowe i 64-bitowych bibliotek x64 komputerów i program vcredist\_ARM.exe instaluje biblioteki dla ARM komputery. Zalecamy wdrożenie centralnej, ponieważ firma Microsoft mogłaby użyć usługi Windows Update, można zaktualizować niezależnie te biblioteki. Oprócz kopiowania w instalacji programu Visual Studio bieżące pakiety redystrybucyjne są dostępne do pobrania. Łącza do najnowszych obsługiwanych pakietu redystrybucyjnego pakiety dla procesami bieżące i starsze, zobacz [najnowszej obsługiwane pliki do pobrania Visual C++](https://support.microsoft.com/help/2977003/the-latest-supported-visual-c-downloads). Określone wcześniejszych wersji do dystrybucji pakietów można znaleźć przeszukując [Microsoft Download Center](http://go.microsoft.com/fwlink/p/?LinkId=158431) dla "Visual C++ Redistributable Packages".

Główny numer wersji pakietu redystrybucyjnego wdrożeniem musi odpowiadać wersji zestawu narzędzi programu Visual Studio, używany do tworzenia aplikacji, a wersja pomocnicza musi być taka sama lub nowszej. Visual Studio 2017 i programu Visual Studio 2015 mają numery wersji narzędzi zgodne, co oznacza, że Visual 2017 Studio wstępnie pliki mogą być używane przez aplikacje przy użyciu zestawu narzędzi 2015. Może być zgodne, nie jest obsługiwana przy użyciu plików pakietu redystrybucyjnego 2015 w aplikacji skompilowanych za pomocą zestawu narzędzi 2017 r. Obsługiwany jest tylko przy użyciu pakietu redystrybucyjnego pakietu, która jest taka sama jak lub nowszej wersji zestawu narzędzi.
