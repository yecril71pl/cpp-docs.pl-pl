...
Aby ukończyć ten przewodnik, musisz mieć program Visual Studio 2017 lub nowszy. Jeśli potrzebujesz kopii, Oto krótki przewodnik: [Zainstaluj obsługę języka C++ w programie Visual Studio](../build/vscpp-step-0-installation.md). Jeśli jeszcze tego nie zrobiono, wykonaj następujące czynności po zakończeniu instalacji za pomocą samouczka "Hello, World", aby upewnić się, że składniki języka C++ są prawidłowo zainstalowane i wszystkie działają.

Pozwala ona na zapoznanie się z podstawą języka C++ i wiedzą, w jaki sposób kompilator, konsolidator i debuger są używane dla programu. W tym samouczku założono również, że znasz system Windows i jak używać menu, okien dialogowych,
...
1. Edytuj plik plik Cardgame. h i wprowadź następujące zmiany:
   ...
   - Po domyślnym destruktorze Dodaj wbudowaną deklarację dla `static int` funkcji składowej o nazwie *getuczestnicy* , która nie przyjmuje parametrów i zwraca `totalParticipants` wartość.
   ...
   Wiersz `#pragma once` nakazuje kompilatorowi dołączenie pliku nagłówka tylko jeden raz. Aby uzyskać więcej informacji, zobacz [jeden raz](../preprocessor/once.md). Aby uzyskać informacje na temat innych słów kluczowych języka C++ w pliku nagłówkowym, zobacz [klasy](../cpp/class-cpp.md), [int](../cpp/fundamental-types-cpp.md), [static](../cpp/storage-classes-cpp.md)i [Public](../cpp/public-cpp.md).
...
1. W oknie Edytor **gier. cpp** Zastąp istniejący kod:
   ...
   Kod dodaje funkcję testową, `PlayGames` , do kodu źródłowego i wywołuje ją w `main` .
...
Gratulacje, projekt aplikacji i rozwiązanie zostały pomyślnie skompilowane. Przejdź do przewodnika, aby dowiedzieć się więcej na temat tworzenia projektów kodu C++ w programie Visual Studio.

## <a name="next-steps"></a>Następne kroki

**Poprzedni:** [Używanie środowiska IDE programu Visual Studio na potrzeby programowania aplikacji klasycznych w języku C++](../ide/using-the-visual-studio-ide-for-cpp-desktop-development.md)<br/>
**Dalej:** [Przewodnik: kompilowanie projektu (C++)](../ide/walkthrough-building-a-project-cpp.md)
