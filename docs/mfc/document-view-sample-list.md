---
title: "Lista przykładów dokumentów widok | Dokumentacja firmy Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-windows
ms.tgt_pltfrm: 
ms.topic: article
dev_langs: C++
ms.assetid: 6f087ce8-2f46-433c-b674-4c110743b289
caps.latest.revision: "11"
author: mikeblome
ms.author: mblome
manager: ghogen
ms.openlocfilehash: 9a98e087a9155268997e913c2ecafc1bfd7020e4
ms.sourcegitcommit: ebec1d449f2bd98aa851667c2bfeb7e27ce657b2
ms.translationtype: MT
ms.contentlocale: pl-PL
ms.lasthandoff: 10/24/2017
---
# <a name="documentview-sample-list"></a>Lista przykładów dokumentów/widoków
Zobacz następujące programy przykładowe ilustrujące przy użyciu architektury dokument/widok MFC w sposób:  
  
 *Dokument/Widok zmian*  
  
-   [MDI](../visual-cpp-samples.md)  
  
-   [BAZGROŁÓW](../visual-cpp-samples.md)  
  
-   [VIEWEX](../visual-cpp-samples.md)  
  
 *Okno dialogowe interfejsu zastępuje dokument/widok*  
  
-   [CMNCTRL1](../visual-cpp-samples.md)  
  
-   [CMNCTRL2](../visual-cpp-samples.md)  
  
 *Przy użyciu COleDocument i jej klas pochodnych*  
  
-   [KONTENER](../visual-cpp-samples.md)  
  
-   [HIERSVR](../visual-cpp-samples.md)  
  
-   [OCLIENT](../visual-cpp-samples.md)  
  
## <a name="see-also"></a>Zobacz też  
 [Architektura dokument/widok](../mfc/document-view-architecture.md)

