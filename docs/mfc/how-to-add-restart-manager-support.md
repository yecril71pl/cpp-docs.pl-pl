...
1. Upewnij się, że `InitInstance` metoda aplikacji wywołuje jego element nadrzędny `InitInstance` metoda: [CWinApp::InitInstance](../mfc/reference/cwinapp-class.md#initinstance) lub `CWinAppEx::InitInstance`. `InitInstance` Metoda jest odpowiedzialna za sprawdzanie *m_dwRestartManagerSupportFlags* parametru.
