...
2.  Powiadomień dotyczących formantu karty  
     ...
     W tym WM_COMMAND powiadomień od formantów i innych okien podrzędnych do ich nadrzędnej systemu windows. Na przykład formantu edycyjnego wysyła nadrzędnego komunikatów WM_COMMAND, zawierającego kod powiadomień dotyczących formantu EN_CHANGE, gdy użytkownik ma podjąć akcję, która może zmienić tekst w formancie edycyjnym. Okna obsługi wiadomości odpowiada na komunikat powiadomienia w odpowiedni sposób, takie jak pobieranie tekstu w formancie.  
  
     Platformę kieruje komunikaty powiadomień dotyczących formantu, jak inny **WM_** wiadomości. Jedynym wyjątkiem jest jednak komunikatów powiadomień dotyczących formantu BN_CLICKED wysyłane przez przyciski, gdy użytkownik kliknie je. Ten komunikat jest specjalnym jako komunikat polecenia i kierowania podobnie jak inne polecenia.  
  
3.  Komunikaty poleceń  
  
     Obejmuje to WM_COMMAND — komunikaty powiadomień z obiektów interfejsu użytkownika: menu, przycisków paska narzędzi i klawiszy skrótów. Platformę przetwarza polecenia inaczej niż inne komunikaty i mogą być obsługiwane za więcej typów obiektów, zgodnie z objaśnieniem w [obiekty docelowe poleceń](../mfc/command-targets.md).  
