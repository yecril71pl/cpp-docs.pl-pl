...
1. Dodaj parametr o nazwie *Row* (typ *Short*), używając kontrolek **Nazwa parametru** i **Typ parametru** .
...
Ten kod deklaruje dwie funkcje o nazwie `GetArray` i `SetArray` zezwala użytkownikowi na żądanie określonego wiersza i kolumny podczas uzyskiwania dostępu do właściwości.
...
Jeśli klasą bazową kontrolki ActiveX jest `COleControl` , można przesłonić [CWnd::P retranslatemessage](reference/cwnd-class.md#pretranslatemessage) do obsługi komunikatów przed ich przetworzeniem przez kontener. W przypadku korzystania z tej techniki zawsze zwraca **wartość true** , jeśli wiadomość została przesłonięta `PreTranslateMessage` .
