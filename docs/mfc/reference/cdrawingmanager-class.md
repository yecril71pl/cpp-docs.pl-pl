...
|[CDrawingManager::D rawEllipse](#drawellipse)|Rysuje elipsę z podanymi kolorami wypełnienia i obramowania.|
...
*zmienia*\
podczas Parametr [CSize](../../atl-mfc-shared/reference/csize-class.md) , który wskazuje rozmiar mapy bitowej.

*pBits*\
określoną Wskaźnik do wskaźnika danych, który odbiera lokalizację wartości bitowych DIB.
...
*x1*\
podczas Współrzędna x, w której uruchamiany jest wiersz.

*Y1*\
podczas Współrzędna y, w której uruchamiany jest wiersz.

*x2*\
podczas Współrzędna x, w której znajduje się linia.

*Y2*\
podczas Współrzędna y, w której znajduje się linia.

*clrLine*\
podczas Kolor wiersza.
...
Można podać dwie prawidłowe mapy bitowe dla dolnego i prawego cienia przy użyciu parametrów *pBmpSaveBottom* i *pBmpSaveRight*. Jeśli te obiekty [CBitmap](../../mfc/reference/cbitmap-class.md) mają dołączony obiekt GDI, `DrawShadow` będą używać tych map bitowych jako cieni. Jeśli `CBitmap` parametry nie mają dołączonego obiektu GDI, program `DrawShadow` rysuje cień i dołącza mapy bitowe do parametrów. W przyszłych wywołaniach do `DrawShadow` , możesz podać te mapy bitowe, aby przyspieszyć proces rysowania. Aby uzyskać więcej informacji na temat `CBitmap` klas i obiektów GDI, zobacz [grafika Objects](../../mfc/graphic-objects.md).
...
*bHorz*<br/>
podczas Parametr logiczny, który wskazuje, czy `Fill4ColorsGradient` kolory gradientu poziomego lub pionowego. Wartość TRUE wskazuje gradient poziomy.
...
Ta metoda i `CDrawingManager::HLStoRGB_TWO` Metoda wykonują tę samą operację, ale wymagają różnych wartości parametru *H* . W tej metodzie *H* jest wartością procentową okręgu. W `CDrawingManager::HLStoRGB_TWO` metodzie *H* jest wartością stopnia z zakresu od 0 do 360, co reprezentuje czerwony. Na przykład, w przypadku `HLStoRGB_ONE` , wartość 0,25 dla *H* jest równa wartości 90 z `HLStoRGB_TWO` .
...
Ta metoda oraz Metoda [CDrawingManager:: HLStoRGB_ONE](#hlstorgb_one) wykonują tę samą operację, ale wymagają różnych wartości parametru *H* . W tej metodzie *H* jest wartością stopnia z zakresu od 0 do 360, co reprezentuje czerwony. W metodzie [CDrawingManager:: HLStoRGB_ONE](#hlstorgb_one) *H* jest wartością procentową okręgu. Na przykład, w przypadku `HLStoRGB_ONE` , wartość 0,25 dla *H* jest równa wartości 90 z `HLStoRGB_TWO` .
...
*C*\
podczas Liczba z zakresu od 0 do 360, która wskazuje odcień koloru.

*Wolumin*\
podczas Liczba z zakresu od 0 do 1, która wskazuje nasycenie koloru.

*V*\
podczas Liczba z zakresu od 0 do 1, która wskazuje wartość koloru.
...
Ta metoda jest metodą pomocniczą `CDrawingManager` używaną przez klasę do obliczania pojedynczych składników czerwony, zielony i niebieski koloru w reprezentacji HSV lub HSL. Ta metoda nie jest przeznaczona do wywoływania bezpośrednio przez programistę. Parametry wejściowe są wartościami, które są zależne od algorytmu konwersji.
...
*Przestrzeń*\
podczas Kolor w wartościach RGB.

*C*\
określoną Wskaźnik do podwójnej lokalizacji, w której metoda przechowuje odcień koloru.

*Wolumin*\
określoną Wskaźnik do podwójnej lokalizacji, gdzie metoda przechowuje nasycenie koloru.

*&*\
określoną Wskaźnik do podwójnej lokalizacji, gdzie metoda przechowuje jasność dla koloru.
...
Ta metoda jest metodą pomocnika, która jest używana przez metodę [CDrawingManager::D rawshadow](#drawshadow) . Zalecamy, aby narysować cień, `CDrawingManager::DrawShadow` zamiast tego wywołać.
...
*pBits*\
podczas Wskaźnik do wartości bitowych mapy bitowej.

*CX*\
podczas Łączna szerokość mapy bitowej.

*cy*\
podczas Łączna wysokość mapy bitowej.

*y*\
podczas Współrzędna x pikseli w mapie bitowej, która ma zostać zmieniona.

*t*\
podczas Współrzędna y pikseli w mapie bitowej, która ma zostać zmieniona.

*Kolor*\
podczas Nowy kolor pikseli identyfikowany przez podane współrzędne.
...
*color1*\
podczas Pierwszy kolor, który ma zostać zmieszany.

*color2*\
podczas Drugi kolor, który ma zostać zmieszany.

*dblLumRatio*\
podczas Współczynnik jaskrawości nowego koloru. `SmartMixColors` Mnoży jaskrawość mieszanego koloru przez ten współczynnik przed określeniem koloru końcowego.

*K1*\
podczas Ważony współczynnik dla pierwszego koloru.

*K2*\
podczas Ważony współczynnik dla drugiego koloru.
