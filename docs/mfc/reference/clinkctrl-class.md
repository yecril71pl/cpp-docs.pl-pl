...
Ten formant (i w związku `CLinkCtrl` z tym Klasa) jest dostępny tylko dla programów uruchomionych w systemie Windows XP i nowszych.
...
*dwStyle*<br/>
Określa styl kontrolki link. Zastosuj dowolną kombinację stylów kontrolek. Aby uzyskać więcej informacji, `Windows SDK` zobacz [typowe style kontrolek](/windows/win32/Controls/common-control-styles) w.
...
`CLinkCtrl` Obiekt jest konstruowany w dwóch krokach. Najpierw Wywołaj konstruktora, a następnie Wywołaj `Create`, który tworzy formant łącza i dołącza go `CLinkCtrl` do obiektu. Jeśli chcesz użyć rozszerzonych stylów systemu Windows z kontrolką, wywołaj [CLinkCtrl:: CreateEx](#createex) zamiast `Create`.
...
Ta funkcja członkowska implementuje zachowanie komunikatu Win32 [LM_GETIDEALHEIGHT](/windows/win32/Controls/lm-getidealheight), zgodnie z opisem w Windows SDK.
...
|Parametr|Opis|
|---------------|-----------------|
|*cxMaxWidth*|podczas Maksymalna szerokość łącza (w pikselach).|
|określoną \* *psize*|Wskaźnik do struktury [rozmiaru](/windows/win32/api/windef/ns-windef-size) systemu Windows. Gdy ta metoda zwraca, element członkowski `SIZE` w strukturze zawiera idealną wysokość tekstu linku dla szerokości tekstu linku, która jest określona przez *cxMaxWidth*. Element członkowski *CX* struktury zawiera szerokość tekstu linku, która jest faktycznie wymagana.|

### <a name="return-value"></a>Wartość zwracana

Preferowana wysokość tekstu łącza (w pikselach). Wartość zwracana jest taka sama jak wartość wartości składowej `SIZE` *cy* struktury.
...
Ta funkcja członkowska implementuje zachowanie komunikatu Win32 [LM_GETITEM](/windows/win32/Controls/lm-getitem), zgodnie z opisem w Windows SDK.
...
Ta funkcja członkowska implementuje zachowanie komunikatu Win32 [LM_HITTEST](/windows/win32/Controls/lm-hittest), zgodnie z opisem w Windows SDK.
...
Ta funkcja członkowska implementuje zachowanie komunikatu Win32 [LM_SETITEM](/windows/win32/Controls/lm-setitem), zgodnie z opisem w Windows SDK.
