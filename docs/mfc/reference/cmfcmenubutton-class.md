...
`CMFCMenuButton`Klasa pochodzi od klasy [CMFCButton](../../mfc/reference/cmfcbutton-class.md) , która z kolei pochodzi od [klasy CButton](../../mfc/reference/cbutton-class.md). W związku z tym możesz użyć `CMFCMenuButton` w kodzie w taki sam sposób jak w przypadku użycia `CButton` .
...
Jeśli `m_bStayPressed` element członkowski ma wartość false, przycisk menu nie zostanie wciśnięty po kliknięciu przycisku. W takim przypadku struktura wyświetla tylko menu podręczne.

Jeśli `m_bStayPressed` element członkowski ma wartość true, przycisk menu zostanie naciśnięty, gdy użytkownik kliknie przycisk. Pozostaje wciśnięty do momentu zamknięcia przez użytkownika menu podręcznego przez wybranie lub anulowanie.
...
Jeśli wywołasz tę funkcję, a *bCalcOnly* ma wartość true, `SizeToContent` program obliczy tylko nowy rozmiar przycisku.
