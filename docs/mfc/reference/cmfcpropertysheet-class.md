...
2.  Konstruowania [CMFCPropertyPage](../../mfc/reference/cmfcpropertypage-class.md) obiektu dla każdej strony właściwości.  
  
3.  Wywołaj [CMFCPropertySheet::SetLook](#setlook) metody w Konstruktorze CMyPropertySheet. Parametr metody Określa, czy strony właściwości są wyświetlane jako karty wzdłuż górnej lub lewej krawędzi arkusza właściwości; karty w stylu arkusz właściwości programu Microsoft OneNote; przycisków w formancie paska narzędzi programu Microsoft Outlook; węzły w drzewie; lub jako listę elementów po lewej stronie arkusza właściwości.  
  
4.  Jeśli tworzysz arkusz właściwości w stylu narzędzi programu Microsoft Outlook, wywołaj [CMFCPropertySheet::SetIconsList](#seticonslist) do kojarzenia listy obrazów oraz na stronach właściwości.  
  
5.  Wywołaj [CMFCPropertySheet::AddPage](#addpage) metody dla każdej strony właściwości.  
  
6.  Tworzenie `CMFCPropertySheet` kontroli i wywołać jej `DoModal` metody.  
