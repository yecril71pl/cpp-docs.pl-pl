...
|Name (Nazwa)|Opis|
|----------|-----------------|
|[CToolBarCtrl:: CToolBarCtrl](#ctoolbarctrl)|Konstruuje obiekt `CToolBarCtrl`.|
...
|[CToolBarCtrl:: Create](#create)|Tworzy formant Toolbar i dołącza go do obiektu `CToolBarCtrl`.|
|[CToolBarCtrl:: CreateEx](#createex)|Tworzy formant Toolbar z określonymi stylami rozszerzonymi systemu Windows i dołącza go do obiektu `CToolBarCtrl`.|
...
|[CToolBarCtrl:: SetButtonStructSize](#setbuttonstructsize)|Określa rozmiar struktury `TBBUTTON`.|
...
Ten formant (i w związku z tym Klasa `CToolBarCtrl`) jest dostępny tylko dla programów uruchomionych w systemach Windows 95/98 i Windows NT w wersji 3,51 lub nowszej.
...
obiekty `CToolBarCtrl` zawierają kilka ważnych struktur danych wewnętrznych: listę map bitowych obrazów przycisków lub listę obrazów, listę ciągów etykiet przycisków i listę struktur `TBBUTTON`, które kojarzą obraz i/lub ciąg z pozycją, stylem, stanem i IDENTYFIKATORem polecenia przycisku. Każdy element tych struktur danych jest określany przez indeks (liczony od zera). Aby można było użyć obiektu `CToolBarCtrl`, należy skonfigurować te struktury danych. Lista ciągów może być używana tylko dla etykiet przycisków; nie można pobrać ciągów z paska narzędzi.

Aby użyć obiektu `CToolBarCtrl`, należy zwykle wykonać następujące czynności:

1. Konstruowanie obiektu `CToolBarCtrl`.

1. [Utwórz wywołanie Create](#create) , aby utworzyć formant wspólny paska narzędzi systemu Windows i dołączyć go do obiektu `CToolBarCtrl`. Wskaż styl paska narzędzi, używając stylów, takich jak TBSTYLE_TRANSPARENT przezroczystego paska narzędzi lub TBSTYLE_DROPDOWN dla paska narzędzi, który obsługuje przyciski stylu listy rozwijanej.
...
1. Jeśli chcesz, aby dla przycisku paska narzędzi w oknie właściciela nie było `CFrameWnd`, musisz obsługiwać komunikaty TTN_NEEDTEXT w oknie właściciela paska narzędzi, zgodnie z opisem w temacie [Obsługa powiadomień etykietki narzędzia](../../mfc/handling-tool-tip-notifications.md). Jeśli okno nadrzędne paska narzędzi pochodzi od `CFrameWnd`, etykietki narzędzi są wyświetlane bez jakichkolwiek dodatkowych nakładów pracy, ponieważ `CFrameWnd` udostępnia domyślną procedurę obsługi.
...
Aby uzyskać więcej informacji na temat używania `CToolBarCtrl`, zobacz [Controls](../../mfc/controls-mfc.md) and [using CToolBarCtrl](../../mfc/using-ctoolbarctrl.md).
...
Wskaźnik *lpButtons* wskazuje tablicę struktur `TBBUTTON`. Każda struktura `TBBUTTON` kojarzy dodawany przycisk z stylem przycisku, obrazem i/lub ciągiem, IDENTYFIKATORem polecenia, stanem i danymi zdefiniowanymi przez użytkownika:
...
- `idCommand`
   ...
   Identyfikator polecenia skojarzony z przyciskiem. Ten identyfikator jest wysyłany w WM_COMMAND komunikat, gdy przycisk zostanie wybrany. Jeśli element członkowski `fsStyle` ma wartość TBSTYLE_SEP, ten element członkowski musi być równy zero.
...
Nie należy przekazywać obiektu `CString` do tej funkcji, ponieważ nie można mieć więcej niż jednego znaku null w `CString`.
...
Tworzy formant Toolbar i dołącza go do obiektu `CToolBarCtrl`.
...
Należy skonstruować `CToolBarCtrl` w dwóch krokach. Najpierw Wywołaj konstruktora, a następnie Wywołaj `Create`, który tworzy formant Toolbar i dołącza go do obiektu `CToolBarCtrl`. Zastosuj następujące style okna do kontrolki paska narzędzi.
...
Aby użyć rozszerzonych stylów paska narzędzi, wywołaj metodę [Setrozszerzoną](#setextendedstyle) po wywołaniu `Create`. Aby utworzyć pasek narzędzi z stylami okna rozszerzonego, wywołaj [CToolBarCtrl:: CreateEx](#createex) , a nie `Create`.
...
Użyj `CreateEx` zamiast [tworzyć](#create) , aby zastosować rozszerzone style systemu Windows, określone przez **WS_EX_** prekroju stylu systemu Windows. `CreateEx` tworzy formant przy użyciu rozszerzonych stylów systemu Windows określonych przez *dwExStyle*. Ustawianie stylów rozszerzonych specyficznych dla kontrolki przy użyciu [setextended](#setextendedstyle). Na przykład użyj `CreateEx`, aby ustawić takie style jako WS_EX_CONTEXTHELP, ale Użyj `SetExtendedStyle` do ustawienia takich stylów jako TBSTYLE_EX_DRAWDDARROWS. Aby uzyskać więcej informacji, zobacz Style opisane na [pasku narzędzi style rozszerzone](/windows/win32/Controls/toolbar-extended-styles) w Windows SDK.

##  <a name="ctoolbarctrl"></a>CToolBarCtrl:: CToolBarCtrl

Konstruuje obiekt `CToolBarCtrl`.
...
Pobiera metryki obiektu `CToolBarCtrl`.
...
*ptbm*<br/>
Wskaźnik do struktury [TBMETRICS](/windows/win32/api/commctrl/ns-commctrl-tbmetrics) obiektu `CToolBarCtrl`.
...
Ta funkcja członkowska implementuje zachowanie [TB_GETSTRING](/windows/win32/Controls/tb-getstring)komunikatu Win32, zgodnie z opisem w Windows SDK.
...
Określa rozmiar struktury `TBBUTTON`.
...
Jeśli chcesz przechowywać dodatkowe dane w strukturze `TBBUTTON`, możesz utworzyć nową strukturę od `TBBUTTON`, dodać wymaganych członków lub utworzyć nową strukturę, która zawiera strukturę `TBBUTTON` jako jej pierwszy element członkowski. Następnie Wywołaj tę funkcję, aby poinformować pasek narzędzi o rozmiarze nowej struktury.
...
Ta funkcja członkowska implementuje zachowanie [TB_SETDISABLEDIMAGELIST](/windows/win32/Controls/tb-setdisabledimagelist)komunikatu Win32, zgodnie z opisem w Windows SDK. Implementacja MFC `SetDisabledImageList` używa obiektu `CImageList` zawierającego obrazy przycisków z wyłączonym formantem paska narzędzi, a nie uchwytu do listy obrazów.
...
Implementacja MFC `SetHotImageList` używa obiektu `CImageList` zawierającego obrazy przycisków aktywnych kontrolki paska narzędzi, a nie uchwytu do listy obrazów. Przycisk gorąca zostanie wyróżniony, gdy wskaźnik myszy znajduje się nad nim.
...
Implementacja MFC `SetImageList` używa obiektu `CImageList` zawierającego obrazy przycisków kontrolki paska narzędzi, a nie uchwytu do listy obrazów.
...
Ustawia metryki obiektu `CToolBarCtrl`.
...
*pWnd*<br/>
Wskaźnik do `CWnd` lub obiekt pochodny `CWnd`, który będzie nowym oknem właściciela dla kontrolki paska narzędzi.
...
Należy zauważyć, że funkcja członkowska `SetRows` zawsze wybiera układ, który powoduje, że jest to najmniejszy rozmiar paska narzędzi. Tworzenie paska narzędzi z stylem TBSTYLE_WRAPABLE, a następnie zmienianie rozmiarów kontrolki po prostu zastosuje metodę podaną powyżej, podając szerokość kontrolki.
...
Ta funkcja jest szczególnie przydatna, jeśli chcesz ustawić więcej niż jeden stan przycisku. Aby ustawić tylko jeden stan, użyj jednej z następujących funkcji składowych: [EnableButton](#enablebutton), [CheckButton](#checkbutton), [HideButton](#hidebutton), [unnieokreślone](#indeterminate)lub [PressButton](#pressbutton).
...
Ustawia styl wizualizacji obiektu `CToolBarCtrl`.
