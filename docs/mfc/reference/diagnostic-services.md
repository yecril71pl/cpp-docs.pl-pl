...
Te makra i funkcje są dostępne dla wszystkich klas pochodnych `CObject` w debugowaniu i wydaniu wersji MFC. Jednak wszystkie z wyjątkiem DEBUG_NEW i sprawdź, czy w wersji wydanej nic nie rób.
...
|[AfxDoForAllObjects](#afxdoforallobjects)|Wykonuje określoną funkcję dla wszystkich `CObject` obiektów pochodnych, które zostały przydzieloną przy użyciu **`new`** .|
...
## <a name="assert"></a><a name="assert"></a>STANOWCZ
...
Parametr *pObject* powinien być wskaźnikiem do obiektu i może być **`const`** . Obiekt wskazywany i Klasa musi obsługiwać `CObject` informacje klasy czasu wykonywania. Na przykład, aby upewnić się, że `pDocument` jest wskaźnikiem do obiektu `CMyDoc` klasy lub dowolnym z jego pochodnych, można kod:

[!code-cpp[NVC_MFCDocView#194](../../mfc/codesnippet/cpp/diagnostic-services_3.cpp)]

Użycie `ASSERT_KINDOF` makra jest dokładnie takie samo jak kodowanie:
...
Możesz użyć DEBUG_NEW wszędzie w programie, aby normalnie używać **`new`** operatora do alokowania magazynu sterty.
...
Po wstawieniu tej dyrektywy preprocesor wstawi DEBUG_NEW wszędzie tam, gdzie używasz **`new`** , a MFC wykonuje resztę. Podczas kompilowania wersji programu, DEBUG_NEW jest rozpoznawana jako prosta **`new`** operacja, a informacje o nazwie pliku i numerze wiersza nie są generowane.

> [!NOTE]
> W poprzednich wersjach MFC (4,1 i starszych) należy umieścić `#define` instrukcję po wszystkich instrukcjach, które wywołały makra IMPLEMENT_DYNCREATE lub IMPLEMENT_SERIAL. Nie jest to już konieczne.
...
Makro DEBUG_ONLY jest równoznaczne z otaczającym *wyrażeniem* zawierającym `#ifdef _DEBUG` i `#endif` .
...
### <a name="ensure-and-ensure_valid"></a><a name="ensure"></a>Upewnij się i ENSURE_VALID
...
## <a name="verify"></a><a name="verify"></a>SPRAWDZANIA
...
`AfxDump`wywołuje `Dump` funkcję członkowską obiektu i wysyła informacje do lokalizacji określonej przez `afxDump` zmienną. `AfxDump`jest dostępny tylko w wersji debugowania MFC.

Kod programu nie powinien być wywoływany `AfxDump` , ale powinien zamiast tego wywołać `Dump` funkcję członkowską odpowiedniego obiektu.
...
Wszystkie bloki pamięci aktualnie przydzielone do sterty są sprawdzane, łącznie z tymi przydzielonymi przez **`new`** bezpośrednie wywołania do przydziałów pamięci, takich jak funkcja **malloc** lub `GlobalAlloc` Funkcja systemu Windows. Jeśli dowolna z bloków zostanie uszkodzona, komunikat jest drukowany w danych wyjściowych debugera.

Jeśli dołączysz wiersz

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

w module programu następnie kolejne wywołania `AfxCheckMemory` pokazujące nazwę pliku i numer wiersza, do którego została przypisana pamięć.
...
- Plik IMAGEHLP.DLL musi znajdować się w ścieżce. Jeśli nie masz tej biblioteki DLL, funkcja wyświetli komunikat o błędzie. Zobacz [Biblioteka pomocy obrazów](/windows/win32/Debug/image-help-library) , aby uzyskać informacje o zestawie funkcji dostarczonym przez program Imagehlp.
...
Testuje adres pamięci, aby upewnić się, że reprezentuje aktualnie aktywny blok pamięci, który został przydzielony przez wersję diagnostyczną programu **`new`** .
...
*plRequestNumber*<br/>
Wskazuje liczbę **`long`** całkowitą, która zostanie wypełniona z numerem sekwencji alokacji bloku pamięci lub zero, jeśli nie reprezentuje aktualnie aktywnego bloku pamięci.
...
Sprawdza również określony rozmiar względem oryginalnego przyznanego rozmiaru. Jeśli funkcja zwraca wartość różną od zera, numer sekwencji alokacji jest zwracany w *plRequestNumber*. Ta liczba reprezentuje kolejność, w której został przydzielony blok względem wszystkich innych **`new`** alokacji.
...
Adres nie jest ograniczony do bloków przyznanych przez **`new`** .
...
*bObject*<br/>
Ma wartość TRUE, jeśli alokacja dotyczy `CObject` obiektu pochodnego; w przeciwnym razie wartość false.
...
Wywołuje określoną funkcję iteracji dla wszystkich `CObject` klas pochodnych, które można serializować w przestrzeni pamięci aplikacji.
...
Klasy pochodne z możliwością serializacji `CObject` są klasami pochodnymi przy użyciu makra DECLARE_SERIAL. Wskaźnik, który jest przesyłany do `AfxDoForAllClasses` w *pContext* , jest przesyłany do określonej funkcji iteracji przy każdym wywołaniu.
...
Wykonuje określoną funkcję iteracji dla wszystkich obiektów pochodnych z `CObject` , które zostały przydzieloną za pomocą **`new`** .
