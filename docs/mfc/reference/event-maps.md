...
> [!NOTE]
> Dodatkowe stałe variant zostały zdefiniowane dla wszystkich typów variant, z wyjątkiem produktów `VTS_FONT` i `VTS_PICTURE`, które zapewniają wskaźnik do stała danych variant. Stałe te są nazywane przy użyciu `VTS_Pconstantname` Konwencji. Na przykład `VTS_PCOLOR` jest wskaźnikiem do `VTS_COLOR` stałej.
