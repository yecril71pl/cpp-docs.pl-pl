...
Użyj declare_event_map — makro końca z deklaracji klasy. Następnie w pliku .cpp, który definiuje funkcji elementów członkowskich klasy, użyj begin_event_map — makro, makro wpisy dla każdego ze zdarzeń formantu i end_event_map — makro do deklarowania na końcu listy zdarzeń.
...
W pliku implementacji (.cpp), który definiuje funkcje Członkowskie dla klasy zaczynać się begin_event_map — makro, mapa zdarzeń, a następnie dodaj makro wpisy dla każdego ze zdarzeń, a następnie ukończ Mapa zdarzeń z end_event_map — makro.

Aby uzyskać więcej informacji o mapy zdarzeń i begin_event_map — makro, zobacz artykuł [formantów ActiveX: zdarzenia](../../mfc/mfc-activex-controls-events.md).
...
End_event_map — makro umożliwia Zakończ definicję mapy zdarzeń.
...
> [!NOTE]
> Stałe typu variant dodatkowe zostały zdefiniowane dla wszystkich wariantów typów, z wyjątkiem vts_font — i vts_picture —, zapewniające wskaźnik do stała danych variant. Stałe te są nazywane przy użyciu `VTS_Pconstantname` Konwencji. Na przykład VTS_PCOLOR jest wskaźnik do vts_color — stała.
...
*idsVerbName* identyfikator zasobu ciągu oznacza, że nazwa.
