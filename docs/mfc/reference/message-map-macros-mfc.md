...
W pliku implementacji (. cpp), który definiuje funkcje elementu członkowskiego dla klasy, uruchom mapę komunikatów za pomocą makra BEGIN_MESSAGE_MAP, a następnie Dodaj wpisy makr dla każdej funkcji obsługi komunikatów i wypełnij mapę wiadomości za pomocą makra END_MESSAGE_MAP.
...
> [!NOTE]
> Jeśli zadeklarujesz dowolny element członkowski po DECLARE_MESSAGE_MAP, musisz określić nowy typ dostępu ( **`public`** , **`private`** lub **`protected`** ).
...
Gdy obiekt docelowy polecenia odbiera komunikat WM_COMMAND systemu Windows o określonym IDENTYFIKATORze, ON_COMMAND wywoła funkcję członkowską, `memberFxn` Aby obsłużyć komunikat.
...
Kieruje polecenia za pomocą interfejsu wysyłania poleceń `IOleCommandTarget` .
...
Funkcja systemu Windows `RegisterWindowMessage` służy do definiowania nowego komunikatu okna, który ma być unikatowy w całym systemie.
