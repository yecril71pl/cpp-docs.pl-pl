...
2. `RawDllMain` najpierw jest wywoływana. Ustawia stan modułu do stanu znane statycznych modułu dla biblioteki DLL. Z tego powodu `RawDllMain` połączone statycznie do pliku DLL.
...
Afx_module_state — makro nie jest używany będzie powodować problemy z zasobami w bibliotekach DLL. Domyślnie MFC używa zasobów dojście aplikacji głównej do załadowania szablonu zasobów. Ten szablon faktycznie jest przechowywany w bibliotece DLL. Główną przyczyną jest informacji o stanie modułu MFC nie został przełączony przez afx_module_state — makro. Dojście do zasobu jest odzyskiwana ze stanu modułu MFC. Nie przełączający stan modułu powoduje, że dojście niewłaściwego zasobu do użycia.
