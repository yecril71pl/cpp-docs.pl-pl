...
Program `CTaskDialog` obsługuje kilka opcjonalnych elementów, które umożliwiają zbieranie i wyświetlanie informacji. Na przykład `CTaskDialog` można wyświetlić linki poleceń, niestandardowe przyciski, dostosowane ikony i stopkę. Ma `CTaskDialog` także kilka metod, które umożliwiają badanie stanu okna dialogowego zadania w celu określenia elementów opcjonalnych wybranych przez użytkownika.
...
Poniższa procedura przedstawia najbardziej podstawowe użycie `CTaskDialog`programu, który polega na zastępowaniu okna komunikatu systemu Windows. Ten przykład zmienia również ikonę skojarzoną z oknem dialogowym zadania. Zmiana ikony powoduje, `CTaskDialog` że pojawia się ona tak samo jak w przypadku okna komunikatu systemu Windows.
...
1. Użyj **Kreatora aplikacji MFC** , aby utworzyć aplikację MFC ze wszystkimi ustawieniami domyślnymi. Zobacz [Przewodnik: Aby uzyskać instrukcje dotyczące sposobu otwierania](walkthrough-using-the-new-mfc-shell-controls.md) kreatora dla używanej wersji programu Visual Studio, użyj nowych formantów powłoki MFC.
...
Możesz uniknąć kroku 7, jeśli nie chcesz, `CTaskDialog` aby wyświetlać tę samą ikonę co okno komunikatu systemu Windows. W `CTaskDialog` przypadku uniknięcia tego kroku nie ma ikony, gdy aplikacja zostanie wyświetlona.
...
1. `taskDialog.DoModal()` Znajdź instrukcję i Zastąp tę instrukcję poniższym kodem. Ten kod aktualizuje okno dialogowe zadania i dodaje nowe kontrolki:
...
W kodzie w kroku 9 Zastąp komentarze, które zaczynają `PROCESS IF` się od kodu, który ma zostać wykonany w określonych warunkach.
...
Poniższa procedura pokazuje, jak wyświetlić a `CTaskDialog` bez wcześniejszego `CTaskDialog` utworzenia obiektu. Ten przykład kontynuuje poprzednią procedurę.
...
1. Przejdź do nawiasu `if (CTaskDialog::IsSupported())` zamykającego instrukcji.

1. Wstaw poniższy kod bezpośrednio przed nawiasem `if` zamykającym instrukcji ( `else` przed blokiem):
