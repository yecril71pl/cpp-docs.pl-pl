...
Począwszy od C ++ 17 operandów operatory -> *, [], >>, i \< \< muszą być obliczane w kolejności od lewej do prawej. Istnieją dwa przypadki, w których kompilator nie może zagwarantować to zamówienie:
...
Kompilator generuje ostrzeżenie [C4866](https://docs.microsoft.com/cpp/error-messages/compiler-warnings/c4866?view=vs-2017) kiedy nie może zagwarantować oceny od lewej do prawej. To ostrzeżenie jest generowany tylko wtedy, gdy **/STD: c ++ 17** lub nowszej jest określony, wymaganie kolejności od lewej do prawej te operatory wprowadzono w języku C ++ 17.

Aby rozwiązać tego ostrzeżenia, najpierw należy wziąć pod uwagę czy oceny argumentów od lewej do prawej jest to konieczne, takich jak podczas oceny operandów może powodować generowanie efekty uboczne zależnych od kolejności. W wielu przypadkach kolejność, w której argumenty operacji są obliczane nie ma efektu zauważalne. Jeśli kolejność obliczania musi mieć od lewej do prawej, należy rozważyć, czy można przekazywać argumentów przez odniesienie const zamiast tego. Ta zmiana eliminuje ostrzeżenie w następującym przykładzie kodu.
...
## <a name="bug-fixes-in-visual-studio-versions-150-153update153-155update155-157update157-158update158-and-159update159"></a>Poprawki błędów w wersjach programu Visual Studio 15.0, [15.3](#update_153), [15.5](#update_155), [15.7](#update_157), [15.8](#update_158), i [15.9](#update_159)
...
Elementy członkowskie klasy są inicjowane w kolejności, w której są deklarowane, nie kolejności, w której są wyświetlane na listach inicjatora. Poprzednie wersje kompilatora nie ostrzega, gdy kolejność na liście inicjatora różnił się od kolejności deklaracji. Może to prowadzić do niezdefiniowanego zachowania, jeśli Inicjalizacja jednego członka zależała od innego członka na liście, który został już zainicjowany. W poniższym przykładzie Visual Studio 2017 w wersji 15.3 (przy użyciu **/Wall**) zgłasza "ostrzeżenie C5038: element członkowski danych"A::y"zostanie zainicjowany po"A::x"element członkowski danych":
...
**Uwaga:** W bibliotece python Boost wprowadzono przez długi czas obejście MSVC specyficzne dla szablonu deklaracji do przodu w [unwind_type.hpp](https://github.com/boostorg/python/blame/develop/include/boost/python/detail/unwind_type.hpp). W obszarze [/ permissive-](../../build/reference/permissive-standards-conformance.md) trybu, począwszy od programu Visual Studio 2017 w wersji 15.8 (_MSC_VER = 1915), za pomocą kompilatora MSVC jest poprawnie wyszukiwania do nazwy zależnej od argumentu (ADL) i jest spójne za pomocą innych kompilatorów, dzięki czemu ochrona tego rozwiązania niepotrzebne. Aby uniknąć tego błędu *C3861: "unwind_type": nie odnaleziono identyfikatora*, zobacz [229 żądania Ściągnięcia](https://github.com/boostorg/python/pull/229) w repozytorium Boostorg można zaktualizować pliku nagłówka. Firma Microsoft ma już zastosować poprawki względem jakiegokolwiek [vcpkg](../../build/vcpkg.md) Boost pakietu, więc jeśli pobieranie lub uaktualnienie źródła Boost z vcpkg nie należy zastosować poprawkę oddzielnie.
...
- po stronie zużycie (main.cpp) opcja **/ehsc** nie zostanie określony.
- wersja języka C++ jest **/STD: c ++ 17** na stronie tworzenia i **/STD: c ++ 14** stronie zużycia. 
...
Częściowe reguły sortowania obejmujące aliasów w kontekstach — ustalona jest implementacja rozbieżności. W poniższym przykładzie, GCC i kompilator C++ firmy Microsoft (w **/ permissive-** tryb) zgłosić błąd, podczas gdy Clang akceptuje kodu. 
...
Rozbieżności implementacja jest wynika z regresji w standardowych treść, gdzie rozwiązanie do kluczową kwestię 2235 usunięte jakiś tekst, które pozwoliłoby te przeciążenia do zamówienia. Bieżący standard C++ nie zapewnia mechanizm częściowo kolejność tych funkcji, dzięki czemu są one traktowane jako niejednoznaczny.

Jako obejście zaleca się, że nie zależą od częściowe porządkowanie, aby rozwiązać ten problem, a zamiast tego użyj SFINAE, aby usunąć określonego przeciążenia. W poniższym przykładzie użyto klasę pomocy `IsA` do usunięcia pierwszego przeciążenia, kiedy `Alloc` jest specjalizacją `A`:
...
Niedozwolony wyrażeń i typy literału teraz są poprawnie zdiagnozować w definicji funkcji oparte na szablonach, które są jawnie specjalizowany. Takie błędy nie zostały wcześniej wydane dla definicji funkcji. Jednak niedozwolone wyrażenie lub typem literału będzie nadal mieć rozpoznano Jeśli ocenione jako część wyrażeniem stałym. 
...
W programie Visual Studio 2017 w wersji 15.9 kod generuje ten błąd: *błąd C3615: funkcji constexpr<int>:: f "nie może być wyrażenie stałe. Uwaga: niepowodzenie zostało spowodowane przez wywołanie niezdefiniowanej funkcji lub niezadeklarowanej jako jeden"constexpr" Uwaga: Zobacz użycie "g"*. Aby uniknąć tego błędu, Usuń `constexpr` kwalifikator z jawnym utworzeniem wystąpienia f() funkcji. 
