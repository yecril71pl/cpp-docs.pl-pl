...
W tym przykładzie pokazano, jak obsługiwać wyjątki w `parallel` regionie OpenMP i w wywołaniu `parallel_for` . `do_work`Funkcja wykonuje żądanie alokacji pamięci, które nie powiedzie się i w związku z tym zgłasza wyjątek typu [std:: bad_alloc](../../standard-library/bad-alloc-class.md). W wersji korzystającej ze OpenMP należy również przechwycić wątek, który zgłasza wyjątek. Innymi słowy, każda iteracja pętli równoległej OpenMP musi obsłużyć wyjątek. W wersji, która używa środowisko uruchomieniowe współbieżności, główny wątek przechwytuje wyjątek, który jest generowany przez inny wątek.
