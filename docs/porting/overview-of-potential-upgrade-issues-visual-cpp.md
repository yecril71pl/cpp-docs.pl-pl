...
Jeśli masz plik obiektu, który zawiera zewnętrzne symbole z C++ powiązania, który obiekt pliku nie może poprawnie łącza z plikami obiektu z różnych wersji głównej zestawu narzędzi programu. Należy zauważyć, że w tym miejscu "może nie działać" ma wiele możliwych wyników: link może zakończyć się niepowodzeniem całkowicie (na przykład, jeśli zmienione dekorowania nazwy), może się powieść, link i czynności może nie działać w czasie wykonywania (na przykład zmienić układ typu), lub zdarzyć, że elementy działają w wielu przypadkach i nie element będzie pójdzie nie tak. Należy zauważyć, że podczas interfejsie ABI języka C++ nie jest stabilna C ABI i podzbiór interfejsie ABI języka C++ wymagane dla modelu COM są stabilne.

Jeśli łączysz się do biblioteki importu żadnych nowszej wersji programu Visual Studio redystrybucyjnych bibliotek, które zachowania zgodności z interfejsem ABI może służyć w czasie wykonywania. Na przykład jeśli Twoja aplikacja jest kompilowana i połączone, przy użyciu zestawu narzędzi Visual Studio 2015 Update 3, umożliwia dowolnego programu Visual Studio 2017 lub Visual Studio 2019 redystrybucji, ponieważ biblioteki w wersji 2015 i 2017 zostały zachowane binarne zgodności z poprzednimi wersjami. Odwrotna sytuacja nie jest PRAWDA. Nie można użyć pakiet redystrybucyjny dla starszej wersji zestawu narzędzi programu niż używane do kompilowania swojego kodu, nawet jeśli mają niezgodne interfejsu ABI.
...
Jeśli błąd dotyczy formatu argumentów ciągów, to prawdopodobnie ponieważ kompilator jest bardziej rygorystyczne o wymuszanie standard. Aby uzyskać więcej informacji zobacz historię zmian. Zapłać szczególną uwagę na wszelkie błędy, w tym miejscu, ponieważ mogą one reprezentować potencjalnie zagrożenie bezpieczeństwa.
