...
```  
 ...
 Najprostszym sposobem na wyszukiwanie symboli w plikach dołączonych dostępnych jest za pomocą narzędzia Znajdź w plikach (Ctrl + Shift + F) i określ **Visual C++ Dołącz katalogi**. Firma Microsoft uznała, że w ntverp.h. Zastąpiliśmy verstamp.h dołączone ntverp.h i zniknął tego błędu.  
  
##  <a name="linker_output_settings"></a> Krok 3. Ustawienie Plik_wyjściowy konsolidatora  
 Starsze projekty mają czasami pliki umieszczone w niekonwencjonalne lokalizacje, które mogą być przyczyną problemów po uaktualnieniu. W tym przypadku mamy do dodania $ (solutiondir) do ścieżki dołączania we właściwościach projektu, aby upewnić się, że program Visual Studio można znaleźć pliki nagłówkowe, które są umieszczane w tym folderze, a nie w jednym z folderów projektu.  
  
 Program MSBuild narzeka, że właściwość Link.OutputFile nie pasuje do wartości TargetPath i TargetName wystawianie MSB8012.  
 ...
 **Link.OutputFile** znajdują się dane wyjściowe kompilacji (EXE, DLL, na przykład) i zwykle jest zbudowany z $(TargetDir)$(TargetName)$(TargetExt), podając ścieżkę, nazwa_pliku i rozszerzenia. To jest typowym błędem podczas migrowania projektów ze starego Visual C++ kompilacji narzędzia (vcbuild.exe) nowe narzędzia do kompilowania (MSBuild.exe). Ponieważ zmiana narzędzia kompilacji została wprowadzona w programie Visual Studio 2010, może wystąpić ten problem, zawsze wtedy, gdy migracja projektu pre-2010 do 2010 lub nowszej wersji. Podstawowego problemu jest, że Kreator migracji projektu nie jest aktualizowany **Link.OutputFile** wartości, ponieważ nie zawsze jest możliwe ustalenie, co jej wartość powinna być zgodnie z ustawieniami projektu. W związku z tym zazwyczaj należy ręcznie ustawić. Aby uzyskać więcej informacji, zobacz ten [wpis](http://blogs.msdn.com/b/vcblog/archive/2010/03/02/visual-studio-2010-c-project-upgrade-guide.aspx) na blogu Visual C++.  
 ...
 W tym przypadku **Link.OutputFile** ustawiono właściwość w projekcie przekonwertowanego.\Debug\Spyxx.exe i.\Release\Spyxx.exe dla Spy ++ projektu, w zależności od konfiguracji. Najlepiej aby po prostu zastąpić te wartości zapisane na stałe $(TargetDir)$(TargetName)$(TargetExt) dla wszystkich konfiguracji. Jeśli to nie zadziała, można dostosować w tym miejscu lub zmienić właściwości w sekcji Ogólne, w którym te wartości są ustawiane (właściwości są **katalog wyjściowy**, **Nazwa docelowego**, i  **Rozszerzenie docelowe**. Należy pamiętać, że jeśli właściwość wyświetlasz używa makra, możesz wybrać **Edytuj** na liście rozwijanej, aby wyświetlić okno dialogowe, które wyświetla ostatni ciąg podstawienia — makro wprowadzone. Wszystkich dostępnych makr oraz ich bieżących wartości można wyświetlić, wybierając **makra** przycisku.  
...
```  
 ...
 Aby pozbyć się błąd, należy zdefiniować WINVER, aktualizując **właściwości projektu** ustawienie Najniższa wersja systemu Windows, które obecnie ma pod kątem. Znajdź tabelę wartości dla różnych wersji Windows [tutaj](http://msdn.microsoft.com/library/windows/desktop/aa383745.aspx).  
...
```  
 ...
 Punkt wejścia dla biblioteki DLL powinny nie można wyeksportować. Punkt wejścia jest przeznaczona tylko do wywoływana przez moduł ładujący, gdy biblioteki DLL, najpierw jest ładowany do pamięci, dlatego nie powinny być w tabeli eksportu dla innych klientów. Musimy upewnić się, że nie ma `__declspec(dllexport)` dyrektywy dołączono do niego. Spyxxhk.c firma Microsoft musi usunąć ją z dwóch miejsc: deklaracja i definicja DLLEntryPoint. Nigdy nie przeprowadza warto użyć tej dyrektywy, ale poprzednie wersje kompilatora i konsolidatora nie Flaga go jako problem. Nowsze wersje konsolidator ostrzeżenie.  
...
```  
 ...
 Dzięki tej zmianie mamy problemy z ostrstream —, który nie jest już używana. Odpowiednie zastąpienia jest ostringstream. Staramy się, dodając element typedef dla ostrstream — uniknąć modyfikacji kodu zbyt dużo przynajmniej rozpoczęcia.  
...
```  
 ...
 Obecnie projekt jest kompilowany przy użyciu MBCS (wielobajtowego zestawu znaków), więc `char` jest odpowiednich danych znakowych. Jednakże, aby umożliwić łatwiejsze aktualizacji kodu Unicode UTF-16, aktualizujemy ten element, aby `TCHAR`, która jest rozpoznawana jako `char` lub `wchar_t` zależności od tego, czy **zestaw znaków** w ustawieniach projektu jest właściwością MBCS lub Unicode.  
  
 Kilka fragmentów kodu, muszą zostać zaktualizowane.  Zastąpiliśmy klasy bazowej dla systemu ios przy użyciu ios_base — i zastąpiliśmy ostream polega na basic_ostream\<T >. Możemy dodać dwie dodatkowe definicje typów, a następnie kompiluje w tej sekcji.  
...
```  
 ...
 Następny problem jest tym basic_stringbuf — nie ma metody blokowanie. Metoda blokowanie jest używana do zapobiegania przeciek pamięci w starym ostream. Teraz, gdy firma Microsoft korzysta z nowych ostringstream nie potrzebujemy jej. Firma Microsoft może usunąć wywołania, można zablokować.  
  
```cpp  
//rdbuf()->freeze(0);  
```  
  
 Następne dwa wystąpiły błędy w wierszach sąsiadująco. Pierwszy narzeka o używaniu kończy się, czyli manipulator we/wy stare biblioteką iostream, który dodaje terminator o wartości null na ciąg.  Drugi z tych błędów wyjaśniono, że dane wyjściowe str — metoda nie można przypisać do wskaźnika niebędącego stałą.  
...
```  
 ...
 Za pomocą nowej biblioteki strumienia, kończy się nie jest konieczna, ponieważ ciąg jest zawsze zakończony znakiem null, dzięki czemu można usunąć wiersza. Dla drugiego problemu problemu jest to, że teraz str() nie zwracają wskaźnik do tablicy znaków ciągu; Zwraca typ std::string. Rozwiązanie do drugiego jest Zmień typ na LPCSTR i zażądać wskaźnik przy użyciu metody c_str().  
...
```  
 ...
 Makro `MOUT` jest rozpoznawana jako \*g_pmout, który jest obiektem typu `mstream`. Klasa mstream pochodzi od klasy ciąg standardowego wyjścia, `std::basic_ostream<TCHAR>.` jednak za pomocą _T wokół literału ciągu, na którym umieściliśmy w ramach przygotowania do konwersji na format Unicode, Rozpoznanie przeciążenia operatora << kończy się niepowodzeniem z powodu następującego błędu Komunikat:  
...
```  
 ...
 Przechodzenie do definicji to makro, widzimy, że odwołuje się funkcja OnNcHitTest.  
...
```  
 ...
 Ponieważ około dziesięciu wystąpień tej funkcji w różnych klas pochodzące od CWnd, warto użyć **przejdź do definicji** (klawiatura: F12) i **przejdź do deklaracji** (klawiatura: Ctrl + F12) po kursor znajduje się na funkcji w edytorze, aby zlokalizować i przechodzić do nich z **Znajdź Symbol** okna narzędzi. **Przejdź do definicji** jest zazwyczaj bardziej użyteczne dwóch. **Przejdź do deklaracji** będzie deklaracji Znajdź inne niż Definiowanie klasy deklaracji, takich jak przyjazne deklaracje klas lub przekazywać odwołania.  
...
```  
 ...
 Problem polega na tym, że drugi parametr CWnd::OnActivateApp zmieniła się z HTASK do typu DWORD. Ta zmiana została wprowadzona w wersji programu Visual Studio, Visual Studio .NET 2002.  
...
```  
 ...
 Następne ostrzeżenie wymaga niektóre wersje kodu. GetVersion — interfejs API systemu Win32 (i GetVersionEx) jest przestarzały.  
...
```  
 ...
 Istnieją metody w klasie CSpyApp, które tworzą zapytania dotyczące wersji systemu operacyjnego: IsWindows9x, IsWindows4x i IsWindows5x. Dobry punkt wyjścia jest założenie, że wersje systemu Windows, które firma Microsoft zamierza obsługiwać (Windows 7 i nowsze) są wszystkie Zamknij, aby zainteresowanym 5 Windows NT, jak daleko technologie, które są używane w tej starszej aplikacji. Korzysta z tych metod były do czynienia z ograniczeniami starszych systemów operacyjnych. Dlatego zmieniliśmy tych metod zwraca wartość TRUE dla IsWindows5x i FALSE dla pozostałych.  
...
```  
 ...
 Deklaracja m_bStdMouse wskazuje bitfield.  
...
```  
 ...
 Ten kod został napisany przed typu logicznego wbudowanych była obsługiwana w programie Visual C++. Taki kod BOOL został element typedef dla int. Typ int jest typ ze znakiem i bitowa reprezentacja podpisem int to użycie pierwszego bitu jako bitu znaku, więc bitfield typu int może zostać zinterpretowane jako 0 albo -1, co prawdopodobnie nie był przeznaczony.  
 ...
 Być może zastanawiasz się, jeśli za pomocą bool standardowego typu w całym mogą być przydatne. Wiele stare wzorców kodu, takie jak typ BOOL zostały utworzoną do rozwiązywania problemów, które zostały później można rozwiązać w standardzie języka C++, więc zmiana bool wbudowany typ bool jest tylko jeden przykład takiej zmiany, rozważ, wykonując po uzyskaniu działanie kodu początkowego w nowej wersji.  
...
```  
 ...
 Zajmuje to nieco wystarczająco nieszkodliwe, ale ponieważ Chcieliśmy czysta kompilacja z zestawem/W4 i /WX, możemy po prostu komentarzami nazwę zmiennej równoczesnym pozostawieniu ich dla czytelności.  
  
```cpp  
virtual void OnSelectTab(int /*nTab*/) {};  
```  
  
 Inne ostrzeżenia, które odebraliśmy były przydatne w przypadku ogólnych kod czyszczenia. Istnieje wiele niejawne konwersje z elementu `int` lub `unsigned int` do `WORD` (czyli element typedef dla `unsigned short`). Obejmują one możliwa utrata danych. Dodaliśmy rzutowania `WORD` w tych przypadkach.  
...
```  
 ...
 LNK1181 występuje, ponieważ nieaktualne biblioteki statycznej wersji biblioteki mfc znajduje się na konsolidatora danych wejściowych. Nie jest to wymagane dłużej, ponieważ firma Microsoft dynamicznie łączy biblioteki MFC, potrzebujemy jeszcze tylko do usuwania wszystkich bibliotek statycznych MFC właściwości danych wejściowych w sekcji konsolidatora we właściwościach projektu. Ten projekt jest również za pomocą opcji/nodefaultlib, a zamiast tego zawiera listę wszystkich zależności biblioteki.  
...
```  
 ...
 Niektóre miejsca w kodzie były już używane `TCHAR`, prawdopodobnie w oczekiwaniu ostatecznie obsługi standardu Unicode. Niektóre nie były dostępne. Przeszukaliśmy dla wystąpienia elementu `CHAR`, czyli element typedef dla char i zastąpione większość z nich przy użyciu TCHAR. Ponadto firma Microsoft wyszukiwanego `sizeof (CHAR)`. Zawsze, gdy firma Microsoft zmieniła się z `CHAR` do `TCHAR`, mieliśmy zazwyczaj można zmienić na `sizeof(TCHAR)` ponieważ był to często używane do określenia liczby znaków w ciągu. W tym miejscu przy użyciu nieprawidłowego typu nie generuje błąd kompilatora, aby poświęcić nieco uwagi w tym przypadku zwracając.  
...
```  
 ...
 Mimo że _tcscpy — funkcja została użyta, czyli TCHAR strcpy — funkcja kopiowania ciągu, został buforu, który został przydzielony `char` buforu. Jest to prosty sposób zmienić na TCHAR.  
...
```  
 ...
 W wielu przypadkach musimy użyć wersji ogólnego interfejsu API Win32, takich jak GetClassName (zamiast GetClassNameA) (makro). W instrukcji switch procedury obsługi komunikatów, niektóre komunikaty o MBCS lub Unicode dla klas w tych przypadkach, musimy Zmień kod, aby jawnie wywołać MBCS wersji, ponieważ firma Microsoft zastąpione ogólną o nazwie funkcji a i W określonych funkcji i dodaje makro nazwa ogólna, który jest rozpoznawany jako prawidłowy element lub nazwę W oparciu o czy UNICODE jest zdefiniowana.  W wielu częściach kodu gdy przeszliśmy do definiowania _UNICODE, wersji W teraz wybierany jest nawet gdy A wersja jest co chcę teraz.  
  
 Istnieje kilka miejsc, gdy miały stosowanie specjalnych działań do wykonania. Każde użycie Procedura WideCharToMultiByte lub MultiByteToWideChar może wymagać bliżej. Poniżej przedstawiono przykładowy gdzie Procedura WideCharToMultiByte było ono używane.  
...
```  
 ...
 Aby rozwiązać ten problem, musimy zrozumieć przyczyny, dla której zostało to zrobione było skopiować szeroki ciąg znaków reprezentujący nazwę czcionki do wewnętrznego buforu CString strFace. Nieco inny kod to wymagane dla wielobajtowego ciągów cstring — jak w przypadku ciągów CString znaków dwubajtowych, więc w takim przypadku dodaliśmy #ifdef.  
...
```  
 ...
 Oczywiście zamiast wcscpy — naprawdę powinniśmy skorzystać w wcscpy_s —, bardziej bezpieczna wersja. Następna sekcja rozwiązuje ten problem.  
  
 W celu sprawdzenia naszej pracy firma Microsoft resetowania zestaw znaków na zestaw znaków wielobajtowych użycia i upewnij się, że kod nadal będzie się kompilować przy użyciu MBCS, jak również Unicode. Needless, że przebieg pełnego testu powinien być wykonywany ponownej kompilacji aplikacji po tych zmian.  
...
##  <a name="porting_to_secure_crt"></a> Krok 12. Przenoszenie do użycia zabezpieczenia CRT  
 Przenoszenie kodu do użycia bezpieczne wersje funkcji CRT (wersje z sufiksem _s) jest dalej. W tym przypadku ogólną strategią jest zastąpić wersji _s funkcję, a następnie, zazwyczaj, Dodaj parametry rozmiar wymaganego buforu dodatkowe. W wielu przypadkach to proste, ponieważ rozmiar jest znany. W innych przypadkach, w których rozmiar nie jest natychmiast dostępna, jest konieczne dodanie dodatkowych parametrów do funkcji, która używa funkcji CRT lub prawdopodobnie zużycie buforu miejsca docelowego i zobacz jakie odpowiedniego rozmiaru obowiązują limity.  
...
```  
 ...
 Po zdefiniowaniu, zawsze wtedy, gdy rozmiar buforu jest tablicą, a nie wskaźnik surowy jego rozmiar jest wnioskowany z typu tablicy i używany jako parametr rozmiaru bez konieczności dostarczenia go. Która pozwala zmniejszyć złożoność ponowne napisanie kodu. Nadal trzeba zastąpić wersją _s nazwy funkcji, ale często może odbywać się za pomocą wyszukiwania i operację zamiany.  
  
 Zmienić wartości zwrócone przez niektóre funkcje. Na przykład _itoa_s — (i _itow_s — i _itot_s — makro) zwraca kod błędu (errno_t) zamiast ciągu. Dlatego w tych przypadkach należy przenieść wywołanie _itoa_s — na osobnym wierszu i zastąp go z identyfikatorem buforu.  
