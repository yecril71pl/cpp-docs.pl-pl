...
- Wyrażenie "każdego *N* w zakresie [*A*, *B*)" oznacza, że *N* zaczyna się od wartości *A* i zwiększany zero lub więcej razy do momentu jej jest równa wartości *B*. Przypadek *N* == *B* nie znajduje się w zakresie.
...
Algorytmy kilka użyć predykatu, który musi powodować ścisłe słabe porządkowanie w pary elementów z sekwencji. Dla predykatu `pr`(*X*, *Y*):

- Strict oznacza, że `pr`(*X*, *X*) ma wartość false.

- Weak oznacza, że *X* i *Y* mieć równoważne if szeregowania!`pr` (*X*, *Y*) & &!`pr` (*Y*, *X*) (*X* == *Y* nie musi być zdefiniowana).

- Kolejność oznacza, że `pr`(*X*, *Y*) & & `pr`(*Y*, Z) oznacza `pr`(*X*, Z).
