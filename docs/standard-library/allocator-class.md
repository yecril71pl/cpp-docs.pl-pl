 ...
 *Typ*  
 Typ obiektu, dla którego ma zostać magazynu przydzielone lub alokację.  
...
|||  
|-|-|  
|[Program przydzielania](#allocator)|Konstruktory użyty do utworzenia `allocator` obiektów.|  
...
|[wskaźnik](#pointer)|Typ, który dostarcza wskaźnik do typu obiektu zarządzanego przez program przydzielania.|  
 ...
 *_Hint*  
 Const wskaźnik, który może pomóc obiektu alokatora spełnić żądania magazynu dzięki umieszczeniu adres obiektu przydzielone przed żądania.  
...
### <a name="parameters"></a>Parametry  
 `ptr`  
 Wskaźnik do lokalizacji, w którym ma być skonstruowany obiekt.  
  
 `val`  
 Wartość, z którym tworzona jest zainicjowane.  
...
### <a name="parameters"></a>Parametry  
 `ptr`  
 Wskaźnik do pierwszego obiektu do cofnięcia alokacji z magazynu.  
  
 `count`  
 Liczba obiektów do cofnięcia alokacji z magazynu.  
...
### <a name="parameters"></a>Parametry  
 `ptr`  
 Wskaźnik wyznaczenie adres obiektu, które mają zostać zniszczone.  
...
### <a name="parameters"></a>Parametry  
 `right`  
 Obiekt alokatora ma być przypisany do innego takiego obiektu.  
