...
|[synchronizacji](#sync)|Synchronizuje urządzenie wejściowe skojarzone z strumienia z buforu strumienia.|  
...
### <a name="parameters"></a>Parametry  
 ...
 `right`  
 A `basic_istream` obiektu do skopiowania.  
...
### <a name="parameters"></a>Parametry  
 ...
 `Delim`  
 Znak, który należy zakończyć odczytu, jeśli okaże się przed `count`.  
  
 `str`  
 Ciąg, w którym można zapisać.  
  
 `Ch`  
 Znak do pobrania.  
  
 `strbuf`  
 Bufor w jakim zostanie zapisane.  
...
### <a name="parameters"></a>Parametry  
 ...
 `Delim`  
 Element, jeśli przed count, powoduje, że **Ignoruj** do zwrócenia, dzięki czemu wszystkie elementy po `Delim` do odczytu.  
...
### <a name="parameters"></a>Parametry  
 `str`  
 Tablica, w którym `readsome` przechowuje znaki tekstu.  
...
### <a name="parameters"></a>Parametry  
 `pos`  
 Położenie bezwzględne, w którym można przenieść wskaźnik odczytu.  
  
 `off`  
 Przesunięcie do odczytu wskaźnika względem `way`.  
...
{sentry — klasa  
   publiczny:  
   jawne sentry (basic_istream —\<elementu, Tr > & _Istr,  
   wartość logiczna _Noskip = false); Operator bool() const; };  
...
### <a name="parameters"></a>Parametry  
 `right`  
 Odwołania do wartości do `basic_istream` obiektu.  
