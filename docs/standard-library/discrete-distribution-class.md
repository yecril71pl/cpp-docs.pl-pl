...
### <a name="parameters"></a>Parametry  
*firstW*  
 Iterator pierwszy na liście, z którego można utworzyć dystrybucji.  
  
*lastW*  
 Ostatni iteratora na liście, z którego można utworzyć dystrybucji (z systemem innym niż inclusive ponieważ Iteratory użyj pustego elementu dla elementu end).  
  
*weightlist*  
 [Initializer_list](../cpp/initializers.md) z którego można utworzyć dystrybucji.  
...
-  Jeśli *liczba* > 0,  **n**   =  *liczba*. Podany **d** = (*wysokiej* - *niski*) /  **n**  jest większa niż zero, przy użyciu **d** uniform podzakresów każdego wagi jest przypisana w następujący sposób: `weight[k] = weightfunc(x)`, gdzie **x** = *niski* + **k**  *  **d** + **d** / 2 do **k** = 0,...,  **n**  - 1.  
