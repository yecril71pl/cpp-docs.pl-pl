...
||||  
|-|-|-|  
|[bezwzględne](#absolute)|[Rozpocznij](#begin)|[Canonical](#canonical)|
...
|[koniec](#end)|[Odpowiednik](#equivalent)|[istnieje](#exists)|
...
2.  Jeśli pval.has_root_name() & &! pval.has_root_directory() funkcja zwraca pval.root_name() / absolute(base).root_directory() / absolute(base).relative_path() / pval.relative_path().  
  
3.  Jeśli! pval.has_root_name() & & pval.has_root_directory() funkcja zwraca absolute(base).root_name() / pval.  
  
4.  Jeśli! pval.has_root_name() & &! pval.has_root_directory() funkcja zwraca absolute(base) / pval.  
...
2.  Każdy składnik ścieżki. (kropka jest bieżącym katalogiem wyznaczane przez poprzednie składniki ścieżki) zostaną usunięte.  
  
3.  Wszystkie pary składników ścieżki X /... (kropka kropka jest wyznaczane przez poprzednie składniki ścieżka katalogu nadrzędnego) zostaną usunięte.  
...
-   Jeśli zdecyduje & (copy_options::create_symlinks &#124; copy_options::skip_symlinks), przez wywołanie metody symlink_status —  
  
-   w przeciwnym razie, wywołując stanu  
  
-   W przeciwnym razie Zgłoś błąd.  
  
 Jeśli! exists(f) &#124; &#124; odpowiednik (f, t) &#124; &#124; is_other(f) &#124; &#124; is_other(t) &#124; &#124; is_directory(f) & & is_regular_file(t), następnie zgłaszały błąd (i nic nie rób else).  
