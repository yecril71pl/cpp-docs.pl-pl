...
Klasa kontenera hash_multiset jest rozszerzeniem standardowej biblioteki języka C++ i służy do przechowywania oraz szybkiego pobierania danych z kolekcji, w której wartości elementów są przechowywane jako wartości klucza i nie muszą być unikatowe.
...
|Nazwa typu|Opis|
|-|-|
|[allocator_type](#allocator_type)|Typ, który reprezentuje `allocator` klasę dla `hash_multiset` obiektu.|
|[const_iterator](#const_iterator)|Typ, który dostarcza iterator dwukierunkowy, który może odczytać **`const`** element w `hash_multiset` .|
|[const_pointer](#const_pointer)|Typ, który dostarcza wskaźnik do **`const`** elementu w `hash_multiset` .|
|[const_reference](#const_reference)|Typ, który zawiera odwołanie do **`const`** elementu przechowywanego w trakcie `hash_multiset` operacji odczytywania i wykonywania **`const`** .|
|[const_reverse_iterator](#const_reverse_iterator)|Typ, który dostarcza iterator dwukierunkowy, który może odczytać dowolny **`const`** element w `hash_multiset` .|
|[difference_type](#difference_type)|Typ liczby całkowitej ze znakiem, który zapewnia różnicę między dwoma iteratorami, które są w tym samym zakresie elementami `hash_multiset` .|
...
|[reverse_iterator](#reverse_iterator)|Typ, który dostarcza iterator dwukierunkowy, który może odczytać lub zmodyfikować element w odwróconej postaci `hash_multiset` .|
|[size_type](#size_type)|Typ liczby całkowitej bez znaku, który może reprezentować liczbę elementów w `hash_multiset` .|
|[value_compare](#value_compare)|Typ, który dostarcza dwa obiekty funkcji, Predykat binarny klasy Compare, który może porównać dwie wartości elementów w, `hash_multiset` Aby określić ich względną kolejność i predykat jednoargumentowy, który miesza elementy.|
...
|[crend](#crend)|Zwraca iterator const, który odnosi się do lokalizacji po ostatnim elemencie w odwróconym stanie `hash_multiset` .|
...
|[puste](#empty)|Testuje, czy `hash_multiset` jest pusty.|
...
|[lower_bound](#lower_bound)|Zwraca iterator do pierwszego elementu w elemencie `hash_multiset` z kluczem, który jest równy lub większy niż określony klucz.|
...
|[value_comp](#value_comp)|Pobiera kopię obiektu cechy skrótu służącego do mieszania i wartości klucza elementu w elemencie `hash_multiset` .|
...
Zobacz przykład dla [get_allocator](#get_allocator) na przykład za pomocą polecenia`allocator_type`
...
Jeśli wartość zwracana `begin` jest przypisana do `const_iterator` , elementy w obiekcie hash_multiset nie mogą być modyfikowane. Jeśli wartość zwracana `begin` jest przypisana do `iterator` , można modyfikować elementy w obiekcie hash_multiset.
...
Z wartością zwracaną `cbegin` `hash_multiset` nie można modyfikować elementów w obiekcie.
...
Stały iterator dwukierunkowy, który odnosi się do lokalizacji po ostatnim elemencie w [hash_multiset](../standard-library/hash-multiset-class.md). Jeśli `hash_multiset` jest pusty, to `hash_multiset::cend == hash_multiset::begin` .

### <a name="remarks"></a>Uwagi

`cend`służy do sprawdzania, czy iterator osiągnął koniec jego `hash_multiset` . Nie można usunąć odwołania do wartości zwracanej przez `cend` .
...
Typ, który dostarcza iterator dwukierunkowy, który może odczytać **`const`** element w hash_multiset.
...
W większości przypadków [const_iterator](#const_iterator) należy używać w celu uzyskania dostępu do elementów w **`const`** obiekcie hash_multiset.
...
Typ, który zawiera odwołanie do **`const`** elementu przechowywanego w hash_multiset do odczytywania i wykonywania **`const`** operacji.
...
Typ, który dostarcza iterator dwukierunkowy, który może odczytać dowolny **`const`** element w hash_multiset.
...
Typ `const_reverse_iterator` nie może zmodyfikować wartości elementu i służy do iterowania przez hash_multiset w odwrocie.
...
`crbegin`jest używany z odwróconym działaniem `hash_multiset` tak samo jak [hash_multiset:: BEGIN](#begin) jest używany z `hash_multiset` .

Z wartością zwracaną `crbegin` , `hash_multiset` nie można zmodyfikować obiektu.

`crbegin`może służyć do iteracji w `hash_multiset` tył.
...
Niepowodzenie odwrotnego iteratora dwukierunkowego, który odnosi się do lokalizacji po ostatnim elemencie w odwróconym [hash_multiset](../standard-library/hash-multiset-class.md) (lokalizacja, która poprzedza pierwszy element w odwrót `hash_multiset` ).

### <a name="remarks"></a>Uwagi

`crend`jest używany z odwróconym działaniem `hash_multiset` tak samo jak [hash_multiset:: end](#end) jest używany z `hash_multiset` .

Z wartością zwracaną `crend` , `hash_multiset` nie można zmodyfikować obiektu.
...
|Parametr|Opis|
|-|-|
|*użyte*|Wartość elementu, który ma zostać wstawiony do [hash_multiset](../standard-library/hash-multiset-class.md) , chyba że `hash_multiset` zawiera już ten element lub, bardziej ogólnie, elementu, którego klucz jest równoważny uporządkowany.|
...
Funkcja członkowska [hash_multiset:: emplace](#emplace) zwraca iterator, który wskazuje na pozycję, w której nowy element został wstawiony do `hash_multiset` .
...
**`true`** Jeśli hash_multiset jest puste; **`false`** jeśli hash_multiset nie jest pusty.
...
`end`służy do sprawdzania, czy iterator osiągnął koniec hash_multiset. Nie można usunąć odwołania do wartości zwracanej przez `end` .
...
Funkcja członkowska zwraca iterator, który odnosi się do elementu w hash_multiset którego klucz sortowania należy `equivalent` do klucza argumentu pod predykatem binarnym, który wywołuje porządkowanie na podstawie mniejszej niż porównywalnej relacji.

Jeśli wartość zwracana `find` jest przypisana do `const_iterator` , nie można zmodyfikować obiektu hash_multiset. Jeśli wartość zwracana `find` jest przypisana do `iterator` , obiekt hash_multiset może być modyfikowany.
...
Aby uzyskać więcej informacji na temat `Allocator` , zobacz sekcję Uwagi w temacie [hash_multiset Class](../standard-library/hash-multiset-class.md) .

### <a name="remarks"></a>Uwagi

Przypisania klasy hash_multiset określają sposób zarządzania magazynem przez klasę. Domyślne przydzielenie klas kontenerów standardowej biblioteki języka C++ jest wystarczające dla większości potrzeb programistycznych. Pisanie i używanie własnej klasy alokatora jest zaawansowanym tematem języka C++.
...
*Wsp*\
Klasa alokatora magazynu, która ma być używana dla tego `hash_multiset` obiektu, która jest domyślna `Allocator` .

*Przepisów*\
Funkcja porównywania typu `const Traits` służąca do porządkowania elementów w `hash_multiset` , które domyślnie są `hash_compare` .

*Kliknij*\
`hash_multiset`Którego konstrukcja `hash_multiset` ma być kopią.
...
Wszystkie konstruktory przechowują typ obiektu alokatora, który zarządza magazynem pamięci dla `hash_multiset` i, który można później zwrócić, wywołując [hash_multiset:: get_allocator](#get_allocator). Parametr alokatora jest często pomijany w deklaracjach klas i makrach przetwarzania wstępnego służących do podstawiania alternatywnych metod przydzielania.

Wszystkie konstruktory inicjują hash_multisets.

Wszystkie konstruktory przechowują obiekt funkcji typu `Traits` , który jest używany do ustanowienia zamówienia między kluczami `hash_multiset` i, które później mogą być zwracane przez wywołanie [hash_multiset:: key_comp](#key_comp). Aby uzyskać więcej informacji na temat `Traits` , zobacz temat [Klasa hash_multiset](../standard-library/hash-multiset-class.md) .
...
Ostatnie trzy konstruktory kopiują zakres [ `first` , `last` ) z `hash_multiset` o rosnącej jawności w określaniu typu funkcji porównania klasy Compare i Alokator.

Rzeczywista kolejność elementów w kontenerze zestawu skrótów zależy od funkcji skrótu, funkcji porządkowania i bieżącego rozmiaru tabeli skrótów i nie może być ogólnie przewidywalna, ponieważ może to być obiektem zestawu, w którym został określony przez funkcję porządkowania.
...
Trzecia funkcja członkowska wstawia sekwencję wartości elementów do hash_multiset odpowiadającej każdemu elementowi, który jest adresowany do zakresu [ `first` , `last` ) określonego hash_multiset.
...
|Parametr|Opis|
|-|-|
|*Kliknij*|[Hash_multiset](../standard-library/hash-multiset-class.md) kopiowana do programu `hash_multiset` .|

### <a name="remarks"></a>Uwagi

Po wymazaniu wszelkich istniejących elementów w programie `hash_multiset` , `operator=` kopiuje lub przenosi zawartość *bezpośrednio* do `hash_multiset` .
...
`rbegin`jest używany z odwróconą hash_multiset, tak jak [początek](#begin) jest używany z hash_multiset.

Jeśli wartość zwracana `rbegin` jest przypisana do `const_reverse_iterator` , wówczas obiekt hash_multiset nie może być modyfikowany. Jeśli wartość zwracana elementu `rbegin` jest przypisana do `reverse_iterator` , można zmodyfikować obiekt hash_multiset.

`rbegin`może służyć do iteracji w hash_multiset do tyłu.
...
`rend`jest używany z odwróconą hash_multiset, tak jak [koniec](#end) jest używany z hash_multiset.

Jeśli wartość zwracana `rend` jest przypisana do `const_reverse_iterator` , wówczas obiekt hash_multiset nie może być modyfikowany. Jeśli wartość zwracana elementu `rend` jest przypisana do `reverse_iterator` , można zmodyfikować obiekt hash_multiset. Nie można usunąć odwołania do wartości zwracanej przez `rend` .
...
Typ `reverse_iterator` jest używany do iteracji przez hash_multiset w odwrotny sposób.
...
Zapoznaj się z przykładem na [value_comp](#value_comp) , aby zapoznać się z przykładem sposobu deklarowania i używania `value_compare` .
