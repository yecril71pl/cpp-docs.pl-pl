...
Klasa kontenera hash_set jest rozszerzeniem standardowej biblioteki języka C++ i służy do przechowywania oraz szybkiego pobierania danych z kolekcji, w której wartości zawartych elementów są unikatowe i służą jako wartości klucza.
...
*Cech*\
Typ, który zawiera dwa obiekty funkcji, jeden z porównywanych klas, który jest predykatem binarnym, może porównać dwie wartości elementów jako klucze sortowania, aby określić ich względną kolejność i funkcję mieszania, która jest jednoargumentowym kluczem mapowania predykatów elementów do niepodpisanych liczb całkowitych typu `size_t` . Ten argument jest opcjonalny i `hash_compare<Key, less<Key> >` jest wartością domyślną.
...
Hash_set porządkuje sekwencję, która kontroluje, przez wywołanie przechowywanego `Traits` obiektu skrótu typu [value_compare](#value_compare). Dostęp do tego przechowywanego obiektu można uzyskać, wywołując funkcję członkowską [key_comp](#key_comp). Taki obiekt funkcji musi zachowywać się tak samo jak obiekt klasy *hash_compare<klucz, mniej \<Key> >.* W odniesieniu do wszystkich wartości `key` typu klucz, cecha wywołania ( `key` ) daje rozkład wartości typu size_t.
...
|Nazwa typu|Opis|
|-|-|
|[allocator_type](#allocator_type)|Typ, który reprezentuje `allocator` klasę dla `hash_set` obiektu.|
|[const_iterator](#const_iterator)|Typ, który dostarcza iterator dwukierunkowy, który może odczytać **`const`** element w `hash_set` .|
|[const_pointer](#const_pointer)|Typ, który dostarcza wskaźnik do **`const`** elementu w `hash_set` .|
|[const_reference](#const_reference)|Typ, który zawiera odwołanie do **`const`** elementu przechowywanego w trakcie `hash_set` operacji odczytywania i wykonywania **`const`** .|
|[const_reverse_iterator](#const_reverse_iterator)|Typ, który dostarcza iterator dwukierunkowy, który może odczytać dowolny **`const`** element w `hash_set` .|
...
|[reverse_iterator](#reverse_iterator)|Typ, który dostarcza iterator dwukierunkowy, który może odczytać lub zmodyfikować element w odwróconej postaci `hash_set` .|
|[size_type](#size_type)|Typ liczby całkowitej bez znaku, który może reprezentować liczbę elementów w `hash_set` .|
|[value_compare](#value_compare)|Typ, który dostarcza dwa obiekty funkcji, Predykat binarny klasy Compare, który może porównać dwie wartości elementów w, `hash_set` Aby określić ich względną kolejność i predykat jednoargumentowy, który miesza elementy.|
...
|[crend](#crend)|Zwraca iterator const, który odnosi się do lokalizacji po ostatnim elemencie w odwróconym stanie `hash_set` .|
...
|[puste](#empty)|Testuje, czy `hash_set` jest pusty.|
...
|[lower_bound](#lower_bound)|Zwraca iterator do pierwszego elementu w elemencie `hash_set` z kluczem, który jest równy lub większy niż określony klucz.|
...
|[value_comp](#value_comp)|Pobiera kopię obiektu cechy skrótu służącego do mieszania i wartości klucza elementu w elemencie `hash_set` .|
...
Jeśli wartość zwracana `begin` jest przypisana do `const_iterator` , elementy w obiekcie hash_set nie mogą być modyfikowane. Jeśli wartość zwracana `begin` jest przypisana do `iterator` , można modyfikować elementy w obiekcie hash_set.
...
Z wartością zwracaną `cbegin` `hash_set` nie można modyfikować elementów w obiekcie.
...
Stały iterator dwukierunkowy, który odnosi się do lokalizacji po ostatnim elemencie w [hash_set](../standard-library/hash-set-class.md). Jeśli `hash_set` jest pusty, to `hash_set::cend == hash_set::begin` .
...
Typ, który dostarcza iterator dwukierunkowy, który może odczytać **`const`** element w hash_set.
...
Typ, który zawiera odwołanie do **`const`** elementu przechowywanego w hash_set do odczytywania i wykonywania **`const`** operacji.
...
Typ, który dostarcza iterator dwukierunkowy, który może odczytać dowolny **`const`** element w hash_set.
...
Typ `const_reverse_iterator` nie może zmodyfikować wartości elementu i służy do iterowania przez hash_set w odwrocie.
...
Niepowodzenie odwrotnego iteratora dwukierunkowego, który odnosi się do lokalizacji po ostatnim elemencie w odwróconym [hash_set](../standard-library/hash-set-class.md) (lokalizacja, która poprzedza pierwszy element w odwrót `hash_set` ).
...
`emplace`Funkcja członkowska zwraca parę, której **`bool`** składnik zwraca **`true`** , jeśli wstawiono, i **`false`** Jeśli `hash_set` już zawiera element, którego klucz ma odpowiednik wartości w kolejności, a Składnik iteratora zwraca adres, w którym został wstawiony nowy element lub w którym element został już umieszczony.
...
**`true`** Jeśli hash_set jest puste; **`false`** jeśli hash_set nie jest pusty.
...
Funkcja członkowska zwraca iterator, który odnosi się do elementu w hash_set którego klucz sortowania należy `equivalent` do klucza argumentu pod predykatem binarnym, który wywołuje porządkowanie na podstawie mniejszej niż porównywalnej relacji.

Jeśli wartość zwracana `find` jest przypisana do `const_iterator` , nie można zmodyfikować obiektu hash_set. Jeśli wartość zwracana `find` jest przypisana do `iterator` , obiekt hash_set może być modyfikowany.
...
Przypisania klasy hash_set określają sposób zarządzania magazynem przez klasę. Domyślne przydzielenie klas kontenerów standardowej biblioteki języka C++ jest wystarczające dla większości potrzeb programistycznych. Pisanie i używanie własnej klasy alokatora jest zaawansowanym tematem języka C++.
...
Wszystkie konstruktory przechowują typ obiektu alokatora, który zarządza magazynem pamięci dla `hash_set` i, który można później zwrócić, wywołując [hash_set:: get_allocator](#get_allocator). Parametr alokatora jest często pomijany w deklaracjach klas i makrach przetwarzania wstępnego służących do podstawiania alternatywnych metod przydzielania.

Wszystkie konstruktory inicjują hash_sets.

Wszystkie konstruktory przechowują obiekt funkcji typu `Traits` , który jest używany do ustanowienia zamówienia między kluczami `hash_set` i, które później mogą być zwracane przez wywołanie [hash_set:: key_comp](#key_comp). Aby uzyskać więcej informacji na temat `Traits` , zobacz temat [Klasa hash_set](../standard-library/hash-set-class.md) .

Pierwszy Konstruktor tworzy puste inicjał `hash_set` drugi określa typ funkcji porównywania ( `Comp` ), która ma zostać użyta w celu ustalenia kolejności elementów, a trzeci jawnie określa typ alokatora ( `Al` ), który ma być używany. Słowo kluczowe **`explicit`** pomija niektóre rodzaje automatycznej konwersji typów.
...
Ostatni konstruktorzy skopiują zakres [ `First` , `Last` ) z `hash_set` o wzrastającej jawności w określaniu typu funkcji porównania cech klasy i alokatora.
...
Pierwsza `insert` funkcja członkowska zwraca parę, której **`bool`** składnik zwraca **`true`** , jeśli wstawiono, i **`false`** Jeśli `hash_set` już zawiera element, którego klucz ma odpowiednik wartości w kolejności, a Składnik iteratora zwraca adres, w którym został wstawiony nowy element lub w którym element został już umieszczony.

Aby uzyskać dostęp do składnika iteratora pary `pr` zwracanej przez tę funkcję elementu członkowskiego, należy użyć `pr.first` i aby usunąć odwołanie do niego, użyj `*(pr.first)` . Aby uzyskać dostęp do **`bool`** składnika pary `pr` zwracanej przez tę funkcję elementu członkowskiego, należy użyć `pr.second` , i aby usunąć odwołanie do niego, użyj `*(pr.second)` .

Druga `insert` funkcja członkowska zwraca iterator, który wskazuje na pozycję, w której nowy element został wstawiony do `hash_set` .
...
Po wymazaniu wszelkich istniejących elementów w programie `hash_set` , `operator=` kopiuje lub przenosi zawartość *bezpośrednio* do `hash_set` .
...
Typ `reverse_iterator` jest używany do iteracji przez hash_set w odwrotny sposób.
