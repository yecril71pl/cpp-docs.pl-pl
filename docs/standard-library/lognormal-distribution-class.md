...
Klasa szablonu opisuje dystrybucję, która produkuje wartości typu całkowitego określonego przez użytkownika, lub typ **Double** , jeśli nie jest podany, dystrybuowany zgodnie z rozkładem normalnym dziennika. Poniższa tabela zawiera linki do artykułów na temat poszczególnych członków.
...
Funkcje `m()` właściwości i `s()` zwracają wartości odpowiednio dla przechowywanych parametrów dystrybucji *m* i *s*.
