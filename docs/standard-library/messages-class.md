...
### <a name="parameters"></a>Parametry  
 `_Catval`  
 Wartość Identyfikacja określenie katalogu wiadomości do wyszukania.  
  
 `_Set`  
 Zidentyfikowany pierwszy używana do lokalizowania komunikat w katalogu wiadomości.  
  
 `_Message`  
 Drugi zidentyfikowane używana do lokalizowania komunikat w katalogu wiadomości.  
  
 `_Dfault`  
 Ciąg, który ma zostać zwrócona w przypadku awarii.  
...
### <a name="parameters"></a>Parametry  
 `_Catname`  
 Nazwa katalogu, który ma zostać wyszukany.  
  
 `_Loc`  
 Ustawienia regionalne są wyszukiwane w katalogu.  
