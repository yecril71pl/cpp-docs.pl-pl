...
### <a name="parameters"></a>Parametry  
 ...
 `_Iosbase`  
 Określony strumień, który zawiera ustawienia regionalne z aspekt numpunct — umożliwia znak przestankowy dane wyjściowe i flagi formatowanie danych wyjściowych.  
  
 `_Fill`  
 Znak używany do rozdzielenia.  
  
 `val`  
 Liczba lub typ Boolean, który ma być danych wyjściowych.  
