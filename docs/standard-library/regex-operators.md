...
Każdy operator szablonu zwraca wartość `!(left == right)` .
...
Każdy operator szablonu zwraca wartość `!(right < left)` .
...
argumenty, których typy są specjalizacją szablonu klasy `match_results` lub `sub_match` są konwertowane przez wywołanie `str` funkcji składowej;

argumenty, których typy są specjalizacją szablonu klasy, nie `basic_string` są zmieniane;
