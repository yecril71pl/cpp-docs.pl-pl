...
Projekty MFC zawsze zawierają kilka plików nagłówkowych, które obsługują system Windows. Są one dodawane za pośrednictwem `#include` instrukcji w `StdAfx.h` pliku:
...
Pliki nagłówkowe zawierają wartości identyfikatorów symboli dla wspólnych wartości MFC. Te symbole są dostępne tylko wtedy, gdy pracujesz nad projektem MFC. `AFX_`Po prefiksie następuje standardowe prefiksy nazw symboli.
...
## <a name="mfc-database-programming"></a>Programowanie baz danych MFC

Symbole w tej tabeli są wyświetlane tylko w projekcie MFC po dodaniu obsługi bazy danych. Obsługują one połączenie MFC z serwerami SQL i programowaniem baz danych.
...
## <a name="mfc-windows-programming"></a>Programowanie MFC systemu Windows
