...
1. Dodaj klasę o nazwie `MyMathFuncs` można wykonywać typowe operacje matematyczne, takie jak dodawanie, odejmowanie, mnożenie i dzielenie. Kod powinien wyglądać następująco:
...
1. Użyj tego pliku źródłowego do implementowania **MyMathFuncs**. Kod powinien wyglądać następująco:

   [!code-cpp[NVC_Walkthrough_Create_Static_Lib#110](../windows/codesnippet/CPP/walkthrough-creating-and-using-a-static-library-cpp_2.cpp)]

1. Kompilowanie statycznej biblioteki łączy, wybierając **kompilacji** > **Kompiluj rozwiązanie** na pasku menu. Tworzy to bibliotekę statyczną, które mogą być używane przez inne programy.
...
1. Określ nazwę dla projektu — na przykład *MyExecRefsLib*— w **nazwa** pole. Z listy rozwijanej liście obok **rozwiązania**, wybierz opcję **Dodaj do rozwiązania**. Spowoduje to dodanie nowego projektu do rozwiązania, które zawiera bibliotekę statyczną. Wybierz **OK** przycisku.
...
1. Przed użyciem procedury matematyczne w bibliotece statycznej, należy to odwołać. Aby to zrobić, otwórz menu skrótów dla **MyExecRefsLib** projektu w **Eksploratora rozwiązań**, a następnie wybierz **Dodaj** > **odwołania**.

1. **Dodaj odwołanie** okno dialogowe wyświetla listę bibliotek, których można się odwołać. **Projektów** zawiera listę wszystkich projektów w bieżącym rozwiązaniu i wszystkie biblioteki w nich zawarte. Na **projektów** zaznacz **MathFuncsLib** pole wyboru, a następnie wybierz **OK** przycisku.
...
1. Teraz możesz używać `MyMathFuncs` klasy w tej aplikacji. Aby to zrobić, Zastąp zawartość `MyExecRefsLib.cpp` przy użyciu tego kodu:
...
1. Aby uruchomić projekt, na pasku menu, wybierz opcję **debugowania** > **Rozpocznij bez debugowania**. Wynik powinien przypominać poniższe:
